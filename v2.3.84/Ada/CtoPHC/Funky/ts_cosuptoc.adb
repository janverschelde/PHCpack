...
procedure ts_cosuptoc is
  ...
  function cosupoly_to_c ( n,ns : integer; s : C_Integer_Array;
  ...
  function cosupsys_to_c ( n,m : integer; moncnt : C_Integer_Array;
  ...
  procedure Export_to_C ( n : in natural; p : in Poly ) is
  ...
  begin
    ...
    return_of_call := cosupoly_to_c(n,sup'length,sup,cff'length,cff);
    if return_of_call = 0
  ...
  begin
    ...
    put("The support :"); put(sup'length,sup); new_line;
    put_line("The coefficients "); put(cff'length,cff);
    return_of_call := cosupsys_to_c(n,m,moncnt,sup'length,sup,cff'length,cff);
    if return_of_call = 0
     then put_line("Call to C terminated normally.");
     else put("Call to C terminated abnormally with exit code ");
          put(return_of_call,1); new_line;
  ...
  begin
    ...
    if ans = '1'
     then put("Give the number of variables : "); get(n);
          Symbol_Table.Init(n);
