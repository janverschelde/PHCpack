...
-- THE INPUT OPERATIONS :
  ...
  procedure get ( n : in out natural32; p : in out Poly_Sys ) is
  begin
    get(Standard_Input,n,p);
  end get;

  procedure get ( n,m : in out natural32; p : in out Poly_Sys ) is
  begin
    get(Standard_Input,n,m,p);
  end get;

  procedure get ( file : in file_type;
                  n : in out natural32; p : in out Poly_Sys ) is
  ...
  procedure get ( file : in file_type;
                  n,m : in out natural32; p : in out Poly_Sys ) is
  ...
  procedure get ( p : in out Poly_Sys ) is
  begin
    get(Standard_Input,p);
  end get;

  procedure get ( file : in file_type; p : in out Poly_Sys ) is
...
-- MORE USER FRIENDLY INPUT OPERATIONS :
  ...
  procedure get ( lp : out Link_to_Poly_Sys ) is
  ...
  begin
    ...
    if onfile = 'y' then
      ...
      exception
        when others =>
          put_line("The data on the file is not correct.");
    ...
    declare
      ...
      exception
        when others =>
          if onfile = 'y' then Close(inpt); end if;
  ...
  procedure get ( file : in file_type; lp : out Link_to_Poly_Sys ) is
...
-- THE OUTPUT OPERATIONS :
  ...
  procedure put ( p : in Poly_Sys; pow : in power ) is
  begin
    put(Standard_Output,p,pow);
  end put;

  procedure put ( file : in file_type; p : in Poly_Sys; pow : in power ) is
  begin
    for i in p'range loop
      put(file,p(i),pow); new_line(file);
    end loop;
  end put;
  ...
  procedure put ( n,m : in natural32; p : in Poly_Sys;
                  s : in Array_of_Symbols; pow : in power := '^' ) is
  begin
    put(Standard_Output,n,m,p,s,pow);
  end put;
  ...
  begin
    for i in p'range loop
      put(file,p(i),s,pow); new_line(file);
    end loop;
  end put;
  ...
  procedure put_line ( p : in Poly_Sys; pow : in Power ) is
  begin
  ...
  begin
    ...
    new_line(file);
    for i in p'range loop
      put_line(file,p(i),pow);
    end loop;
  end put_line;

  procedure put_line ( p : in Poly_Sys; s : in Array_of_Symbols ) is
  begin
  ...
  begin
    ...
    new_line(file);
    for i in p'range loop
      put_line(file,p(i),s,pow);
    end loop;
  end put_line;

  procedure put_pair ( p : in Poly_Sys ) is
  begin
    put_pair(Standard_Output,p);
  end put_pair;

  procedure put_pair ( file : in file_type; p : in Poly_Sys ) is
  begin
    put_pair(file,p,'^');
  end put_pair;

  procedure put_pair ( p : in Poly_Sys; pow : in Power ) is
  begin
  ...
  begin
    ...
    new_line(file);
    for i in p'range loop
      put_pair(file,p(i),pow);
    end loop;
  end put_pair;

  procedure put_pair ( p : in Poly_Sys; s : in Array_of_Symbols ) is
  begin
  ...
  procedure put_pair ( file : in file_type; p : in Poly_Sys;
    ...
    n : constant natural32 := Number_of_Unknowns(p(p'first));
    nq : constant natural32 := p'length;
