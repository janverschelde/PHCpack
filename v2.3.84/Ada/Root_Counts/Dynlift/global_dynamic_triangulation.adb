...
with Standard_Integer_Vertices;
...
package body Global_Dynamic_Triangulation is
  ...
  procedure Initial_Simplex 
               ( pts : in List; order : in boolean;
                 s : out Standard_Integer_Simplices.Simplex;
                 rest : in out List ) is

    n : integer32;
    res,tmp,rest_last : List;
    pt : Link_to_Vector;
    use Standard_Integer_Simplices;
    ...
    if Is_Null(pts) then
      ...
      if integer32(Length_Of(pts)) < n+1 then
        ...
        if order then
          ...
          if Is_Null(res)
           then res := Standard_Integer_Vertices.Extremal_Points(n,pts);
          end if;
        else
          res := Standard_Integer_Vertices.Extremal_Points(n,pts);
        end if;
