...
package body Standard_Integer_Kernel is
  ...
  procedure Normalize_Sign ( v : in out Vector ) is
  begin
  ...
  function Is_In ( v : Vector; x : integer32 ) return boolean is
  begin
    for i in v'range loop
      if v(i) = x
       then return true;
  ...
  function Complement ( n,r : integer32; p : Vector ) return Vector is
  ...
  procedure Upper_Kernel ( U : in Matrix; n,r : in integer32;
                           p : in Vector; V : out Link_to_Matrix ) is

    W : Matrix(1..n,1..n-r);
    B : Matrix(1..r,1..r+1);
    x : Vector(B'range(2));
    q : constant Vector(1..n-r) := Complement(n,r,p); -- nonpivots
