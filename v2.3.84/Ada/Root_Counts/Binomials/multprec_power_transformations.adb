  ...
  begin
    for i in res'range(1) loop
      for j in res'range(2) loop
        if i = j
         then res(i,j) := Multprec_Integer_Numbers.Create(1);
         else res(i,j) := Multprec_Integer_Numbers.Create(0);
        end if;
      end loop;
    end loop;
    return res;
    ...
    if Equal(v(i),0) then
	  ...
	  exit when j /= 0;
        ...
        if j /= 0 then
          ...
          t(i,i) := Create(1); t(j,j) := Create(1);
          t(i,j) := Create(0); t(j,i) := Create(0);
        end if;
        exit when j = 0;
      end loop;
  ...
  begin
    if ind < v'first or ind > v'last then
      ...
      if Equal(v(i),0) then
        ...
        res(i,i) := Create(0);   res(i,ind) := Create(1);
        res(ind,i) := Create(1); res(ind,ind) := Create(0);
        Copy(w(ind),w(i)); Clear(w(ind)); w(ind) := Create(0);
      end if;
      ...
      if j1 /= i then
        ...
        loop
          ...
          t(j1,j1) := Create(1); t(j2,j2) := Create(1);
          t(j1,j2) := Create(0); t(j2,j1) := Create(0);
          if j2 < i
           then j1 := j2;
           else exit;
          end if;
      ...
      if j2 /= i then
	   ...
	   then j2 := j1;
        ...
        if w(i) < 0 then
          Clear(t(i,i)); t(i,i) := Create(1); Min(t(i,i));
          Multprec_Integer_Matrices.Mul2(t,res);
        end if;
      end if;
      Multprec_Integer_Vectors.Clear(w);
