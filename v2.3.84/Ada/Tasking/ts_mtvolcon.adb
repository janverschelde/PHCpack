...
--   Development of multithreaded polyhedral continuation.
  ...
  procedure Solve_Start_Systems
              ( nt : in natural; q : in Laur_Sys;
                m : in natural; mix : in Standard_Integer_Vectors.Vector;
  ...
  begin
    if m = q'last then
      ...
      tstart(timer);
      Semi_Mixed_Start_Systems(q,m,mix,mcc);
      tstop(timer);
      new_line;
      print_times(standard_output,timer,"single thread start solutions");
    end if;
