...
-- CONSTRUCTORS :
  ...
  function Insert_Zero
              ( v : Multprec_Integer_Vectors.Vector; p : integer )
  ...
  begin
    ...
    res(p) := Multprec_Integer_Numbers.Create(0);
  ...
  begin
    for k in v'range loop
      w(k) := Multprec_Integer_Numbers.Create(0);
  ...
  function Filter_non_Vertices
    ...
    ind : integer := res.points'first-1;
  ...
  function Ridges_of_Facet
    ...
    d : constant natural := v'last;
    n : constant natural := s'last;

  begin
    Convex_Hull_of_Ridge(A,v,s,p,M,Cf);
    declare
      mc : constant natural := Multprec_Lattice_3d_Facets.Length_Of(Cf);
  ...
  function Inner_Normal
              ( A : Matrix; p : Standard_Integer_Vectors.Vector;
                k : integer ) return Multprec_Integer_Vectors.Vector is
  ...
  procedure Connect_along_Ridge
               ( f : in Link_to_4d_Facet; r : in natural ) is
...
-- for plain check of Euler characteristic
  ...
  function Vertices ( n : natural; f : Facet_4d_List )
                    return Standard_Integer_Vectors.Vector is
