...
--   Computes the initial form of a polynomial system.
  ...
  function Number_of_Variables ( p : Laur_Sys ) return natural is
  begin
  ...
  function Special_Eliminate ( v : Vector; p : integer ) return Matrix is

  -- DESCRIPTION :
  --   Returns a special elimination matrix, assuming v(p) = 1,
  --   where p is the pivot in v.

    res : Matrix(v'range,v'range) := Identity_Matrix(v'last);
  ...
  procedure Curve_Tropism ( p : in Laur_Sys ) is
    ...
    n : constant natural := Number_of_Variables(p);
    v : Vector(1..n);
