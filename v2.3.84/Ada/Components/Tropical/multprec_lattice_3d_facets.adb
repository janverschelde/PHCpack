...
-- INITIAL FACET :
  ...
  begin
    ...
    if Equal(d(d'first+2),0) then
      res(res'first+2) := Multprec_Integer_Numbers.Create(1); 
...
-- ROTATE facet normal :
  ...
  procedure Connect ( f : in Link_to_3d_Facet; p,i,j : in integer ) is
  ...
  procedure Previous_and_Next_Edge
              ( f : in Link_to_3d_Facet;
                i : in integer; p,q : out integer ) is
  begin
  ...
  procedure Connect ( f,g : in Link_to_3d_Facet; i,j : in integer;
...
-- SELECTORS :
  ...
  function Vertices ( n : natural; f : Facet_3d_List )
                    return Standard_Integer_Vectors.Vector is
  ...
  function Vertices ( n : natural; f : Array_of_3d_Facets )
                    return Standard_Integer_Vectors.Vector is
  ...
  function First_Incident_Vertex ( f : Facet_3d_List ) return integer is
  ...
  procedure Check_Euler_Characteristic
               ( m : in natural; f : in Facet_3d_List ) is
...
-- ENUMERATORS : walk to enumerate vertices, crawl for edges
  ...
  procedure Walk ( f : in Facet_3d_List; v : in integer;
  ...
  procedure Crawl ( f : in Facet_3d_List; v : in integer;
