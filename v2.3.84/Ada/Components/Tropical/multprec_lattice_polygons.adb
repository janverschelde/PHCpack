...
-- I. convert set into lexicographically decreasing matrix
  ...
  begin
    for j in A'range(2) loop
      for i in A'range(1) loop
        v(i) := integer(Multprec_Integer_Numbers.Create(A(i,j)));
      end loop;
      Append(s,last,v);
    end loop;
  end Convert;

  function Is_Larger ( A : Matrix; i,j : natural ) return boolean is
  ...
  procedure Swap ( A : in out Matrix; i,j : in natural ) is
...
-- II. compute vertex set
  ...
  function Position ( A : Matrix; i,j,k : integer ) return integer is
  ...
  function Vertices ( v : Standard_Integer_Vectors.Vector ) return natural is
  ...
  procedure Normal ( V : in Matrix; i : in natural; N : in out Matrix ) is
  ...
  begin
    ...
    if Equal(x,0) then
      N(1,i) := Multprec_Integer_Numbers.Create(1);
      N(2,i) := Multprec_Integer_Numbers.Create(0);
    elsif Equal(y,0) then
      N(1,i) := Multprec_Integer_Numbers.Create(0);
      N(2,i) := Multprec_Integer_Numbers.Create(1);
    else
      d := gcd(x,y);
      N(1,i) := y/d; Min(N(1,i));
      N(2,i) := x/d;
  ...
  function Inner_Product
              ( A,B : in Matrix; i,j : in natural ) return Integer_Number is
  ...
  procedure Orient ( V : in Matrix; i : in natural; N : in out Matrix ) is
  ...
  begin
    if V'last(2) = V'first(2) then
      res(1,res'first(2)) := Multprec_Integer_Numbers.Create(0);
      res(2,res'first(2)) := Multprec_Integer_Numbers.Create(0);
...
-- III. hyperplane representations
  ...
  function Rank ( A : Matrix; i : natural; v : Vector ) return Integer_Number is
  ...
  function Number_of_Minima ( v : Vector ) return natural is
