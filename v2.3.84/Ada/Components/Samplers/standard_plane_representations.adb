...
package body Standard_Plane_Representations is
...
-- AUXILIARY OPERATION FOR COMPUTING GENERATORS :
  ...
  --   Returns the index of the largest entry in v, between a and b.
    ...
    max : constant double_float := Radius(v(a));
    res : integer32 := a;
...
-- DATA STRUCTURE CONVERSIONS :
  ...
  begin
    ...
    for j in v'range loop
      v(j) := new Standard_Complex_Vectors.Vector(b'range);
...
-- FROM EQUATIONS TO GENERATORS :
  ...
  begin
    ...
    for i in 1..n loop
      if i /= piv then
        cnt := cnt + 1;
        directions(cnt)
          := new Standard_Complex_Vectors.Vector'(1..n => Create(0.0));
        directions(cnt)(i) := Create(1.0);
  ...
  procedure Generators ( n,k : in integer32; hyp : in VecVec;
    ...
    mat : Matrix(1..k,1..k);
    rhs : Standard_Complex_Vectors.Vector(1..k);
  ...
  begin
    ...
    for j in k+1..n loop
      ...
      for i in k+1..n loop
        res(i,j-k) := Create(0.0);
      end loop;
      res(j,j-k) := Create(1.0);
    end loop;
    return res;
  end Generators;
  ...
  begin
    ...
    m2 := Standard_Random_Matrices.Orthogonalize(m1);
    for i in v'range(1) loop
      res(i,0) := v(i,0);
      for j in 1..v'last(2) loop
        res(i,j) := m2(i,j);
  ...
  begin
    ...
    for j in res'range loop
      res(j) := new Standard_Complex_vectors.Vector(1..n);
      for i in 1..n loop
        res(j)(i) := m2(i,j);
      end loop;
    end loop;
...
-- FROM GENERATORS TO EQUATIONS :
  ...
  function Equations ( g : Matrix ) return Matrix is
    ...
    mat : Matrix(1..k,1..k);
    b : Standard_Complex_Vectors.Vector(1..k);
    ipvt : Standard_Integer_Vectors.Vector(1..k);
    info : integer32;
    sum : Complex_Number;
  ...
  begin
    ...
    for j in k+1..n loop
      ...
      for i in k+1..n loop
        res(j-k,i) := Create(0.0);
      end loop;
      res(j-k,j) := Create(1.0);
      sum := g(j,0);
      for i in 1..k loop
        sum := sum + b(i)*g(i,0);
      end loop;
...
end Standard_Plane_Representations;
