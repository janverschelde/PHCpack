...
package Multprec_Lattice_Facets is
...
-- DATA STRUCTURES :
  ...
  type Array_of_Facets is array ( integer range <> ) of Link_to_Facet;

  type Facet ( d,n : natural ) is record
    label : natural;                     -- identification number
    ...
    neighbors : Array_of_Facets(1..n);   -- neighbors share an edge
  end record;

  package Lists_of_Facets is new Generic_Lists(Link_to_Facet);
  type Facet_List is new Lists_of_Facets.List;

  type Array_of_Facet_Lists is array ( integer range <> ) of Facet_List;
...
-- INITIAL FACET : compute the first facet
  ...
  function Independent ( A : Matrix; i,j,k : integer ) return boolean;
  ...
  function Third_Point
              ( A : Matrix; i,j : integer; m : Integer_Number;
                v : Multprec_Integer_Vectors.Vector ) return integer;
  ...
  function Largest_Angle
              ( A : Matrix; i,j : integer;
                v,w : Multprec_Integer_Vectors.Vector ) return integer;
  ...
  function Drop ( A : Matrix; p : integer ) return Matrix;

  -- DESCRIPTION :
  --   The matrix on return has the same columns as A,
  --   but with the p-th coordinate dropped.

  function Match ( A,B : Matrix; p,i,j : integer ) return boolean;
  ...
  function Initial_Facet ( A : Matrix ) return Facet;
...
-- ROTATE facet normal :
  ...
  procedure Connect ( f : in Link_to_Facet; p,i,j : in integer );
  ...
  procedure Previous_and_Next_Edge
              ( f : in Link_to_Facet; i : in integer; p,q : out integer );
  ...
  procedure Connect ( f,g : in Link_to_Facet );
...
-- MAIN ALGORITHM :
  ...
  procedure Connect ( f : in Facet_List; lft : in Link_to_Facet );

  -- DESCRIPTION :
  --   Connects the facet lft to the facets in the list f.

  function Convex_Hull_3D ( A : Matrix ) return Facet_List;
...
-- SELECTORS :
  ...
  function Facet_Normals
             ( f : Facet_List ) return Lists_of_Integer64_Vectors.List;
  ...
  function Get_Facet
              ( f : Facet_List; v : Multprec_Integer_Vectors.Vector )
              return Link_to_Facet;

  -- DESCRIPTION :
  --   Returns the facet in the list f with inner normal equal to v.

  function Edges ( f : Facet_List ) return Lists_of_Integer_Vectors.List;
  ...
  function First_Incident_Vertex ( f : Facet_List ) return integer;

  -- DESCRIPTION :
  --   Returns the first vertex incident to the first facet of f,
  --   as a default vertex to start the walk in the enumerators below.

  procedure Check_Euler_Characteristic ( m : in natural; f : in Facet_List );
...
-- ENUMERATORS : walk to enumerate vertices, craw for edges

  generic

    with procedure Report ( w : in integer; continue : out boolean );
  ...
  generic
    ...
    with procedure Report ( v,w : in integer; continue : out boolean );
...
-- RANK : compute the linear span of a set of points.

  function Shift ( A : Matrix; k : integer ) return Matrix;
  ...
  function Rank_of_Upper ( A : Matrix ) return natural;
...
-- DESTRUCTORS :
  ...
  procedure Clear ( f : in out Facet_List );
