...
package Standard_Stacked_Sample_Grids is
...
--   The grids managed by this package contain samples in standard
--   floating-point machine numbers.

-- DATA STRUCTURES :

  type Link_to_Array_of_Standard_Sample_Lists is
    access Array_of_Standard_Sample_Lists;
  ...
  type Stacked_Sample_Grid ( k,d : integer32 ) is record
    ...
    case k is
      when 1 => g : Link_to_Array_of_Standard_Sample_Lists;
...
-- DIAGNOSTICS :
  ...
  function Maximal_Error ( grid : Stacked_Sample_Grid ) return double_float;

  -- DESCRIPTION :
  --   Returns the maximal "err" value for every sample in the grid.

  function Minimal_Distance ( grid : Stacked_Sample_Grid ) return double_float;
...
-- DESTRUCTOR :
  ...
  procedure Clear ( l : in out Link_to_Array_of_Standard_Sample_Lists );
