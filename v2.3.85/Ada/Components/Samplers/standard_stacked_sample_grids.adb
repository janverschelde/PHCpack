...
package body Standard_Stacked_Sample_Grids is

-- CREATORS :

  function Create ( k,n,d : integer32; sps : Standard_Sample_List;
  ...
  begin
    ...
    if k = 1 then
      len := Length_Of(sps);
      res.g := new Array_of_Standard_Sample_Lists(0..integer32(len));
      res.g(0) := sps;
  ...
  function Create_Full ( k,n,d : integer32; sps : Standard_Sample_List;
                         sli : VecVec; rancff : Matrix ) 
                       return Stacked_Sample_Grid is
...
-- DIAGNOSTICS :
  ...
  function Maximal_Error ( grid : Stacked_Sample_Grid ) return double_float is

    res,err : double_float;

  begin
    if grid.k = 1
     then res := Maximal_Error(grid.g.all);
     else res := Maximal_Error(grid.a(1).all);
  ...
  function Minimal_Distance ( grid : Stacked_Sample_Grid )
    ...
    min,dist : double_float;

  begin
    if grid.k = 1
     then min := Minimal_Distance(grid.g.all);
     else min := Minimal_Distance(grid.a(1).all);
          for i in 2..grid.d loop
          ...
          if grid.a(0) /= null
           then dist := Minimal_Distance(grid.a(0).all);
                if dist < min
                 then min := dist;
                end if;
          end if;
...
-- DESTRUCTORS :
  ...
  procedure Clear ( l : in out Link_to_Array_of_Standard_Sample_Lists ) is

    procedure free is
      new unchecked_deallocation(Array_of_Standard_Sample_Lists,
                                 Link_to_Array_of_Standard_Sample_Lists);
  begin
    if l /= null
...
end Standard_Stacked_Sample_Grids;
