...
package body Polynomial_Systems_Container is
...
-- CREATORS :
  ...
  begin
    lp := new Poly_Sys'(1..n => Null_Poly);
  end Initialize;

  procedure Create_Evaluator is
  begin
    if lp /= null
     then ep := new Eval_Poly_Sys'(Create(lp.all));
    end if;
...
-- CONSTRUCTORS :
  ...
  procedure Add_Term ( k : in natural; t : in Term ) is
  begin
    Add(lp(k),t);
  end Add_Term;

  procedure Add_Poly ( k : in natural; p : in Poly ) is
  begin
    Add(lp(k),p);
  end Add_Poly;
...
-- SELECTORS :
  ...
  function Number_of_Terms ( k : natural ) return natural is
  begin
    if lp = null
     then return 0;
  ...
  function Retrieve_Term ( k,i : natural ) return Term is
  ...
  function Retrieve_Poly ( k : natural ) return Poly is
  begin
    if lp = null then
      return Null_Poly;
  ...
  function Retrieve return Link_to_Poly_Sys is
  begin
    return lp;
  end Retrieve;

  function Evaluator return Link_to_Eval_Poly_Sys is
  begin
    return ep;
  end Evaluator;
...
begin
  ...
  jm := null;
  ej := null;
end Polynomial_Systems_Container;
