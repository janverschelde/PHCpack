...
void write_standard_tableau_form(int neq, int nvr, int *nbterms,
  ...
  for (idx = 0; idx < neq; idx++) {
    printf("%d\n", nbterms[idx]);
    for (idxtrm = 1; idxtrm <= nbterms[idx]; idxtrm++) {
      printf("%22.15e  %22.15e", coefficients[cffidx],
             coefficients[cffidx + 1]);
      cffidx = cffidx + 2;
      for (k = 0; k < nvr; k++)
        printf(" %d", exponents[expidx++]);
      printf("\n");
    }
  }
}
...
int store_standard_tableau_form(int neq, int nvr, int *nbterms,
...

  {
    ...
    for (idx = 0; idx < neq; idx++)
      dim[3 + idx] = nbterms[idx];

    dim[neq + 3] = verbose;

    fail = _ada_use_c2phc(889, dim, exponents, coefficients, 0);
  }
  return fail;
}
...
int load_standard_tableau_dimensions(int *neq, int *nvr, int *nbt) {
  ...
  fail = _ada_use_c2phc(890, dim, b, c, 0);
