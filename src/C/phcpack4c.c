...
int read_solution_banner(FILE *fp, int *len, int *dim) {
  ...
  do {
    ...
    else {               /* shift the characters in buffer */
      int i;
      for (i = 0; i < size - 1; i++)
        buffer[i] = buffer[i + 1];
      buffer[size - 1] = c;
...
char *read_solution_banner_and_string(FILE *fp, int k, int *len, int *dim) {
  ...
  if (fail == 0)
    result = read_solution_string(fp, k, *len, *dim);
...
int create_dobldobl_homotopy_with_given_gamma(double gamma_re,
                                              double gamma_im) {
  int *a, *b, fail;
  double c[2];
  c[0] = gamma_re;
...
int create_quaddobl_homotopy_with_given_gamma(double gamma_re,
                                              double gamma_im) {
  int *a, *b, fail;
  double c[2];
  c[0] = gamma_re;
...
int create_multprec_homotopy_with_given_gamma(double gamma_re,
                                              double gamma_im) {
  int *a, *b, fail;
  double c[2];
  c[0] = gamma_re;
