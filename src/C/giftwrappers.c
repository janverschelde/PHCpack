...
int convex_hull_2d(int nc_pts, char *pts, int *nc_hull, char *hull) {
  ...
  fail = _ada_use_c2phc(580, &nc_pts, points, c, 0);

  *nc_hull = nc_pts;

  for (i = 0; i < nc_pts; i++)
    hull[i] = (char)points[i];
...
int convex_hull(int nc_pts, char *pts) {
  ...
  fail = _ada_use_c2phc(581, &nc_pts, points, c, 0);
...
int number_of_facets(int dim, int *nbr) {
  ...
  fail = _ada_use_c2phc(582, &dim, nbr, c, 0);
...
int retrieve_facet(int dim, int fcn, int *nc_rep, char *fctrep) {
  ...
  fail = _ada_use_c2phc(583, &a, b, c, 0);
...
int clear_3d_facets(void) {
  ...
  fail = _ada_use_c2phc(584, a, b, c, 0);
...
int clear_4d_facets(void) {
  ...
  fail = _ada_use_c2phc(585, a, b, c, 0);
...
int support_size(int idx) {
  ...
  fail = _ada_use_c2phc(586, &nbr, b, c, 0);
...
int support_string(int size, char *supp) {
  ...
  fail = _ada_use_c2phc(587, &size, support, c, 0);
  for (i = 0; i < size; i++)
    supp[i] = (char)support[i];
  supp[size] = '\0';
...
int clear_support_string(void) {
  ...
  fail = _ada_use_c2phc(588, a, b, c, 0);
...
int initial_form(int dim, int nbc, char *normal) {
  ...
  for (i = 0; i < nbc; i++)
    nrm[i] = (int)normal[i];
  fail = _ada_use_c2phc(589, pars, nrm, c, 0);
