...
int Pieri_root_count(int m, int p, int q, int *r) {
  ...
  fail = _ada_use_c2phc(223, mpq, r, c, 0);
...
int resolve_Schubert_conditions(int n, int k, int c, int *brackets, int verbose,
  ...
  fail = _ada_use_c2phc(228, dim, brackets, &rc, 0);
...
int standard_Littlewood_Richardson_homotopies(
  ...
  for (i = 0; i < nbchar; i++)
    rc[i] = (double)filename[i];

  fail = _ada_use_c2phc(229, dim, brackets, rc, 0);
...
int dobldobl_Littlewood_Richardson_homotopies(
  ...
  fail = _ada_use_c2phc(180, dim, brackets, rc, 0);
...
int quaddobl_Littlewood_Richardson_homotopies(
  ...
  fail = _ada_use_c2phc(181, dim, brackets, rc, 0);
...
int localization_poset(int m, int p, int q, int *nc, char *ps) {
  ...
  fail = _ada_use_c2phc(224, mpq, b, c, 0);
  *nc = mpq[0];
  for (i = 0; i < *nc; i++)
    ps[i] = (char)b[i];
  ps[*nc] = '\0';
...
int scan_input_planes(int n, int mdim, char *A, double *cff)
/*
...
int scan_complex_interpolation_points(int n, char *points, double *cff)
/*
...
int scan_real_interpolation_points(int n, char *points, double *cff)
/*
 * Scans the string points for n interpolation points.
...
int show_input_planes(int n, int mdim, double *cff)
/*
...
int Pieri_polynomial_system(int m, int p, int q, int nc, char *A, int is_real) {
  ...
  mpq[1] = p;
  mpq[2] = q;
  fail = _ada_use_c2phc(227, mpq, b, cff, 0);
...
int run_Pieri_homotopies(int m, int p, int q, int nc, int *r, char *A,
  ...
  fail = _ada_use_c2phc(225, mpq, r, cff, 0);
...
int real_osculating_planes(int m, int p, int q, int *nc, char *s,
                           char *planes) {
  int i, fail;
  ...
  for (i = 0; i < n; i++)
    coeffs[i] = cffpts[i];
  fail = _ada_use_c2phc(226, mpq, b, coeffs, 0);
