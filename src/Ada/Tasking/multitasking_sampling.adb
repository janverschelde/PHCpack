...
--   of the package Multitasking_Continuation.
  ...
  procedure Driver_to_Sampler
    ...
    h : Standard_Complex_VecVecs.VecVec(1..d)
      := Witness_Sets.Random_Hyperplanes(d,ep'last);
...
-- AUXILIARIES for monodromy breakup :
  ...
  --   with the solutions in s.
    ...
    res : Standard_Complex_Vectors.Vector(1..Length_Of(s));
    tmp : Solution_List := s;
    ls : Link_to_Solution;
  ...
  -- REQUIRED : Length_Of(s1) = Length_Of(s2).
    ...
    dim : constant natural := Length_Of(s1);
  ...
  --   obtained after one loop.
    ...
    p : Standard_Natural_Vectors.Vector(1..Length_Of(s1)) := Map(k,s1,s2,tol);

  begin
    put_line("the permutation : "); put(p); new_line;
    Monodromy_Partitions.Add_Map(deco,nbfac,p);
  ...
  begin
    ...
    for i in 1..10 loop
      -- put_line("going forward ...");
       h := Witness_Sets.Random_Hyperplanes(d,ep'last);
