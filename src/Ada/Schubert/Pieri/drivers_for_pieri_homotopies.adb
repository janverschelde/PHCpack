...
package body Drivers_for_Pieri_Homotopies is
...
--  end Basis_Change;
  ...
  --   Creates a deformation poset and applies the Solve operator.
    ...
    planes : constant VecMat(1..mp) := Random_Complex_Planes(m,p);
  ...
  --   Applies the solver to general intersection conditions.
    ...
    deform_poset : Array_of_Array_of_VecMats(index_poset'range)
                 := Create(index_poset);
    planes : constant VecMat(k'range) := Random_Complex_Planes(m,p,k);
...
-- GENERAL SOLVERS :
  ...
  procedure Driver_for_Pieri_Homotopies ( n,d : in natural32 ) is
