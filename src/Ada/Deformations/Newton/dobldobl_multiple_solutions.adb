...
package body DoblDobl_Multiple_Solutions is
  ...
  begin
    for i in 1..integer32(n) loop
      ...
      if Double_Double_Numbers.to_double(AbsVal(difference)) > tol
       then return false;
      end if;
    end loop;
    return true;
  ...
  procedure Compute_Multiplicities
              ( nd : in out DoblDobl_Deflation_Trees.Node;
                tol : in double_float; n : in natural32 ) is
  ...
  begin
    if not Is_Null(nd.sols) then
      Compute_Multiplicities(nd.sols,tol,n);
...
end DoblDobl_Multiple_Solutions;
