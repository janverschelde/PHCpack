...
package body Multprec_Deflation_Matrices is
  ...
  procedure Multi_Loop
              ( s : in natural32;
                d,n : in Standard_Natural_Vectors.Vector ) is
...
-- MULTIPLIERS :
  ...
  function Multiply ( B : VecMat; x : Multprec_Complex_VecVecs.VecVec )
  ...
  begin
    for i in res'range loop
      res(i) := new Multprec_Complex_Vectors.Vector'(B(i).all*x(i).all);
    end loop;
    return res;
  end Multiply;
  ...
  procedure Multiply ( A : in out Link_to_Matrix; r,c : in integer32;
                       ...
                       Bl : in Multprec_Complex_Vectors.Link_to_Vector ) is
    ...
    for k in 0..m-1 loop
      for i in JM'range(1) loop
        ...
        for j in Bl'first+1..Bl'last loop
          prod := JM(i,j+offset)*Bl(j);
          Add(acc,prod);
          Clear(prod);
        end loop;
        Copy(acc,A(r+i-1,c+k));
        Clear(acc);
      end loop;
  ...
  begin
    ...
    for k in 0..m-1 loop
      ...
      for i in JM'range(1) loop
        ...
        Copy(acc,A(r+i-1,c+k)); Clear(acc);
      end loop;
    end loop;
  end Multiply;
      ...
      for k in 1..nbvars loop           -- select from A into buffer
        ...
        while offset <= nbcols loop
          ...
          for i in 1..nbrows loop
            buffer(i,index) := A(row+i-1,col-1+offset);
          end loop;
      ...
      for j0 in 0..m-1 loop         -- m stages: m-column block structure
        for j1 in 1..integer32(R0(k-1)) loop
          ...
          for j2 in 1..integer32(R0(j0)) loop
            prod := JM(i,JM_offset+j2)*Bl(Bl_offset+j2);
            Add(acc,prod);
            Clear(prod);
          end loop;
          Add(A(r+i-1,c+j1-1),acc);
          Clear(acc);
  ...
  --   If d is mixed, then true is returned, otherwise false is returned.
    ...
    cnt : natural32 := 0;  -- counts nonzero elements in d
  ...
  begin
    res := new Matrix(A'range(1),A'range(2));
  ...
  procedure Multi_Right_Multiply_Deflation
    ...
    begin
      ...
      for j3 in 1..integer32(R0(block)) loop
        prod := JM(i,JM_offset+j3)*Bl(Bl_offset+j3);
        Add(acc,prod);
        Clear(prod);
      end loop;
      A_offset := (j1-1)*integer32(R0(pivot-1)) + j2-1;
      Add(A(r+i-1,c+A_offset),acc); Clear(acc);
    ...
    for i in JM'range(1) loop       -- run over all rows of JM
      ...
      for j0 in 0..m-1 loop         -- m stages: m-column block structure
        for j1 in 1..integer32(R0(k-1)) loop
          acc := Create(integer(0));
          for j2 in 1..integer32(R0(j0)) loop
            prod := JM(i,JM_offset+j2)*B(B_offset+j2,p);
            Add(acc,prod); Clear(prod);
          end loop;
          Add(A(r+i-1,c+j1-1),acc); Clear(acc);
          JM_offset := JM_offset + integer32(R0(j0));
        end loop;
...
-- ASSIGNMENTS :
  ...
  procedure Assign_Scaling_Coefficients
               ( A : in out Link_to_Matrix;
                 h : in Multprec_Complex_Vectors.Link_to_Vector ) is
  ...
  procedure Assign_from_Jacobian_Matrices
               ( A : in out Link_to_Matrix; col : in out integer32;
                 jms : in Link_to_VecMat; nbcols : in integer32 ) is
      ...
      if jms(k) /= null then
        ...
        for i in JM'range(1) loop
          for j in JM'range(2) loop
            Copy(JM(i,j),A(i,j+col-1));
          end loop;
        end loop;
      end if;
      col := col + nbcols;       -- we skip void matrices
    end loop;
  ...
  procedure Assign_Higher_Jacobian_Matrices
    ...
    procedure Assign ( m : in Standard_Natural_Vectors.Vector ) is
      ...
      JM : Link_to_Matrix := jms(ind);

    begin
      if JM /= null then
        for i in JM'range(1) loop
          for j in JM'range(2) loop
            Copy(JM(i,j),A(row+i-1,col+j-1));
          end loop;
        end loop;
      end if;
      col := col + n;
  ...
  procedure Assign_Higher_Jacobian_Matrices
               ( A : in out Link_to_Matrix; row : in integer32;
                 col : in out integer32; jms : in Link_to_VecMat;
                 monkeys : in Standard_Natural64_VecVecs.VecVec;
                 Bl : in Multprec_Complex_Vectors.Link_to_Vector;
                 k,n : in integer32 ) is
  ...
  procedure Assign_Root_Child
               ( A : in out Link_to_Matrix; d0,m : in integer32;
                 R0 : in Standard_Natural_Vectors.Vector;
  ...
  begin
    if ((v0'first(1) = v0'last(1)) and
      ...
      if v0'last(2) = B'last(1) then      -- assign v0*B into A
        ...
        One_Right_Multiply_Deflation(A,m,row,col,1,R0,v0,B);
      else
        One_Right_Multiply_Deflation(A,m,row,col,m,R0,v0,B);
      end if;
    end if;
  ...
  begin
    if ((v0'first(1) = v0'last(1)) and
      ...
      put(file," B'last(1) = ");  put(file,B'last(1),1); new_line(file);
      for i in v0'range(1) loop           -- copy v0 into A
        for j in v0'range(2) loop
          A(i,j) := v0(i,j);
        end loop;
  ...
  --   Assigns into A a Jacobian matrix of the original system.
    ...
    row,nbrows,nbcols,nc,backup_col : integer32;
  ...
  begin
    ...
    for k in 1..v'last loop
      if v(k) /= null then
        ...
        A_offset := Offset_for_One_Right_Multiply(R0,k)*B'last(2);
        nc := integer32(R0(k-1))*B'last(2);
        Alternating_Permute(file,A,row,col+A_offset,JM'last(1),nc,
                            integer32(R0(m)));
      end if;
    end loop;
  end Assign_Children;
  ...
  begin
    if v(0) /= null then             -- assign v(0) into A
      Assign_Root_Child(A,integer32(d0),integer32(m),R0,v(0),monkeys,jrt,B);
    end if;
    ...
    for k in 1..v'last loop          -- compute v(k)*Bl
      if v(k) /= null then
        ...
        if AA'first(1) = AA'last(1) and AA'first(2) = AA'last(2) then
          rp := REAL_PART(AA(AA'first(1),AA'first(2)));
          ind := integer32(Round(rp));
          Clear(rp);
  ...
  begin
    if v(0) /= null then             -- assign v(0) into A
      Assign_Root_Child(file,A,integer32(d0),integer32(m),
                        R0,v(0),monkeys,jrt,B);
    end if;
    col := A'first(2);               -- start column for v(1..m)*Bl
    for k in 1..v'last loop          -- compute v(k)*Bl
      if v(k) /= null then
...
end Multprec_Deflation_Matrices;
