...
with QuadDobl_Multiple_Solutions;       use QuadDobl_Multiple_Solutions;
...
package body QuadDobl_Deflation_Methods is

-- INTERACTIVE NEWTON with DEFLATION :

  procedure Display_Results_of_One_Newton_Step
              ( file : in file_type; z : in QuadDobl_Complex_Vectors.Vector;
                tol : double_float; err,rco,res : in quad_double;
  ...
  procedure One_Symbolic_Newton_Step
              ( file : in file_type;
                ep : in QuadDobl_Complex_Poly_SysFun.Eval_Poly_Sys;
                ejm : in QuadDobl_Complex_Jaco_Matrices.Eval_Jaco_Mat;
                z : in out QuadDobl_Complex_Vectors.Vector;
                tol : in double_float; err,rco,res : out quad_double;
    ...
    use QuadDobl_Complex_Newton_Steps;
  ...
  function Stack ( n : integer32; x : QuadDobl_Complex_VecVecs.VecVec )
  ...
  procedure Split ( z : in QuadDobl_Complex_Vectors.Vector;
                    x : in out QuadDobl_Complex_VecVecs.VecVec ) is
  ...
  procedure One_Algorithmic_Newton_Step
              ( file : in file_type;
                ep : in QuadDobl_Complex_Poly_SysFun.Eval_Poly_Sys;
                ejm : in QuadDobl_Complex_Jaco_Matrices.Eval_Jaco_Mat;
                evt : in QuadDobl_Evaluate_Deflation.Link_to_Eval_Tree;
                ...
                B : in QuadDobl_Complex_VecMats.VecMat;
                h : in QuadDobl_Complex_VecVecs.VecVec;
                x : in out QuadDobl_Complex_VecVecs.VecVec;
    ...
    use QuadDobl_Complex_Poly_SysFun;
    use QuadDobl_Complex_Jaco_Matrices;
    use QuadDobl_Evaluate_Deflation;
    use QuadDobl_Complex_Newton_Steps;
  ...
  procedure Add_Multipliers
               ( file : in file_type; output : in boolean;
                 z : in out QuadDobl_Complex_Vectors.Link_to_Vector;
                 f : in QuadDobl_Complex_Poly_Systems.Poly_Sys;
                 m : in natural32; res : out quad_double ) is

    use QuadDobl_Complex_Vectors;
    use QuadDobl_Deflate_Singularities;
  ...
  begin
    ...
    if res < 0.1 then
      ...
      for i in la'range loop
        nz(z'last+i) := QuadDobl_Complex_Numbers.Create(zero);
      end loop;
    end if;
    Clear(z);
    z := new Vector'(nz);
  ...
  procedure Add_Multipliers_for_Corank_One
               ( file : in file_type; output : in boolean;
                 z : in out QuadDobl_Complex_Vectors.Link_to_Vector;
                 f : in QuadDobl_Complex_Poly_Systems.Poly_Sys;
                 n,m : in natural32; res : out quad_double ) is
  ...
  begin
    ...
    res := Max_Norm(dum);
    if res < 0.0 then
      x(k) := new QuadDobl_Complex_Vectors.Vector'(lambda);
    else
      lambda := (1..integer32(R1(k)) => Create(zero));
      x(k) := new QuadDobl_Complex_Vectors.Vector'(lambda);
    end if;
    QuadDobl_Evaluate_Deflation.Clear(t1);
  end Add_Multipliers;
  ...
  procedure Add_Deflation_Data
              ( k : in integer32; m : in natural32;
                ...
                B : in out QuadDobl_Complex_VecMats.VecMat;
                h : in out QuadDobl_Complex_VecVecs.VecVec ) is

    hk : QuadDobl_Complex_Vectors.Vector(1..integer32(m));
    Bk : QuadDobl_Complex_Matrices.Matrix
  ...
  begin
    ...
    for j in 1..integer32(m) loop
      hk(j) := QuadDobl_Random_Numbers.Random1;
    end loop;
    h(k) := new QuadDobl_Complex_Vectors.Vector'(hk);
    for j1 in Bk'range(1) loop
       for j2 in 1..integer32(m) loop
         Bk(j1,j2) := QuadDobl_Random_Numbers.Random1;
       end loop;
    end loop;
    B(k) := new QuadDobl_Complex_Matrices.Matrix'(Bk);
  ...
  procedure Interactive_Algorithmic_Newton
    ...
    z : QuadDobl_Complex_Vectors.Vector(1..integer32(nv(k)));
  ...
  procedure Deflate
    ...
    use QuadDobl_Complex_Poly_Systems;
    dp : constant Poly_Sys := QuadDobl_Deflate_Singularities.Deflate(p.all,m);
  ...
  procedure Deflate_Corank_One
              ( p : in out QuadDobl_Complex_Poly_Systems.Link_to_Poly_Sys ) is

    use QuadDobl_Complex_Poly_Systems;
    dp : constant Poly_Sys
       := QuadDobl_Deflate_Singularities.Deflate_Corank_One(p.all);
  ...
  procedure Deflate_Corank_One
    ...
    use QuadDobl_Complex_Poly_Systems;
    dp : constant Poly_Sys
       := QuadDobl_Deflate_Singularities.Deflate_Corank_One(p.all,k,nq,nv);
  ...
  procedure Write_Results
               ( file : in file_type; i : natural32;
                 p,dp : in QuadDobl_Complex_Poly_Systems.Poly_Sys;
  ...
  procedure Interactive_Algorithmic_Deflation
    ...
    B : QuadDobl_Complex_VecMats.VecMat(1..max_order);
    h : QuadDobl_Complex_VecVecs.VecVec(1..max_order);
    x : QuadDobl_Complex_VecVecs.VecVec(0..max_order);
    ep : Eval_Poly_Sys(p'range) := Create(p);
  ...
  begin
    ...
    k := 0;
    x(0) := new QuadDobl_Complex_Vectors.Vector'(sol);
    loop
      if k = 0 then
  ...
  begin
    ...
    for i in 1..len loop
      k := 0;
      x(0) := new QuadDobl_Complex_Vectors.Vector'(z);
      loop
        if k = 0 then
          Interactive_Symbolic_Newton(file,p,x(0).all,err,rco,res,tol,rank);
        else
        ...
        if ans = 'y' then
          ...
          put("The number of multipliers : "); put(m,1); new_line; 
          k := k+1;
         -- QuadDobl_Evaluate_Deflation.Clear(evt); OOPS!!
          evt := Create(natural32(k));
          Add_Multiplier_Symbols(natural32(k),m);     
          Add_Deflation_Data(k,m,nv,nq,R1,B,h);
          Add_Multipliers(standard_output,true,ep,ejm,nd,monkeys,k,nv(0..k),
...
-- NEWTON with DEFLATION and CLUSTERING : 
  ...
  procedure Deflate_Solution
              ( file : in file_type; m : in integer32; output : in boolean;
                df : in QuadDobl_Complex_Poly_Systems.Poly_Sys;
                ls : in out QuadDobl_Complex_Solutions.Link_to_Solution ) is
  ...
  procedure Apply_Newton_Step
    ...
    sv : QuadDobl_Complex_Vectors.Vector(ls.v'range);
  ...
  procedure Apply_Newton_Step
    ...
    s : QuadDobl_Complex_Vectors.Vector(1..m);
    z : QuadDobl_Complex_Vectors.Vector(1..integer32(nv(k)));
    evt : QuadDobl_Evaluate_Deflation.Link_to_Eval_Tree
        := QuadDobl_Evaluate_Deflation.Create(natural32(k));
  ...
  begin
    ...
    QuadDobl_Evaluate_Deflation.Clear(evt);
  end Apply_Newton_Step;
  ...
  begin
    for i in 1..nit loop
      evt := QuadDobl_Evaluate_Deflation.Create(natural32(k));
      One_Algorithmic_Newton_Step
        (f,jf,evt,nd,monkeys,k,nv(0..k),nq(0..k),R1(1..k),
         B(1..k),h(1..k),x(0..k),z,tolrnk,err,rco,res,s,rank);
      QuadDobl_Evaluate_Deflation.Clear(evt);
...
end QuadDobl_Deflation_Methods;
