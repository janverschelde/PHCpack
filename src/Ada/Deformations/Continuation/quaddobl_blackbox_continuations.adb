...
with QuadDobl_BlackBox_Refiners;         use QuadDobl_BlackBox_Refiners;
...
-- STABLE POLYNOMIAL CONTINUATION :
  ...
  begin
    if verbose > 0 then
      put("-> in quaddobl_blackbox_continuations.");
      put_line("Stable_Polynomial_Continuation 2 ...");
    end if;
    QuadDobl_Homotopy.Create(p,q,k,gamma);
    QuadDobl_Coefficient_Homotopy.Create(q,p,k,gamma);
    Add(sols,sol);
...
-- GENERAL AND STABLE CONTINUATION :
  ...
  begin
    ...
    Black_Box_Polynomial_Continuation
      (p,q,gamma,sols,sols0,pocotime,verbose-1);
  end Black_Box_Polynomial_Continuation;
...
-- for Laurent polynomial systems :
  ...
  begin
    if verbose > 0 then
      ...
      put_line(", in quaddobl_blackbox_continuations.Main ...");
    end if;
    if targetname /= "" then
