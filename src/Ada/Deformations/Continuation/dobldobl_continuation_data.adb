...
package body DoblDobl_Continuation_Data is
...
-- CREATORS :
  ...
  begin
    res.sol := s;
    Init_Info(res);
    res.cora := Double_Double_Numbers.hi_part(s.err);
    res.rcond := Double_Double_Numbers.hi_part(s.rco);
    res.resa := Double_Double_Numbers.hi_part(s.res);
    return res;
  end Shallow_Create;
  ...
  begin
    s.sol.err := Double_Double_Numbers.Create(s.cora);
    s.sol.rco := Double_Double_Numbers.Create(s.rcond);
    s.sol.res := Double_Double_Numbers.Create(s.resa);
    return s.sol;
  end Shallow_Create;
...
end DoblDobl_Continuation_Data;
