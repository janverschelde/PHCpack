...
package body DoblDobl_Tracked_Solutions_io is
    ...
    if found then
      ...
      if found then
        ...
        if verbose then
          new_line;
          put("Read ");
          put(DoblDobl_Complex_Solutions.Length_Of(qsols),1);
        ...
        if found then
          ...
          if verbose then
            new_line;
            put("Read ");
            put(DoblDobl_Complex_Solutions.Length_Of(psols),1);
            put_line(" solutions.");
          end if;
        end if;
      end if;
...
end DoblDobl_Tracked_Solutions_io;
