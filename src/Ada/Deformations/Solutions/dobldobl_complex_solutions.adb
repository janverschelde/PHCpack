...
package body DoblDobl_Complex_Solutions is
...
-- CREATORS :
  ...
  begin
    res.t := Standard_to_DoblDobl_Complex(s.t);
    res.m := s.m;
    res.v := Standard_to_DoblDobl_Complex(s.v);
  ...
  begin
    res.t := Multprec_to_DoblDobl_Complex(s.t);
    res.m := s.m;
    res.v := Multprec_to_DoblDobl_Complex(s.v);
    res.err := to_double_double(s.err);
    res.rco := to_double_double(s.rco);
    res.res := to_double_double(s.res);
    return res;
  end Create;
    ...
    while not Is_Null(tmp) loop
      declare
        ls : constant Standard_Complex_Solutions.Link_to_Solution
           := Head_Of(tmp);
        ms : constant DoblDobl_Complex_Solutions.Solution(ls.n)
           := Create(ls.all);
      begin
...
end DoblDobl_Complex_Solutions;
