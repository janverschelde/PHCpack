...
package body HexaDobl_Complex_Solutions is
...
-- CREATORS :
  ...
  function Create ( s : Standard_Complex_Solutions.Solution )
    ...
    res : HexaDobl_Complex_Solutions.Solution(s.n);

  begin
    res.t := Standard_to_HexaDobl_Complex(s.t);
    res.m := s.m;
    res.v := Standard_to_HexaDobl_Complex(s.v);
  ...
  begin
    res.t := Multprec_to_HexaDobl_Complex(s.t);
    res.m := s.m;
    res.v := Multprec_to_HexaDobl_Complex(s.v);
    res.err := to_Hexa_double(s.err);
    res.rco := to_Hexa_double(s.rco);
    res.res := to_Hexa_double(s.res);
    return res;
  end Create;
    ...
    while not Is_Null(tmp) loop
      declare
        ls : constant Standard_Complex_Solutions.Link_to_Solution
           := Head_Of(tmp);
        ms : constant HexaDobl_Complex_Solutions.Solution(ls.n)
           := Create(ls.all);
      begin
...
end HexaDobl_Complex_Solutions;
