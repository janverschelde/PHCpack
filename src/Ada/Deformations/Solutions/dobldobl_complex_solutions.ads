...
package DoblDobl_Complex_Solutions is

-- DESCRIPTION :
--   This package provides an abstraction of a list and an array
--   of solutions as vectors of double double complex numbers.
...
-- DATA STRUCTURES :
  ...
  type Solution ( n : integer32 ) is record
    ...
    err : double_double;        -- error = |correction| from Newton
    rco : double_double;        -- inverse of condition number
    res : double_double;        -- norm of residual vector
  end record;
...
-- CREATORS :
  ...
  --   into a representation that uses double double numbers.
