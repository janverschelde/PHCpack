...
package QuadDobl_Complex_Solutions is

-- DESCRIPTION :
--   This package provides an abstraction of a list and an array
--   of solutions as vectors of quad double complex numbers.
...
-- DATA STRUCTURES :
  ...
  type Solution ( n : integer32 ) is record
    ...
    err : quad_double;          -- error = |correction| from Newton
    rco : quad_double;          -- inverse of condition number
    res : quad_double;          -- norm of residual vector
  end record;
...
-- CREATORS :
  ...
  --   into a representation that uses quad double numbers.
