...
-- FOR WARNINGS ABOUT FORMATTING :
  ...
  procedure write_warning ( expected,readchar : in character );

  -- DESCRIPTION :
  --   Writes a warning when the expected character does
  --   not match the the character read.

-- FOR SOLUTION VECTORS ONLY :

  procedure get_vector ( s : in out Solution );
  procedure get_vector ( s : in out Solution; asy : in Array_of_Symbols );
  procedure get_vector ( file : in file_type; s : in out Solution );
  procedure get_vector ( file : in file_type; s : in out Solution;
                         asy : in Array_of_Symbols );

  -- DESCRIPTION :
  --   Reads solution coordinates from standard input or from file.
  --   If an array of symbols is provided as the parameter "asy",
  --   then this array will be used instead of the symbol_table.
  ...
  -- DESCRIPTION :
  --   Writes the coordinates of the solution to screen or to file.
...
-- FOR SOLUTIONS :
  ...
  procedure get ( s : in out Solution; asy : in Array_of_Symbols );
  ...
  --   a vector of s.n complex numbers.  If the array of symbols is
  --   provided, then the "asy" is used to read in the vector.

  procedure try_get ( file : in file_type; s : in out Solution;
                      fail : out boolean );

  -- DESCRIPTION :
  --   Reads a solution from file, skipping lines where exception happen.
...
-- FOR LISTS OF SOLUTIONS :
  ...
  procedure try_get ( file : in file_type; sols : in out Solution_List );
  ...
  -- DESCRIPTION :
  --   This versions of get ignore exceptions when reading numbers...

  procedure put_bar ( file : in file_type );

  -- DESCRIPTION :
  --   Writes one line of 75 equal symbols used as separating bar
  --   to write lists of solutions.
...
-- INCREMENTAL READ and WRITE (for huge solution lists):
  ...
  --   asy       symbol table to be used for reading the vectors.
