...
package Multprec_Solution_Strings is
...
-- PART I: compute lengths and write the strings
  ...
  function Length_Number ( f : Floating_Number ) return natural32;
  ...
  function Write_Diagnostics ( err,rco,res : Floating_Number ) return string;
...
--   than the dimension n, then additional symbols are added.
  ...
  procedure Parse_Diagnostics
               ( s : in string; k : in out integer;
                 err,rco,res : out Floating_Number; fail : out boolean );
