...
package body Test_TD_Newton_Matrix_Series is

  procedure Read_Series_Vector
              ( v : out TripDobl_Complex_Series_Vectors.Link_to_Vector;
  ...
  procedure Test_LU_Newton_Step
              ( p : in TripDobl_CSeries_Poly_Systems.Poly_Sys;
                degree : in integer32;
                x : in TripDobl_Complex_Series_Vectors.Vector ) is
  ...
  --  in double double precision.
    ...
    ans : character;
    otp : boolean;
    z : TripDobl_Complex_Series_Vectors.Vector(x'range);
  ...
  begin
    ...
    if otp then
      TripDobl_Newton_Matrix_Series.LU_Newton_Step
        (standard_output,p,degree,z,info);
    else
      TripDobl_Newton_Matrix_Series.LU_Newton_Step(p,degree,z,info);
    end if;
    if info /= 0 then
      ...
      eva := TripDobl_CSeries_Poly_SysFun.Eval(p,z);
  ...
  --   If otp, then extra output is written to screen.
    ...
    z : TripDobl_Complex_Series_Vectors.Vector(x'range) := x;

  begin
    Complex_Series_and_Polynomials.Set_Degree(z,deg);
    if otp then
      TripDobl_Newton_Matrix_Series.LU_Newton_Steps
        (standard_output,p,deg,maxdeg,nbrit,z,info);
    else
      TripDobl_Newton_Matrix_Series.LU_Newton_Steps(p,deg,maxdeg,nbrit,z,info);
    end if;
    if info /= 0 then
      put("info = "); put(info,1); new_line;
    else
  ...
  begin
    ...
    Complex_Series_and_Polynomials.Set_Degree(z,degree);
    if otp then
      TripDobl_Newton_Matrix_Series.QR_Newton_Step
        (standard_output,p,degree,z,info);
    else
      TripDobl_Newton_Matrix_Series.QR_Newton_Step(p,degree,z,info);
    end if;
    if info /= 0 then
      put("info = "); put(info,1); new_line;
    else
  ...
  begin
    Complex_Series_and_Polynomials.Set_Degree(z,degree);
    if otp then
      TripDobl_Newton_Matrix_Series.QR_Newton_Steps
        (standard_output,p,deg,maxdeg,nbrit,z,info);
    else
      TripDobl_Newton_Matrix_Series.QR_Newton_Steps
  ...
  begin
    ...
    Complex_Series_and_Polynomials.Set_Degree(z,degree);
    if otp then
      TripDobl_Newton_Matrix_Series.SVD_Newton_Step
        (standard_output,p,deg,z,info,rcond);
    else
      TripDobl_Newton_Matrix_Series.SVD_Newton_Step(p,deg,z,info,rcond);
    end if;
  ...
  begin
    ...
    Complex_Series_and_Polynomials.Set_Degree(z,degree);
    if otp then
      TripDobl_Newton_Matrix_Series.SVD_Newton_Steps
  ...
  begin
    ...
    Complex_Series_and_Polynomials.Set_Degree(z,degree);
    if otp then
      TripDobl_Newton_Matrix_Series.Echelon_Newton_Step
        (standard_output,p,degree,z,det);
    else
      TripDobl_Newton_Matrix_Series.Echelon_Newton_Step(p,degree,z,det);
    end if;
  ...
  begin
    Complex_Series_and_Polynomials.Set_Degree(z,deg);
    if otp then
      TripDobl_Newton_Matrix_Series.Echelon_Newton_Steps
  ...
  procedure Test_LU_Newton is
    ...
    ls : TripDobl_CSeries_Poly_Systems.Link_to_Poly_Sys;
    sol : TripDobl_Complex_Series_Vectors.Link_to_Vector;
...
end Test_TD_Newton_Matrix_Series;
