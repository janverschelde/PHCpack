...
--   Test on the application of Newton's method to computed series solutions.
  ...
  --  in standard double precision.
    ...
    ans : character;
    otp : boolean;
    z : Standard_Complex_Series_Vectors.Vector(x'range);
  ...
  begin
    ...
    if ans = 'y' then
      ...
      if info /= 0 then
        ...
        eva := Standard_CSeries_Poly_SysFun.Eval(p,z);
  ...
  --  in quad double precision.
    ...
    ans : character;
    otp : boolean;
    z : QuadDobl_Complex_Series_Vectors.Vector(x'range);
  ...
  begin
    ...
    if ans = 'y' then
      ...
      if info /= 0 then
        ...
        put_line("The updated power series solution :");
        Complex_Series_and_Polynomials_io.put(z);
        eva := QuadDobl_CSeries_Poly_SysFun.Eval(p,z);
        Complex_Series_and_Polynomials.Filter(eva,tol);
        put_line("The evaluated solution :");
        Complex_Series_and_Polynomials_io.put(eva);
      end if;
  ...
  begin
    ...
    if otp then
      ...
      Standard_Newton_Series.LU_Newton_Steps(p,deg,maxdeg,nbrit,z,info);
    end if;
    if info /= 0 then
      put("info = "); put(info,1); new_line;
    else
  ...
  begin
    ...
    if otp then
      ...
      QuadDobl_Newton_Series.LU_Newton_Steps(p,deg,maxdeg,nbrit,z,info);
    end if;
    if info /= 0 then
      put("info = "); put(info,1); new_line;
    else
  ...
  begin
    ...
    if otp then
      ...
      Standard_Newton_Series.QR_Newton_Steps(p,deg,maxdeg,nbrit,z,info);
    end if;
    if info /= 0 then
      put("info = "); put(info,1); new_line;
    else
  ...
  begin
    ...
    if otp then
      ...
      QuadDobl_Newton_Series.QR_Newton_Steps(p,deg,maxdeg,nbrit,z,info);
    end if;
    if info /= 0 then
      put("info = "); put(info,1); new_line;
    else
  ...
  begin
    ...
    Complex_Series_and_Polynomials.Set_Degree(z,degree);
    if otp then
      Standard_Newton_Matrix_Series.SVD_Newton_Step
        (standard_output,p,deg,z,info,rcond);
    else
      Standard_Newton_Matrix_Series.SVD_Newton_Step(p,deg,z,info,rcond);
    end if;
  ...
  begin
    ...
    Complex_Series_and_Polynomials.Set_Degree(z,degree);
    if otp then
      QuadDobl_Newton_Matrix_Series.SVD_Newton_Step
        (standard_output,p,deg,z,info,rcond);
    else
      QuadDobl_Newton_Matrix_Series.SVD_Newton_Step(p,deg,z,info,rcond);
    end if;
    put("rcond = "); put(rcond,3); new_line;
    if one + rcond /= one then
      Complex_Series_and_Polynomials.Filter(z,tol);
  ...
  begin
    ...
    if nbr = 1 then
      ...
      put("Give the maximum degree of the series : "); get(maxdeg);
      Test_QR_Newton_Steps(ls.all,degree,maxdeg,nbr,sol.all);
    end if;
  end QuadDobl_Test_QR_Newton;

  procedure Standard_Test_SVD_Newton is

  -- DESCRIPTION :
  --   Prompts the user for a system of series coefficients
  --   and for an initial series approximation.
  --   Then the test on the SVD Newton method are done,
  --   in standard double precision.
  ...
  procedure DoblDobl_Test_SVD_Newton is
  ...
  procedure QuadDobl_Test_SVD_Newton is
