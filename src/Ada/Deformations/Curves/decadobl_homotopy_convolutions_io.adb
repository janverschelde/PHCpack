...
package body DecaDobl_Homotopy_Convolutions_io is

  function Make_Homotopy ( nq,idx,deg : integer32 ) return Link_to_System is

    res : Link_to_System;
    hom : constant DecaDobl_Complex_Poly_Systems.Poly_Sys(1..nq)
        := DecaDobl_Homotopy.Homotopy_System;
    serhom : DecaDobl_CSeries_Poly_Systems.Poly_Sys(1..nq)
  ...
  begin
    ...
    DecaDobl_Cseries_Poly_Systems.Clear(serhom);
    return res;
  end Make_Homotopy;
  ...
  begin
    Test_Series_Predictors.DecaDobl_Homotopy_Reader(nbeq,idxpar,sols);
    new_line;
    if idxpar = 0 then
      ...
      declare
        dropsols : constant DecaDobl_Complex_Solutions.Solution_List
  ...
  begin
    if artificial then
      DecaDobl_Define_Homotopy(nbq,nvr,gamma,mhom,z,idz,sols);
      idxpar := nbq + 1;
      s := sols;
    else
      mhom := 0;
      Homotopy_Series_Readers.DecaDobl_Parameter_Reader(nbq,nvr,idxpar,sols);
      declare
        dropsols : constant DecaDobl_Complex_Solutions.Solution_List
                 := Solution_Drops.Drop(sols,natural32(idxpar));
      begin
...
end DecaDobl_Homotopy_Convolutions_io;
