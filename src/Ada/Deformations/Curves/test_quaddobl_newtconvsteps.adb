...
package body Test_QuadDobl_NewtConvSteps is
  ...
  begin
    ...
    if useqrls or usesvd then
      ...
      if usesvd then
        QuadDobl_Newton_Convolution_Steps.SVD_Newton_Steps
          (standard_output,s,scf,dx,xd,maxit,nbrit,tol,absdx,fail,
           svl,U,V,info,rcond,ewrk,wrk,scale);
        put("rcond : "); put(rcond,3); new_line;
      else
        QuadDobl_Newton_Convolution_Steps.QR_Newton_Steps
          (standard_output,s,scf,dx,xd,maxit,nbrit,tol,absdx,fail,
  ...
  begin
    new_line;
    put_line("Newton on convolution circuits in quad double precision.");
    new_line;
