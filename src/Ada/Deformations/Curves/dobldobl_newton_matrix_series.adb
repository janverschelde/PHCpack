...
-- ONE NEWTON STEP WITH LU WITHOUT CONDITION NUMBER ESTIMATE :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 1 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 3 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(p,x);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 4 ...");
    end if;
...
-- ONE NEWTON STEP WITH LU ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 5 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 6 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(f,c,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
...
-- ONE NEWTON STEP WITH LU WITH CONDITION NUMBER ESTIMATE :
  ...
  procedure LU_Newton_Step
              ( p : in DoblDobl_CSeries_Poly_Systems.Poly_Sys;
                ...
                degree : in integer32;
                x : in out DoblDobl_Complex_Series_Vectors.Vector;
                rcond : out double_double; vrblvl : in integer32 := 0 ) is
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 7 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(p,x);
    DoblDobl_Complex_Series_Vectors.Min(px);
    Complex_Series_and_Polynomials.Set_degree(px,degree);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 8 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 9 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(p,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Step 10 ...");
    end if;
...
-- ONE NEWTON STEP WITH QR :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Step 1 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Step 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Step 3 ...");
    end if;
    DoblDobl_CSeries_Poly_Systems.Copy(p,wp);
    Complex_Series_and_Polynomials.Set_degree(wp,degree);
    px := DoblDobl_CSeries_Poly_SysFun.Eval(wp,x);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Step 4 ...");
    end if;
...
-- ONE NEWTON STEP WITH QR ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Step 5 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(f,c,x);
    nrm := DoblDobl_CSeries_Vector_Norms.Max_Norm(px);
    if nrm > tol then
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Step 6 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(f,c,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
...
-- ONE NEWTON STEP WITH SVD :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Step 1 ...");
    end if;
    DoblDobl_CSeries_Poly_Systems.Copy(p,wp);
    Complex_Series_and_Polynomials.Set_degree(wp,degree);
    px := DoblDobl_CSeries_Poly_SysFun.Eval(wp,x);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Step 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Step 3 ...");
    end if;
    DoblDobl_CSeries_Poly_Systems.Copy(p,wp);
    Complex_Series_and_Polynomials.Set_degree(wp,degree);
    px := DoblDobl_CSeries_Poly_SysFun.Eval(wp,x);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Step 4 ...");
    end if;
...
-- ONE NEWTON STEP WITH ECHELON FORM :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.Echelon_Newton_Step 1 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(p,x);
    DoblDobl_Complex_Series_Vectors.Min(px);
    Complex_Series_and_Polynomials.Set_degree(px,degree);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.Echelon_Newton_Step 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.Echelon_Newton_Step 3 ...");
    end if;
    px := DoblDobl_CSeries_Poly_SysFun.Eval(p,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.Echelon_Newton_Step 4 ...");
    end if;
...
-- MANY NEWTON STEPS WITH LU WITHOUT CONDITION NUMBER ESTIMATE :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 1 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 4 ...");
    end if;
...
-- MANY NEWTON STEPS WITH LU ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 5 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 6 ...");
    end if;
    for i in 1..nbrit loop
      put(file,"LU Newton step "); put(file,i,1); put_line(file," :");
...
-- MANY NEWTON STEPS WITH LU WITH CONDITION NUMBER ESTIMATE :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 7 ...");
    end if;
    for i in 1..nbrit loop
      LU_Newton_Step(p,jp,degree,x,rcond,vrblvl-1);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 8 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 9 ...");
    end if;
    for i in 1..nbrit loop
      put(file,"LU Newton step "); put(file,i,1); put_line(file," :");
      LU_Newton_Step(file,p,jp,degree,x,rcond,vrblvl-1);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.LU_Newton_Steps 10 ...");
    end if;
...
-- MANY NEWTON STEPS WITH QR :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Steps 1 ...");
    end if;
    for i in 1..nbrit loop
      QR_Newton_Step(p,jp,degree,x,info,vrblvl-1);
      exit when (info /= 0); -- stop if Jacobian matrix is singular
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Steps 4 ...");
    end if;
...
-- MANY QR NEWTON STEPS ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Steps 5 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.QR_Newton_Steps 6 ...");
    end if;
    for i in 1..nbrit loop
      put(file,"QR Newton step "); put(file,i,1); put_line(file," :");
...
-- MANY NEWTON STEPS WITH SVD :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Steps 1 ...");
    end if;
    for i in 1..nbrit loop
      SVD_Newton_Step(p,jp,degree,x,info,rcond,vrblvl-1);
      exit when (one + rcond = one); -- stop if Jacobian matrix is singular
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in dobldobl_newton_matrix_series.SVD_Newton_Steps 4 ...");
    end if;
...
-- MANY NEWTON STEPS WITH ECHELON FORM :
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in dobldobl_newton_matrix_series.Echelon_Newton_Steps 1 ...");
    end if;
    for i in 1..nbrit loop
      Echelon_Newton_Step(p,jp,degree,x,det,vrblvl-1);
      exit when (i = nbrit); -- do not double degree after last step
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in dobldobl_newton_matrix_series.Echelon_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in dobldobl_newton_matrix_series.Echelon_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in dobldobl_newton_matrix_series.Echelon_Newton_Steps 4 ...");
    end if;
