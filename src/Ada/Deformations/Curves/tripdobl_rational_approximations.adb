...
package body TripDobl_Rational_Approximations is

  procedure Denominator_System
              ( numdeg,dendeg : in integer32; 
                cff : in TripDobl_Complex_Vectors.Vector;
                mat : out TripDobl_Complex_Matrices.Matrix;
                rhs : out TripDobl_Complex_Vectors.Vector ) is
  ...
  function Numerator_Coefficients
              ( numdeg,dendeg : integer32;
                dencff,sercff : TripDobl_Complex_Vectors.Vector )
              return TripDobl_Complex_Vectors.Vector is

    res : TripDobl_Complex_Vectors.Vector(0..numdeg);
    mindeg : integer32;
  ...
  procedure Assign_Numerator_Coefficients
              ( numdeg,dendeg : in integer32;
                dencff,sercff : in TripDobl_Complex_Vectors.Vector;
                cff : out TripDobl_Complex_Vectors.Vector ) is
  ...
  procedure Pade ( numdeg,dendeg : in integer32;
                   cff : in TripDobl_Complex_Vectors.Vector;
                   numcff,dencff : out TripDobl_Complex_Vectors.Vector;
                   mat : in out TripDobl_Complex_Matrices.Matrix;
                   rhs : in out TripDobl_Complex_Vectors.Vector;
  ...
  procedure Pade_Vector
              ( numdeg,dendeg : in integer32;
                cff : in TripDobl_Complex_VecVecs.VecVec;
                numcff,dencff : in TripDobl_Complex_VecVecs.VecVec;
                mat : in out TripDobl_Complex_Matrices.Matrix;
                rhs : in out TripDobl_Complex_Vectors.Vector;
                ipvt : in out Standard_Integer_Vectors.Vector;
                info : out integer32; verbose : in boolean := false ) is

    lnkcff,lnknum,lnkden : TripDobl_Complex_Vectors.Link_to_Vector;
  ...
  function Evaluate
              ( p : TripDobl_Complex_Vectors.Vector;
  ...
  function Evaluate
              ( num,den : TripDobl_Complex_Vectors.Vector;
                x : Complex_Number ) return Complex_Number is
  ...
  function Evaluate
              ( num,den : TripDobl_Complex_Vectors.Vector;
                x : triple_double ) return Complex_Number is
  ...
  procedure Evaluate
              ( num,den : in TripDobl_Complex_VecVecs.VecVec;
                x : in Complex_Number;
                eva : out TripDobl_Complex_Vectors.Vector ) is
  begin
...
end TripDobl_Rational_Approximations;
