...
with TripDobl_Series_Matrix_Solvers;    use TripDobl_Series_Matrix_Solvers;
...
package body TripDobl_Newton_Matrix_Series is
...
-- ONE NEWTON STEP WITH LU WITHOUT CONDITION NUMBER ESTIMATE :
  ...
  procedure LU_Newton_Step
    ...
    dx : TripDobl_Complex_Series_Vectors.Vector(x'range);
    px : TripDobl_Complex_Series_Vectors.Vector(p'range);
    jm : TripDobl_Complex_Series_Matrices.Matrix(p'range,x'range);
    xp,xd : TripDobl_Complex_Vector_Series.Vector(degree);
    mj : TripDobl_Complex_Matrix_Series.Matrix(degree);

  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 1 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(p,x);
    TripDobl_Complex_Series_Vectors.Min(px);
    Complex_Series_and_Polynomials.Set_degree(px,degree);
    jm := TripDobl_CSeries_Jaco_Matrices.Eval(jp,x);
    ...
    TripDobl_Complex_Series_Vectors.Clear(px);
    TripDobl_Complex_Series_Matrices.Clear(jm);
    TripDobl_Complex_Matrix_Series.Clear(mj);
    TripDobl_Complex_Vector_Series.Clear(xp);
    TripDobl_Complex_Vector_Series.Clear(xd);
  end LU_Newton_Step;
  ...
  procedure LU_Newton_Step
    ...
    jp : TripDobl_CSeries_Jaco_Matrices.Jaco_Mat(p'range,x'range)
       := TripDobl_CSeries_Jaco_Matrices.Create(p);

  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 2 ...");
    end if;
    LU_Newton_Step(p,jp,degree,x,info,vrblvl-1);
    TripDobl_CSeries_Jaco_Matrices.Clear(jp);
  end LU_Newton_Step;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 3 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(p,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
      TripDobl_Complex_Series_io.put(file,px(i)); new_line(file);
    end loop;
    nrm := TripDobl_CSeries_Vector_Norms.Max_Norm(px);
    ...
    if info /= 0 then
      ...
      for i in dx'range loop
        TripDobl_Complex_Series_io.put(file,dx(i)); new_line(file);
      end loop;
      nrm := TripDobl_CSeries_Vector_Norms.Max_Norm(dx);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 4 ...");
    end if;
...
-- ONE NEWTON STEP WITH LU ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  procedure LU_Newton_Step
    ...
    dx : TripDobl_Complex_Series_Vectors.Vector(x'range);
    px : TripDobl_Complex_Series_Vectors.Vector(f'range);
    jm : TripDobl_Complex_Series_Matrices.Matrix(f'range,x'range);
    xp,xd : TripDobl_Complex_Vector_Series.Vector(degree);
    mj : TripDobl_Complex_Matrix_Series.Matrix(degree);

  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 5 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(f,c,x);
    TripDobl_Complex_Series_Vectors.Min(px);
    Complex_Series_and_Polynomials.Set_degree(px,degree);
    jm := TripDobl_CSeries_Jaco_Matrices.Eval(ejm,mlt,c,x);
    Complex_Series_and_Polynomials.Set_degree(jm,degree);
    mj := TripDobl_Complex_Matrix_Series.Create(jm);
    xp := TripDobl_Complex_Vector_Series.Create(px);
    Solve_by_lufac(mj,xp,info,xd);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 6 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(f,c,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
...
-- ONE NEWTON STEP WITH LU WITH CONDITION NUMBER ESTIMATE :
  ...
  procedure LU_Newton_Step
              ( p : in TripDobl_CSeries_Poly_Systems.Poly_Sys;
                ...
                degree : in integer32;
                x : in out TripDobl_Complex_Series_Vectors.Vector;
                rcond : out triple_double; vrblvl : in integer32 := 0 ) is
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 7 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(p,x);
    TripDobl_Complex_Series_Vectors.Min(px);
    Complex_Series_and_Polynomials.Set_degree(px,degree);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 8 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 9 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(p,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Step 10 ...");
    end if;
...
-- ONE NEWTON STEP WITH QR :
  ...
  procedure QR_Newton_Step
    ...
    wp : TripDobl_CSeries_Poly_Systems.Poly_Sys(p'range);
    dx : TripDobl_Complex_Series_Vectors.Vector(x'range);
    px : TripDobl_Complex_Series_Vectors.Vector(p'range);
    jm : TripDobl_Complex_Series_Matrices.Matrix(p'range,x'range);
    mj : TripDobl_Complex_Matrix_Series.Matrix(degree);
    ...
    wjp : TripDobl_CSeries_Jaco_Matrices.Jaco_Mat(jp'range(1),jp'range(2));

  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Step 1 ...");
    end if;
    TripDobl_CSeries_Poly_Systems.Copy(p,wp);
    Complex_Series_and_Polynomials.Set_degree(wp,degree);
    px := TripDobl_CSeries_Poly_SysFun.Eval(wp,x);
    nrm := TripDobl_CSeries_Vector_Norms.Max_Norm(px);
    if nrm > tol then
      for i in jp'range(1) loop
        for j in jp'range(2) loop
          TripDobl_CSeries_Polynomials.Copy(jp(i,j),wjp(i,j));
        end loop;
      end loop;
      Complex_Series_and_Polynomials.Set_degree(wjp,degree);
      TripDobl_Complex_Series_Vectors.Min(px);
      Complex_Series_and_Polynomials.Set_degree(px,degree);
      jm := TripDobl_CSeries_Jaco_Matrices.Eval(wjp,x);
      Complex_Series_and_Polynomials.Set_degree(jm,degree);
      mj := TripDobl_Complex_Matrix_Series.Create(jm);
      xp := TripDobl_Complex_Vector_Series.Create(px);
      ...
      TripDobl_CSeries_Jaco_Matrices.Clear(wjp);
    end if;
    TripDobl_Complex_Series_Vectors.Clear(px);
    TripDobl_Complex_Series_Matrices.Clear(jm);
    TripDobl_CSeries_Poly_Systems.Clear(wp);
    TripDobl_Complex_Matrix_Series.Clear(mj);
    TripDobl_Complex_Vector_Series.Clear(xp);
    TripDobl_Complex_Vector_Series.Clear(xd);
  end QR_Newton_Step;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Step 2 ...");
    end if;
  ...
  procedure QR_Newton_Step
    ...
    nrm : triple_double;
    tol : constant double_float := 1.0E-26;
    wjp : TripDobl_CSeries_Jaco_Matrices.Jaco_Mat(jm'range(1),jm'range(2));

  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Step 3 ...");
    end if;
    TripDobl_CSeries_Poly_Systems.Copy(p,wp);
    Complex_Series_and_Polynomials.Set_degree(wp,degree);
    px := TripDobl_CSeries_Poly_SysFun.Eval(wp,x);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Step 4 ...");
    end if;
...
-- ONE NEWTON STEP WITH QR ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Step 5 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(f,c,x);
    nrm := TripDobl_CSeries_Vector_Norms.Max_Norm(px);
    if nrm > tol then
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Step 6 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(f,c,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
...
-- ONE NEWTON STEP WITH SVD :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Step 1 ...");
    end if;
    TripDobl_CSeries_Poly_Systems.Copy(p,wp);
    Complex_Series_and_Polynomials.Set_degree(wp,degree);
    px := TripDobl_CSeries_Poly_SysFun.Eval(wp,x);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Step 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Step 3 ...");
    end if;
    TripDobl_CSeries_Poly_Systems.Copy(p,wp);
    Complex_Series_and_Polynomials.Set_degree(wp,degree);
    px := TripDobl_CSeries_Poly_SysFun.Eval(wp,x);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Step 4 ...");
    end if;
...
-- ONE NEWTON STEP WITH ECHELON FORM :
  ...
  procedure Echelon_Newton_Step
    ...
    px : TripDobl_Complex_Series_Vectors.Vector(p'range);
    jm : TripDobl_Complex_Series_Matrices.Matrix(p'range,x'range);
    xp,xd,xdn : TripDobl_Complex_Vector_Series.Vector(degree);
    mj : TripDobl_Complex_Matrix_Series.Matrix(degree);

  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.Echelon_Newton_Step 1 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(p,x);
    TripDobl_Complex_Series_Vectors.Min(px);
    Complex_Series_and_Polynomials.Set_degree(px,degree);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.Echelon_Newton_Step 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.Echelon_Newton_Step 3 ...");
    end if;
    px := TripDobl_CSeries_Poly_SysFun.Eval(p,x);
    put_line(file,"The evaluated series :");
    for i in px'range loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.Echelon_Newton_Step 4 ...");
    end if;
...
-- MANY NEWTON STEPS WITH LU WITHOUT CONDITION NUMBER ESTIMATE :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 1 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 4 ...");
    end if;
...
-- MANY NEWTON STEPS WITH LU ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 5 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 6 ...");
    end if;
    for i in 1..nbrit loop
      put(file,"LU Newton step "); put(file,i,1); put_line(file," :");
...
-- MANY NEWTON STEPS WITH LU WITH CONDITION NUMBER ESTIMATE :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 7 ...");
    end if;
    for i in 1..nbrit loop
      LU_Newton_Step(p,jp,degree,x,rcond,vrblvl-1);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 8 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 9 ...");
    end if;
    for i in 1..nbrit loop
      put(file,"LU Newton step "); put(file,i,1); put_line(file," :");
      LU_Newton_Step(file,p,jp,degree,x,rcond,vrblvl-1);
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.LU_Newton_Steps 10 ...");
    end if;
...
-- MANY NEWTON STEPS WITH QR :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Steps 1 ...");
    end if;
    for i in 1..nbrit loop
      QR_Newton_Step(p,jp,degree,x,info,vrblvl-1);
      exit when (info /= 0); -- stop if Jacobian matrix is singular
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Steps 4 ...");
    end if;
...
-- MANY QR NEWTON STEPS ON COEFFICIENT-PARAMETER HOMOTOPIES :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Steps 5 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.QR_Newton_Steps 6 ...");
    end if;
    for i in 1..nbrit loop
      put(file,"QR Newton step "); put(file,i,1); put_line(file," :");
...
-- MANY NEWTON STEPS WITH SVD :
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Steps 1 ...");
    end if;
    for i in 1..nbrit loop
      SVD_Newton_Step(p,jp,degree,x,info,rcond,vrblvl-1);
      exit when (one + rcond = one); -- stop if Jacobian matrix is singular
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in tripdobl_newton_matrix_series.SVD_Newton_Steps 4 ...");
    end if;
...
-- MANY NEWTON STEPS WITH ECHELON FORM :
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in tripdobl_newton_matrix_series.Echelon_Newton_Steps 1 ...");
    end if;
    for i in 1..nbrit loop
      Echelon_Newton_Step(p,jp,degree,x,det,vrblvl-1);
      exit when (i = nbrit); -- do not double degree after last step
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in tripdobl_newton_matrix_series.Echelon_Newton_Steps 2 ...");
    end if;
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in tripdobl_newton_matrix_series.Echelon_Newton_Steps 3 ...");
    end if;
    for i in 1..nbrit loop
  ...
  begin
    if vrblvl > 0 then
      put_line
        ("-> in tripdobl_newton_matrix_series.Echelon_Newton_Steps 4 ...");
    end if;
