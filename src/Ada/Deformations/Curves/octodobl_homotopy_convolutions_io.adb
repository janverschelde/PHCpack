...
package body OctoDobl_Homotopy_Convolutions_io is

  function Make_Homotopy ( nq,idx,deg : integer32 ) return Link_to_System is

    res : Link_to_System;
    hom : constant OctoDobl_Complex_Poly_Systems.Poly_Sys(1..nq)
        := OctoDobl_Homotopy.Homotopy_System;
    serhom : OctoDobl_CSeries_Poly_Systems.Poly_Sys(1..nq)
  ...
  begin
    ...
    OctoDobl_Cseries_Poly_Systems.Clear(serhom);
    return res;
  end Make_Homotopy;
  ...
  begin
    Test_Series_Predictors.OctoDobl_Homotopy_Reader(nbeq,idxpar,sols);
    new_line;
    if idxpar = 0 then
      ...
      declare
        dropsols : constant OctoDobl_Complex_Solutions.Solution_List
  ...
  begin
    if artificial then
      OctoDobl_Define_Homotopy(nbq,nvr,gamma,mhom,z,idz,sols);
      idxpar := nbq + 1;
      s := sols;
    else
      mhom := 0;
      Homotopy_Series_Readers.OctoDobl_Parameter_Reader(nbq,nvr,idxpar,sols);
      declare
        dropsols : constant OctoDobl_Complex_Solutions.Solution_List
                 := Solution_Drops.Drop(sols,natural32(idxpar));
      begin
...
end OctoDobl_Homotopy_Convolutions_io;
