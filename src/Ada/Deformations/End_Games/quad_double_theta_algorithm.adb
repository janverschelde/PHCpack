...
package body Quad_Double_Theta_Algorithm is

  procedure Allocate
              ( tab : out Quad_Double_VecVecs.VecVec;
  ...
  begin
    ...
    for i in tab'range loop
       declare
         col : constant Quad_Double_Vectors.Vector(0..dim)
             := (0..dim => zero);
         use Quad_Double_Vectors;
       begin
         if tab(i) = null then
           tab(i) := new Quad_Double_Vectors.Vector'(col);
         else -- the dimension may not match, must clear first
           Quad_Double_Vectors.Clear(tab(i));
           tab(i) := new Quad_Double_Vectors.Vector'(col);
         end if;
       end;
    end loop;
  ...
  procedure Columns 
    ...
    d0,d1,d2,newtheta : quad_double;
    prvcol1,prvcol2 : Quad_Double_Vectors.Link_to_Vector;
  ...
  procedure New_Column
    ...
    two : constant quad_double := Quad_Double_Numbers.create(2.0);
    d0,d1,d2,newtheta : quad_double;
    lastcol1,lastcol2 : Quad_Double_Vectors.Link_to_Vector;
...
end Quad_Double_Theta_Algorithm;
