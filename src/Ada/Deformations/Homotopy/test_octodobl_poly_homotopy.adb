...
package body Test_OctoDobl_Poly_Homotopy is
  ...
  begin
    ...
    for i in 1..n loop
      ...
      OctoDobl_Homotopy.Create(p,q,2,a);
      put("after create and before clear #"); put(i,1); put_line(" ...");
      OctoDobl_Homotopy.Clear;
  ...
  procedure Test_Evaluation
    ...
    y : constant Vector := OctoDobl_Homotopy.Eval(x,t);
    px : constant Vector := OctoDobl_Complex_Poly_SysFun.Eval(p,x);
    qx : constant Vector := OctoDobl_Complex_Poly_SysFun.Eval(q,x);
    z : Vector(p'range);
  ...
  begin
    ...
    put(OctoDobl_Homotopy.Dimension,1); new_line;
    loop
      declare
        t : Complex_Number;
	x : Vector(1..nbv);
