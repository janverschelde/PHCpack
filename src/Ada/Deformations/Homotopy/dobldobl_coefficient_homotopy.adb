...
package body DoblDobl_Coefficient_Homotopy is

-- INTERNAL DATA STRUCTURES :

  type homdata ( n : integer32 ) is record
    k : natural32;
    gamma : DoblDobl_Complex_Vectors.Vector(1..n);
    htcff : DoblDobl_Complex_VecVecs.VecVec(1..n);
    cp,cq : DoblDobl_Complex_VecVecs.VecVec(1..n);
...
-- PART I : for one pair of polynomials 
  ...
  function Coefficients ( p : Poly ) return DoblDobl_Complex_Vectors.Vector is

    nbt : constant integer32 := integer32(Number_of_Terms(p));
    res : DoblDobl_Complex_Vectors.Vector(1..nbt);
    idx : integer32 := 0;
  ...
  function Evaluated_Coefficients
             ( nbcff : integer32;
               ...
               t : double_double ) return DoblDobl_Complex_Vectors.Vector is

    res : DoblDobl_Complex_Vectors.Vector(1..nbcff);
...
-- PART II : for a pair of systems
  ...
  function Coefficients
             ( p : Poly_Sys ) return DoblDobl_Complex_VecVecs.VecVec is

    res : DoblDobl_Complex_VecVecs.VecVec(p'range);
    ...
    for i in p'range loop
      declare
        c : constant DoblDobl_Complex_Vectors.Vector
          := Coefficients(p(i));
      begin
        res(i) := new DoblDobl_Complex_Vectors.Vector'(c);
      end;
    end loop;
    return res;
  end Coefficients;
  ...
  procedure Evaluated_Coefficients
    ...
    cffk : DoblDobl_Complex_Vectors.Link_to_Vector;
...
-- PART III : encapsulation
  ...
  begin
    ...
    hd.hf := Create(lh); 
    DoblDobl_Complex_Jaco_Matrices.Create(lh,hd.jf,hd.mf);
    hom := new homdata'(hd);
  ...
  function All_Start_Coefficients return DoblDobl_Complex_VecVecs.VecVec is
  begin
  ...
  function All_Target_Coefficients return DoblDobl_Complex_VecVecs.VecVec is
  begin
  ...
  begin
    if hom /= null then
      ...
      Clear(hom.jf);
      Clear(hom.mf);
      DoblDobl_Complex_VecVecs.Clear(hom.htcff);
      DoblDobl_Complex_VecVecs.Clear(hom.cp);
      DoblDobl_Complex_VecVecs.Clear(hom.cq);
