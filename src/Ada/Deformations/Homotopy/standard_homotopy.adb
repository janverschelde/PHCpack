...
package body Standard_Homotopy is
...
-- AUXILIARIES TO THE CONSTRUCTORS :
  ...
  begin
    ...
    ho.h := Linear_Homotopy(ho.p,ho.q,k,a);
    ho.pe := Create(ho.p);
    ho.qe := Create(ho.q);
    ho.he := Create(ho.h);
    dp := Create(ho.p);
  ...
  begin
    ...
    ho.linear := linear;
    if linear
     then ho.h := Linear_Homotopy(ho.p,ho.q,k,a,b);
     else ho.h := Nonlinear_Homotopy(ho.p,ho.q,k,a,b);      
    end if;
    ho.pe := Create(ho.p);
    ho.qe := Create(ho.q);
    ho.he := Create(ho.h);
