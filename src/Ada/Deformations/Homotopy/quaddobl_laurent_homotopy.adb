...
with QuadDobl_Complex_Laur_Jacomats;     use QuadDobl_Complex_Laur_Jacomats;

package body QuadDobl_Laurent_Homotopy is
    ...
    case ho.ht is
      ...
      when art =>  -- compute  - a*k*(1 - t)^(k-1)*q + b*k*t^(k-1)*p
        declare
          ...
          ddk : constant quad_double := create(ho.k);
        begin
          if ho.linear then
            for i in 1..ho.nq loop
