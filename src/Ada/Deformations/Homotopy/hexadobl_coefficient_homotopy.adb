...
package body HexaDobl_Coefficient_Homotopy is

-- INTERNAL DATA STRUCTURES :

  type homdata ( n : integer32 ) is record
    k : natural32;
    gamma : HexaDobl_Complex_Vectors.Vector(1..n);
    htcff : HexaDobl_Complex_VecVecs.VecVec(1..n);
    cp,cq : HexaDobl_Complex_VecVecs.VecVec(1..n);
...
-- PART I : for one pair of polynomials 
  ...
  function Coefficients ( p : Poly ) return HexaDobl_Complex_Vectors.Vector is

    nbt : constant integer32 := integer32(Number_of_Terms(p));
    res : HexaDobl_Complex_Vectors.Vector(1..nbt);
    idx : integer32 := 0;
  ...
  function Labeled_Coefficients ( p : Poly; real : boolean ) return Poly is
    ...
    res : Poly := Null_Poly;
    cnt : integer32 := 0;
    zero : constant hexa_double := create(integer(0));
  ...
  function Index_of_Labels
             ( c : HexaDobl_Complex_Vectors.Vector; real : boolean )
  ...
  procedure Evaluated_Coefficients
             ( cff : in out HexaDobl_Complex_Vectors.Vector;
               cp,cq : in HexaDobl_Complex_Vectors.Vector;
  ...
  procedure Evaluated_Coefficients
             ( cff : in out HexaDobl_Complex_Vectors.Vector;
               ...
               ip,iq : in Standard_Integer_Vectors.Vector;
               k : in natural32;
               gamma : in HexaDobl_Complex_Vectors.Vector;
  ...
  procedure Evaluated_Coefficients
             ( cff : in HexaDobl_Complex_Vectors.Link_to_Vector;
               cp,cq : in HexaDobl_Complex_Vectors.Link_to_Vector;
  ...
  function Evaluated_Coefficients
             ( nbcff : integer32;
               cp,cq : HexaDobl_Complex_Vectors.Vector;
               ip,iq : Standard_Integer_Vectors.Vector;
               t : hexa_double ) return HexaDobl_Complex_Vectors.Vector is

    res : HexaDobl_Complex_Vectors.Vector(1..nbcff);
...
-- PART II : for a pair of systems
  ...
  function Coefficients
             ( p : Poly_Sys ) return HexaDobl_Complex_VecVecs.VecVec is

    res : HexaDobl_Complex_VecVecs.VecVec(p'range);

  begin
    for i in p'range loop
      declare
        c : constant HexaDobl_Complex_Vectors.Vector
          := Coefficients(p(i));
      begin
        res(i) := new HexaDobl_Complex_Vectors.Vector'(c);
      end;
    end loop;
    return res;
  end Coefficients;
  ...
  function Index_of_Labels
             ( c : HexaDobl_Complex_VecVecs.VecVec; real : boolean )
  ...
  procedure Evaluated_Coefficients
             ( cff : in out HexaDobl_Complex_VecVecs.VecVec;
               cp,cq : in HexaDobl_Complex_VecVecs.VecVec;
               ip,iq : in Standard_Integer_VecVecs.VecVec;
               t : in hexa_double ) is

    cffk : HexaDobl_Complex_Vectors.Link_to_Vector;
...
-- PART III : encapsulation
  ...
  begin
    ...
    hd.hf := Create(lh); 
    HexaDobl_Complex_Jaco_Matrices.Create(lh,hd.jf,hd.mf);
    hom := new homdata'(hd);
  ...
  function All_Start_Coefficients return HexaDobl_Complex_VecVecs.VecVec is
  begin
  ...
  function Start_Coefficients
             ( idx : integer32 )
             return HexaDobl_Complex_Vectors.Link_to_Vector is
  begin
    return hom.cp(idx);
  end Start_Coefficients;

  function All_Target_Coefficients return HexaDobl_Complex_VecVecs.VecVec is
  begin
  ...
  function Eval ( x : HexaDobl_Complex_Vectors.Vector;
                  t : Complex_Number )
                return HexaDobl_Complex_Vectors.Vector is

    res : HexaDobl_Complex_Vectors.Vector(x'range);
  ...
  function Diff ( x : HexaDobl_Complex_Vectors.Vector;
  ...
  begin
    if hom /= null then
      ...
      Clear(hom.jf);
      Clear(hom.mf);
      HexaDobl_Complex_VecVecs.Clear(hom.htcff);
      HexaDobl_Complex_VecVecs.Clear(hom.cp);
      HexaDobl_Complex_VecVecs.Clear(hom.cq);
...
end HexaDobl_Coefficient_Homotopy;
