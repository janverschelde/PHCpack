...
with TripDobl_Random_Numbers;           use TripDobl_Random_Numbers;
...
package body Test_TripDobl_Poly_Homotopy is
  ...
  begin
    ...
    for i in 1..n loop
      ...
      TripDobl_Homotopy.Create(p,q,2,a);
      put("after create and before clear #"); put(i,1); put_line(" ...");
      TripDobl_Homotopy.Clear;
  ...
  procedure Test_Evaluation
    ...
    y : constant Vector := TripDobl_Homotopy.Eval(x,t);
    px : constant Vector := TripDobl_Complex_Poly_SysFun.Eval(p,x);
    qx : constant Vector := TripDobl_Complex_Poly_SysFun.Eval(q,x);
    z : Vector(p'range);
  ...
  begin
    ...
    put(TripDobl_Homotopy.Dimension,1); new_line;
    loop
      declare
        t : Complex_Number;
	x : Vector(1..nbv);
...
end Test_TripDobl_Poly_Homotopy;
