...
with DecaDobl_Random_Numbers;           use DecaDobl_Random_Numbers;
...
package body Test_DecaDobl_Poly_Homotopy is
  ...
  begin
    ...
    for i in 1..n loop
      ...
      DecaDobl_Homotopy.Create(p,q,2,a);
      put("after create and before clear #"); put(i,1); put_line(" ...");
      DecaDobl_Homotopy.Clear;
  ...
  procedure Test_Evaluation
    ...
    y : constant Vector := DecaDobl_Homotopy.Eval(x,t);
    px : constant Vector := DecaDobl_Complex_Poly_SysFun.Eval(p,x);
    qx : constant Vector := DecaDobl_Complex_Poly_SysFun.Eval(q,x);
    z : Vector(p'range);
    qt : constant Complex_Number
       := gamma*(Create(integer(1)) - t)**integer(k);
  ...
  procedure Interactive_Test ( p,q : in Poly_Sys ) is
    ...
    a : constant Complex_Number := Create(integer(1));
    ans : character;
    nbv : constant integer32
        := integer32(Number_of_Unknowns(p(p'first)));
  ...
  begin
    ...
    put(DecaDobl_Homotopy.Dimension,1); new_line;
    loop
      declare
        t : Complex_Number;
	x : Vector(1..nbv);
...
end Test_DecaDobl_Poly_Homotopy;
