...
with PentDobl_Complex_Poly_Functions;    use PentDobl_Complex_Poly_Functions;
...
package body PentDobl_Homotopy is
    ...
    case ho.ht is
      ...
      when art =>  -- compute  - a*k*(1 - t)^(k-1)*q + b*k*t^(k-1)*p
        declare
          ...
          dd_k : constant penta_double
               := Penta_Double_Numbers.create(ho.k);
        begin
          if ho.linear then
            for i in 1..ho.nq loop
            ...
            if is_zero(AbsVal(t)) then
              if ho.k = 1 then
                ...
                return Mul(q_factor,ho.q);
                 ...
                 and then abs(IMAG_PART(t)) + one = one then
              return PentDobl_Complex_Numbers.create(dd_k)*ho.p;
            else
