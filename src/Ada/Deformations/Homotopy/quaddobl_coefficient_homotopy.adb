...
package body QuadDobl_Coefficient_Homotopy is

-- INTERNAL DATA STRUCTURES :

  type homdata ( n : integer32 ) is record
    k : natural32;
    gamma : QuadDobl_Complex_Vectors.Vector(1..n);
    htcff : QuadDobl_Complex_VecVecs.VecVec(1..n);
    cp,cq : QuadDobl_Complex_VecVecs.VecVec(1..n);
...
-- PART I : for one pair of polynomials 
  ...
  function Coefficients ( p : Poly ) return QuadDobl_Complex_Vectors.Vector is

    nbt : constant integer32 := integer32(Number_of_Terms(p));
    res : QuadDobl_Complex_Vectors.Vector(1..nbt);
    idx : integer32 := 0;
  ...
  function Labeled_Coefficients ( p : Poly; real : boolean ) return Poly is
    ...
    res : Poly := Null_Poly;
    cnt : integer32 := 0;
    zero : constant quad_double := create(integer(0));
  ...
  function Index_of_Labels
             ( c : QuadDobl_Complex_Vectors.Vector; real : boolean )
  ...
  procedure Evaluated_Coefficients
             ( cff : in out QuadDobl_Complex_Vectors.Vector;
               cp,cq : in QuadDobl_Complex_Vectors.Vector;
  ...
  function Evaluated_Coefficients
             ( nbcff : integer32;
               cp,cq : QuadDobl_Complex_Vectors.Vector;
               ip,iq : Standard_Integer_Vectors.Vector;
               t : quad_double ) return QuadDobl_Complex_Vectors.Vector is

    res : QuadDobl_Complex_Vectors.Vector(1..nbcff);
...
-- PART II : for a pair of systems
  ...
  function Coefficients
             ( p : Poly_Sys ) return QuadDobl_Complex_VecVecs.VecVec is

    res : QuadDobl_Complex_VecVecs.VecVec(p'range);
    ...
    for i in p'range loop
      declare
        c : constant QuadDobl_Complex_Vectors.Vector
          := Coefficients(p(i));
      begin
        res(i) := new QuadDobl_Complex_Vectors.Vector'(c);
      end;
    end loop;
    return res;
  end Coefficients;
  ...
  function Index_of_Labels
             ( c : QuadDobl_Complex_VecVecs.VecVec; real : boolean )
  ...
  procedure Evaluated_Coefficients
             ( cff : in out QuadDobl_Complex_VecVecs.VecVec;
               cp,cq : in QuadDobl_Complex_VecVecs.VecVec;
               ip,iq : in Standard_Integer_VecVecs.VecVec;
               t : in quad_double ) is

    cffk : QuadDobl_Complex_Vectors.Link_to_Vector;
...
-- PART III : encapsulation
  ...
  begin
    ...
    hd.hf := Create(lh); 
    QuadDobl_Complex_Jaco_Matrices.Create(lh,hd.jf,hd.mf);
    hom := new homdata'(hd);
  ...
  begin
    if hom /= null then
      ...
      Clear(hom.jf);
      Clear(hom.mf);
      QuadDobl_Complex_VecVecs.Clear(hom.htcff);
      QuadDobl_Complex_VecVecs.Clear(hom.cp);
      QuadDobl_Complex_VecVecs.Clear(hom.cq);
...
end QuadDobl_Coefficient_Homotopy;
