...
with DecaDobl_Complex_Poly_SysFun;      use DecaDobl_Complex_Poly_SysFun;
with DecaDobl_Complex_Jaco_Matrices;    use DecaDobl_Complex_Jaco_Matrices;

package body DecaDobl_Coefficient_Homotopy is

-- INTERNAL DATA STRUCTURES :

  type homdata ( n : integer32 ) is record
    k : natural32;
    gamma : DecaDobl_Complex_Vectors.Vector(1..n);
    htcff : DecaDobl_Complex_VecVecs.VecVec(1..n);
    cp,cq : DecaDobl_Complex_VecVecs.VecVec(1..n);
...
-- PART I : for one pair of polynomials 
  ...
  function Coefficients ( p : Poly ) return DecaDobl_Complex_Vectors.Vector is

    nbt : constant integer32 := integer32(Number_of_Terms(p));
    res : DecaDobl_Complex_Vectors.Vector(1..nbt);
    idx : integer32 := 0;
  ...
  function Labeled_Coefficients ( p : Poly; real : boolean ) return Poly is
    ...
    res : Poly := Null_Poly;
    cnt : integer32 := 0;
    zero : constant deca_double := create(integer(0));
  ...
  function Index_of_Labels
             ( c : DecaDobl_Complex_Vectors.Vector; real : boolean )
  ...
  procedure Evaluated_Coefficients
             ( cff : in out DecaDobl_Complex_Vectors.Vector;
               cp,cq : in DecaDobl_Complex_Vectors.Vector;
  ...
  procedure Evaluated_Coefficients
             ( cff : in out DecaDobl_Complex_Vectors.Vector;
               ...
               ip,iq : in Standard_Integer_Vectors.Vector;
               k : in natural32;
               gamma : in DecaDobl_Complex_Vectors.Vector;
  ...
  procedure Evaluated_Coefficients
             ( cff : in DecaDobl_Complex_Vectors.Link_to_Vector;
               cp,cq : in DecaDobl_Complex_Vectors.Link_to_Vector;
  ...
  function Evaluated_Coefficients
             ( nbcff : integer32;
               cp,cq : DecaDobl_Complex_Vectors.Vector;
               ip,iq : Standard_Integer_Vectors.Vector;
               t : deca_double ) return DecaDobl_Complex_Vectors.Vector is

    res : DecaDobl_Complex_Vectors.Vector(1..nbcff);
...
-- PART II : for a pair of systems
  ...
  function Coefficients
             ( p : Poly_Sys ) return DecaDobl_Complex_VecVecs.VecVec is

    res : DecaDobl_Complex_VecVecs.VecVec(p'range);

  begin
    for i in p'range loop
      declare
        c : constant DecaDobl_Complex_Vectors.Vector
          := Coefficients(p(i));
      begin
        res(i) := new DecaDobl_Complex_Vectors.Vector'(c);
      end;
    end loop;
    return res;
  end Coefficients;
  ...
  function Index_of_Labels
             ( c : DecaDobl_Complex_VecVecs.VecVec; real : boolean )
  ...
  procedure Evaluated_Coefficients
             ( cff : in out DecaDobl_Complex_VecVecs.VecVec;
               cp,cq : in DecaDobl_Complex_VecVecs.VecVec;
               ip,iq : in Standard_Integer_VecVecs.VecVec;
               t : in deca_double ) is

    cffk : DecaDobl_Complex_Vectors.Link_to_Vector;
...
-- PART III : encapsulation
  ...
  begin
    ...
    hd.hf := Create(lh); 
    DecaDobl_Complex_Jaco_Matrices.Create(lh,hd.jf,hd.mf);
    hom := new homdata'(hd);
  ...
  function All_Start_Coefficients return DecaDobl_Complex_VecVecs.VecVec is
  begin
  ...
  function All_Target_Coefficients return DecaDobl_Complex_VecVecs.VecVec is
  begin
  ...
  function Eval ( x : DecaDobl_Complex_Vectors.Vector;
                  t : Complex_Number )
  ...
  function Diff ( x : DecaDobl_Complex_Vectors.Vector;
  ...
  begin
    if hom /= null then
      ...
      Clear(hom.jf);
      Clear(hom.mf);
      DecaDobl_Complex_VecVecs.Clear(hom.htcff);
      DecaDobl_Complex_VecVecs.Clear(hom.cp);
      DecaDobl_Complex_VecVecs.Clear(hom.cq);
...
end DecaDobl_Coefficient_Homotopy;
