...
package body QuadDobl_Orthogonal_Correctors is
  ...
  begin
    while nit < c.maxit loop
      ...
      Add(s.sol.v,dx);
      s.cora := hihi_part(Norm(dx)); s.length_path := s.length_path + s.cora;
      y := H(s.sol.v,s.sol.t);
      s.resa := hihi_part(Norm(y));
  ...
  begin
    while nit < c.maxit loop
      ...
      s.rcond := hihi_part(Inverse_Condition_Number(sv));
      Min(y);
      dx := Solve(u,v,sv,y);
      Add(s.sol.v,dx);
      s.cora := hihi_part(Norm(dx)); s.length_path := s.length_path + s.cora;
