...
package body Standard_Quad_Parameters is
...
-- REPORTING and TUNING :
  ...
  begin
    loop
      ...
      exit when (ans = '0');
      case ans is
        when '1' => put("Give new maximal number of predictor steps : ");
                    Numbers_io.Read_Positive(integer(max_predictor_steps));
        when '2' => put("Give new maximal step size for predictor : ");
                    Numbers_io.Read_Positive_Float(max_step_size);
        when '3' => put("Give new multiplier to reduce the step size : ");
                    Numbers_io.Read_Positive_Float(reduction_multiplier);
        when '4' => put("Give new multiplier to expand the step size : ");
                    Numbers_io.Read_Positive_Float(expansion_multiplier);
        when '5' => put("Give new threshold to delay the expansion : ");
                    Numbers_io.Read_Positive(integer(expansion_threshold));
        when '6' => put("Give new tolerance of evaluation residual : ");
                    Numbers_io.Read_Positive_Float(residual_tolerance);
        when '7' => put("Give new tolerance on smallest increment : ");
                    Numbers_io.Read_Positive_Float(increment_tolerance);
        when '8' => put("Give new maximal number of corrector steps : ");
                    Numbers_io.Read_Positive(integer(max_corrector_steps));
        when '9' => put("Give new tolerance of Jacobian determinant : ");
                    Numbers_io.Read_Positive_Float(determinant_tolerance);
        when others => Reset;
      end case;
    end loop;
  end Tune;
