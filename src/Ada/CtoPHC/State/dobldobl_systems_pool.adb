...
package body DoblDobl_Systems_Pool is
...
-- DESTRUCTORS :
  ...
  begin
    if k > 0 and k <= size_pool
     then DoblDobl_Complex_Poly_Systems.Clear(sp(k));
    end if;
  end Clear_System;
  ...
  begin
    if k > 0 and k <= size_pool
     then DoblDobl_Complex_Poly_SysFun.Clear(ep(k));
    end if;
  end Clear_Evaluator;
  ...
  begin
    if k > 0 and k <= size_pool
     then DoblDobl_Complex_Jaco_Matrices.Clear(jm(k));
    end if;
  ...
  begin
    if k > 0 and k <= size_pool
     then DoblDobl_Complex_Jaco_Matrices.Clear(jf(k));
    end if;
