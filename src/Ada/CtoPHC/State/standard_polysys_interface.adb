...
package body Standard_PolySys_Interface is
  ...
  begin
    if vrblvl > 0
     then put_line("-> in use_syscon.Standard_PolySys_Read ...");
    end if;
    new_line;
  ...
  exception
    when others => 
      if vrblvl > 0 then
        put_line("Exception raised in use_syscon.Standard_PolySys_Read.");
      end if;
      return 20;
  ...
  begin
    if vrblvl > 0
     then put_line("-> in use_syscon.Standard_PolySys_Write ...");
    end if;
    if lp /= null then
  ...
  exception
    when others => 
      if vrblvl > 0 then
        put_line("Exception raised in use_syscon.Standard_PolySys_Write.");
      end if;
      return 21;
  end Standard_PolySys_Write;

  function Standard_PolySys_Dimension
             ( a : C_intarrs.Pointer;
               vrblvl : integer32 := 0 ) return integer32 is
  begin
    if vrblvl > 0
     then put_line("-> in use_syscon.Standard_PolySys_Dimension ...");
    end if;
  ...
  exception
    when others => 
      if vrblvl > 0 then
        put_line("Exception raised in use_syscon.Standard_PolySys_Dimension.");
      end if;
      return 22;
  end Standard_PolySys_Dimension;

  function Standard_PolySys_Initialize_Dimension
             ( a : C_intarrs.Pointer;
               vrblvl : integer32 := 0 ) return integer32 is
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in use_syscon.Standard_PolySys_Initialize_Dimension ...");
    end if;
  ...
  exception
    when others => 
      if vrblvl > 0 then
        put("Exception raised in use_syscon.");
        put_line("Standard_PolySys_Initialize_Dimension.");
      end if;
      return 23;
  end Standard_PolySys_Initialize_Dimension;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in use_syscon.Standard_PolySys_Size ...");
    end if;
  ...
  exception
    when others => 
      if vrblvl > 0 then
        put_line("Exception raised in use_syscon.Standard_PolySys_Size");
      end if;
      return 24;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in use_syscon.Standard_PolySys_Get_Term ...");
    end if;
    Assign(t.cf,c);
    Assign(t.dg.all,b);
    return 0;
  exception
    when others => 
      if vrblvl > 0 then
        put_line("Exception raised in use_syscon.Standard_PolySys_Get_Term");
      end if;
      return 25;
  ...
  begin
    if vrblvl > 0 then
      put_line("-> in use_syscon.Standard_PolySys_Add_Term ...");
    end if;
    Assign(c,t.cf);
  ...
  exception
    when others => 
      if vrblvl > 0 then
        put_line("Exception raised in use_syscon.Standard_PolySys_Add_Term");
      end if;
      return 26;
  ...
  begin
    if vrblvl > 0
     then put_line("-> in use_syscon.Standard_PolySys_Clear ...");
    end if;
  ...
  exception
    when others => 
      if vrblvl > 0 then
        put_line("Exception raised in use_syscon.Standard_PolySys_Clear.");
      end if;
      return 27;
