...
--   Development of tools to work with products of polynomials.
  ...
  -- DESCRIPTION :
  --   Prompts the user for a polynomial system and creates a linear-product
  ...
  begin
    ...
    if ans = '0'
     ...
     else Drivers_for_Set_Structures.Read_Set_Structure(natural32(lp'last));
    end if;
    new_line;
    ...
    begin
      ...
      Ask_Yes_or_No(ans);
      if ans = 'y' 
       then Count_All_Solutions_of_Linear_Product_System(rq);
      end if;
    end;
  ...
  procedure Driver_to_Count_Roots ( q : in Prod_Sys ) is

  -- DESCRIPTION :
  --   Offers the user a menu of two possible root counting methods.
  ...
  -- DESCRIPTION :
  --   Prompts the user for a linear-product system
  ...
  -- DESCRIPTION :
  --   The first way to evaluate a linear-product system
  --   is to expand into a polynomial system. 
