...
with QuadDobl_Complex_Laur_Functions;    use QuadDobl_Complex_Laur_Functions;
...
package body QuadDobl_Polyhedral_Continuation is
...
-- SANITY CHECKS :
  ...
  procedure Extract_Regular ( sols : in out Solution_List ) is
    ...
    procedure Extract_Regular_Solutions is new 
                QuadDobl_Complex_Solutions.Delete(To_Be_Removed);
...
-- FIRST LAYER OF CONTINUATION ROUTINES :
  ...
  procedure Mixed_Continuation
    ...
    ctm : QuadDobl_Complex_VecVecs.VecVec(c'range);
    zero : constant quad_double := create(0.0);

    function Eval ( x : QuadDobl_Complex_Vectors.Vector; t : Complex_Number )
    ...
    function dHt ( x : QuadDobl_Complex_Vectors.Vector; t : Complex_Number )
                 return QuadDobl_Complex_Vectors.Vector is
    ...
    function dHx ( x : QuadDobl_Complex_Vectors.Vector; t : Complex_Number )
                 return matrix is
...
-- UTILITIES FOR SECOND LAYER :
  ...
  --   that belong to q.
    ...
    res : QuadDobl_Complex_VecVecs.VecVec(c'range);

  begin
    for i in mix'range loop
      declare
        cri : constant QuadDobl_Complex_Vectors.Vector
            := Sub_Polyhedral_Homotopy(mic.pts(i),e(i).all,c(i).all);
      begin
        res(i) := new QuadDobl_Complex_Vectors.Vector'(cri);
        for j in 1..(mix(i)-1) loop
          declare
            crj : constant QuadDobl_Complex_Vectors.Vector
                := Sub_Polyhedral_Homotopy(mic.pts(i),e(i+j).all,c(i+j).all);
          begin
            res(i+j) := new QuadDobl_Complex_Vectors.Vector'(crj);
          end;
        end loop;
      end;
    end loop;
  ...
  -- REQUIRED : mic.sub /= null.
    ...
    cq : QuadDobl_Complex_VecVecs.VecVec(c'range)
       := Sub_Polyhedral_Homotopy(mix,mic,e,c);

  begin
    Mixed_Solve(q,lif,h,cq,e,j,m,mix,mic.sub.all,qsols);
    QuadDobl_Complex_VecVecs.Clear(cq); Deep_Clear(lif);
...
-- SECOND LAYER :
  ...
  begin
    QuadDobl_Simpomial_Solvers.Solve
    ...
    if fail then
      ...
      Set_Continuation_Parameter(qsols,QuadDobl_Complex_Numbers.Create(zero));
    end if;
    len := Length_Of(qsols);
    if len > 0 then
   ...
   -- end if;
    ...
    exception
      when others =>
        ...
        QuadDobl_Complex_Laur_Systems_io.put(q);
        raise;
    end;
   -- if multprec_hermite
...
end QuadDobl_Polyhedral_Continuation;
