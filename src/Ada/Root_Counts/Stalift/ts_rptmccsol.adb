...
--   random coefficient system, given a mixed cell configuration.
  ...
  --   using the coefficients in the system q.
    ...
    powers : Standard_Floating_VecVecs.VecVec(1..cq'last);
    minpwr : double_float := 1.0;

  begin
    Make_Powers(file,cq,mix,lif,mic,powers);
    ...
    put(file,powers);
    Make_Homotopy(deg,point,cq,mix,lif,powers,thm);
