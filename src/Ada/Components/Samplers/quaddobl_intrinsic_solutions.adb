...
package body QuadDobl_Intrinsic_Solutions is
  ...
  begin
    res.t := s.t;
    res.m := s.m;
    res.v := QuadDobl_Point_Coordinates.Project(s.v(b'range),b,v);
    res.err := s.err;
    res.rco := s.rco;
    res.res := s.res;
    return res;
  ...
  begin
    res.t := s.t;
    res.m := s.m;
    res.v := QuadDobl_Point_Coordinates.Project(s.v(p'range(1)),p);
    res.err := s.err;
    res.rco := s.rco;
    res.res := s.res;
    return res;
  ...
  begin
    res.t := s.t;
    res.m := s.m;
    res.v := QuadDobl_Point_Coordinates.Affine_Expand(s.v,b,v);
    res.err := s.err;
    res.rco := s.rco;
    res.res := s.res;
    return res;
  ...
  begin
    res.t := s.t;
    res.m := s.m;
    res.v := QuadDobl_Point_Coordinates.Affine_Expand(s.v,p);
    res.err := s.err;
    res.rco := s.rco;
    res.res := s.res;
    return res;
