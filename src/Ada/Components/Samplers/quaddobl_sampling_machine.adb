...
package body QuadDobl_Sampling_Machine is

-- INTERNAL DATA :

  qdsys : QuadDobl_Complex_Poly_Systems.Link_to_Poly_Sys;
  qdsys_eval : QuadDobl_Complex_Poly_SysFun.Link_to_Eval_Poly_Sys;
  qdjac_eval : QuadDobl_Complex_Jaco_Matrices.Link_to_Eval_Jaco_Mat;

-- PARAMETERS FOR ROOT REFINER :

  epsxa,epsfa,tolsing : quad_double;
  maxit : natural32;
...
-- AUXILIARY ROUTINES :
  ...
  function QuadDobl_System return QuadDobl_Complex_Poly_Systems.Poly_Sys is
  ...
  function Embed ( p : in QuadDobl_Complex_Poly_Systems.Poly_Sys;
...
-- BUILD EVALUATOR and DIFFERENTIATOR for HOMOTOPY :
  ...
  function Evaluator
             ( p : QuadDobl_Complex_Poly_Systems.Poly_Sys; k : integer32 )
  ...
  --   the rest is copied from the global data qdsys_eval.
    ...
    res : QuadDobl_Complex_Poly_SysFun.Eval_Poly_Sys(p'range);
  ...
  function Jacobi_Evaluator
             ( p : QuadDobl_Complex_Poly_Systems.Poly_Sys; k : integer32 )
             return QuadDobl_Complex_Jaco_Matrices.Eval_Jaco_Mat is
  ...
  --   the rest is copied from the global variable qdjac_eval.
    ...
    res : QuadDobl_Complex_Jaco_Matrices.Eval_Jaco_Mat(p'range,p'range);
    jac : Poly;
...
-- HOMOTOPY CONTINUATION :
  ...
  procedure Silent_Homotopy_Continuation
              ( target_eval : in QuadDobl_Complex_Poly_SysFun.Eval_Poly_Sys;
                target_jaco : in QuadDobl_Complex_Jaco_Matrices.Eval_Jaco_Mat;
                dim : in integer32;
...
-- CORRECTORS :
  ...
  procedure Silent_Correct
              ( p_eval : in QuadDobl_Complex_Poly_SysFun.Eval_Poly_Sys;
                jac_eval : in QuadDobl_Complex_Jaco_Matrices.Eval_Jaco_Mat;
                sols : in out QuadDobl_Complex_Solutions.Solution_List;
...
-- THE TARGET OPERATIONS :
  ...
  procedure Initialize ( ep : in QuadDobl_Complex_Poly_Systems.Poly_Sys ) is
  ...
  function Embedded_System return QuadDobl_Complex_Poly_Systems.Poly_Sys is
  begin
    return qdsys.all;
  end Embedded_System;
  ...
  begin
    epsxa := Quad_Double_Numbers.Create(1.0E-48);
    epsfa := Quad_Double_Numbers.Create(1.0E-48);
    tolsing := Quad_Double_Numbers.Create(1.0E-32);
    maxit := 4;
...
-- SAMPLERS :
  ...
  procedure Sample ( startsol : in QuadDobl_Complex_Solutions.Solution;
    ...
    target : QuadDobl_Complex_Poly_Systems.Poly_Sys(1..Number_of_Equations)
           := Embed(QuadDobl_System,newhyp);
    target_eval : QuadDobl_Complex_Poly_SysFun.Eval_Poly_Sys(target'range)
                := Evaluator(target,dim);
    target_jaco : QuadDobl_Complex_Jaco_Matrices.Eval_Jaco_Mat
  ...
  begin
    ...
    for i in target'last-dim+1..target'last loop       -- mind the sharing !
      QuadDobl_Complex_Polynomials.Clear(target(i));
      QuadDobl_Complex_Poly_Functions.Clear(target_eval(i));
      for j in target'range loop
        QuadDobl_Complex_Poly_Functions.Clear(target_jaco(i,j));
      end loop;
    end loop;
