...
package body Drivers_to_Component_Creators is
  ...
  procedure Multprec_Tuner
               ( file : in file_type; full_output : in out boolean;
                 size : in natural; stoptol,membtol : in out double_float ) is
  ...
  procedure Write_Summary
               ( file : in file_type; k : in natural;
                 deco : in Standard_Irreducible_Component_List ) is

    len : constant natural := Length_Of(deco);
  ...
  begin
    ...
    for i in 1..len loop
      ...
      declare
        ...
        dc : constant natural := Degree(c);
        f : constant Standard_Filter := Filter(c);
        df : constant natural := Degree(f);
        s : constant Standard_Span := Span(c);
      begin
        put(file," has degree "); put(file,dc,1);
        if df > 0 and df < dc
         then put(file,"(="); put(file,df,1);
              put(file,"+"); put(file,dc-df,1);
              put(file,")");
        end if;
        if Empty(s)
         then put_line(file,";");
         else put(file," and spans space of dimension ");
