...
with QuadDobl_Complex_NesVecs;           use QuadDobl_Complex_NesVecs;
...
with QuadDobl_Univariate_Interpolators;  use QuadDobl_Univariate_Interpolators;
with QuadDobl_Power_Traces;              use QuadDobl_Power_Traces;
with QuadDobl_Durand_Kerner;             use QuadDobl_Durand_Kerner;

package body QuadDobl_Trace_Interpolators is
...
-- AUXILIARIES TO EXPAND :
  ...
  function Leading_Coefficient ( p : Poly; tol : quad_double )
                               return Complex_Number is
...
-- AUXILIARIES FOR Create_on_Triangle :
  ...
  --   where d is the degree of the polynomial.
    ...
    z : QuadDobl_Complex_Vectors.Vector(1..d) := Random_Vector(1,d) ;
  ...
  function Roots ( c : QuadDobl_Complex_Vectors.Vector )
                 return QuadDobl_Complex_Vectors.Vector is
...
-- CREATORS :
  ...
  function Create ( file : file_type; grid : Array_of_QuadDobl_Sample_Lists )
                  return Trace_Interpolator1 is
  ...
  function Create ( grid : Array_of_QuadDobl_Sample_Lists; i : integer32 )
                  return Vector is
  ...
  function Create_on_Triangle 
                  ( file : file_type; grid : Array_of_QuadDobl_Sample_Lists )
                  return Trace_Interpolator1 is
...
-- DIAGNOSTICS :
  ...
  function Errors ( t : Trace_Interpolator1;
    ...
    res : Quad_Double_Matrices.Matrix
  ...
  procedure Write_Errors ( file : in file_type; t : in Trace_Interpolator1;
                           grid : in Array_of_QuadDobl_Sample_Lists ) is
  ...
  function Maximal_Error ( residuals : Quad_Double_Matrices.Matrix )
                         return quad_double is

    max : quad_double := residuals(residuals'first(1),residuals'first(2));
  ...
  procedure Write_Errors
              ( file : in file_type; t : in Trace_Interpolator;
                grid : in Stacked_Sample_Grid; maxerr : out quad_double ) is
      ...
      if sg.k = 1 then                     -- this must imply that v.n = 1
        for j in v.v'range loop
          ...
          declare
            ...
            abseva : constant quad_double := AbsVal(val-eva);
          begin
...
-- DESTRUCTORS :
  ...
  begin
    if t /= null
     then QuadDobl_Complex_VecVecs.Clear(t.trc); free(t);
    end if;
  end Clear;
  ...
  begin
    if t /= null then
      if t.k = 1 then
        QuadDobl_Complex_VecVecs.Clear(t.t1.trc);
      else
        Clear(t.rot);
...
end QuadDobl_Trace_Interpolators;
