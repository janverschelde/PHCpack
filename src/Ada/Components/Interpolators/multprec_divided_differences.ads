...
package Multprec_Divided_Differences is
...
--   The precision level of the samples determines the precision level
...
-- CREATORS :
  ...
  --   the Newton_Taylor_Form1.  When the file is supplied, then the
  --   output of additional tests is written to that file.
...
-- DIAGNOSTICS :
  ...
  function Errors ( q : Newton_Interpolator1;
                    grid : Array_of_Multprec_Sample_Lists ) return Matrix;
  ...
  function Maximal_Error ( residuals : Matrix ) return Floating_Number;
...
-- EVALUATORS :
  ...
  --   more efficiently than with the Eval on the Newton_Interpolator1.
