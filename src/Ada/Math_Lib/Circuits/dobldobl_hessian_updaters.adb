...
package body DoblDobl_Hessian_Updaters is

  procedure Speel1 ( H : in out DoblDobl_Complex_Matrices.Matrix;
                     c : in Complex_Number;
    ...
    m1 : integer32;
    powfac : double_double; -- multiplier factor of two powers

  begin
    m1 := xps(fac(1)); -- the monomial is c*x**m1, m1 >= 2.
    powfac := Double_Double_Numbers.Create(m1*(m1-1));
    if m1 = 2 then
  ...
  procedure Speel2 ( H : in out DoblDobl_Complex_Matrices.Matrix;
                     c : in Complex_Number;
                     xps : in Standard_Integer_Vectors.Vector;
                     idx : in Standard_Integer_Vectors.Vector;
                     fac : in Standard_Integer_Vectors.Vector;
  ...
  begin
    ...
    if fac'last = 1 then
      powfac := Double_Double_Numbers.Create(m1);
      if m1 = 2 then
      ...
      powfac := Double_Double_Numbers.Create(m2*(m2-1));
      ...
      powfac := Double_Double_Numbers.Create(m1*m2);
      acc := c*powfac;
      if m1 = 2
  ...
  procedure Speel3 ( H : in out DoblDobl_Complex_Matrices.Matrix;
                     c : in Complex_Number;
                     xps : in Standard_Integer_Vectors.Vector;
                     idx : in Standard_Integer_Vectors.Vector;
                     fac : in Standard_Integer_Vectors.Vector;
  ...
  begin
   ...
   -- compute the off diagonal elements of the Hessian
    powfac := Double_Double_Numbers.Create(xps(idx(1))*xps(idx(2)));
    H(idx(1),idx(2)) := H(idx(1),idx(2)) + offdiagfac*powfac*x(idx(3));
    powfac := Double_Double_Numbers.Create(xps(idx(1))*xps(idx(3)));
    H(idx(1),idx(3)) := H(idx(1),idx(3)) + offdiagfac*powfac*x(idx(2));
    powfac := Double_Double_Numbers.Create(xps(idx(2))*xps(idx(3)));
   ...
   -- ten cases for the on diagonal element of the Hessian
    if fac'last = 3 then -- all variables raised to higher power
      m1 := xps(fac(1)); powfac := Double_Double_Numbers.Create(m1*(m1-1));
      acc := ondiagfac*(pwt(fac(2))(1))*(pwt(fac(3))(1));
      H(fac(1),fac(1)) := H(fac(1),fac(1)) + acc*powfac;
      m1 := xps(fac(2)); powfac := Double_Double_Numbers.Create(m1*(m1-1));
      acc := ondiagfac*(pwt(fac(1))(1))*(pwt(fac(3))(1));
      H(fac(2),fac(2)) := H(fac(2),fac(2)) + acc*powfac;
      m1 := xps(fac(3)); powfac := Double_Double_Numbers.Create(m1*(m1-1));
  ...
  procedure Speel4 ( H : in out DoblDobl_Complex_Matrices.Matrix;
                     c : in Complex_Number;
                     xps : in Standard_Integer_Vectors.Vector;
                     idx : in Standard_Integer_Vectors.Vector;
                     fac : in Standard_Integer_Vectors.Vector;
  ...
  begin
   ...
   -- the last element is a copy of fwd1, with a multiplier factor
    powfac := Double_Double_Numbers.Create(xps(idx(3))*xps(idx(4)));
  ...
  procedure SpeelN ( H : in out DoblDobl_Complex_Matrices.Matrix;
                     c : in Complex_Number;
                     xps : in Standard_Integer_Vectors.Vector;
                     idx : in Standard_Integer_Vectors.Vector;
                     fac : in Standard_Integer_Vectors.Vector;
  ...
  begin
   ...
   -- last element is copy of fwd(sz-3), multiplied with c
    powfac := Double_Double_Numbers.Create(xps(idx(sz-1))*xps(idx(sz)));
   ...
   -- first row is special, starts with x(idx(2)) after diagonal
    ...
    for k in 4..sz-2 loop
      acc := acc*x(idx(k-1));
      powfac := Double_Double_Numbers.Create(xps(idx(1))*xps(idx(k)));
      H(idx(1),idx(k)) := H(idx(1),idx(k)) + acc*powfac*bck(sz-k-1);
    end loop;
    acc := acc*x(idx(sz-2));
    powfac := Double_Double_Numbers.Create(xps(idx(1))*xps(idx(sz-1)));
    H(idx(1),idx(sz-1)) := H(idx(1),idx(sz-1)) + acc*powfac*x(idx(sz));
    powfac := Double_Double_Numbers.Create(xps(idx(1))*xps(idx(sz)));
   ...
   -- second row is special, starts with x(idx(1)) after diagonal
    ...
    for k in 4..sz-2 loop
      acc := acc*x(idx(k-1));
      powfac := Double_Double_Numbers.Create(xps(idx(2))*xps(idx(k)));
      H(idx(2),idx(k)) := H(idx(2),idx(k)) + acc*powfac*bck(sz-k-1);
    end loop;
    acc := acc*x(idx(sz-2));
    powfac := Double_Double_Numbers.Create(xps(idx(2))*xps(idx(sz-1)));
    H(idx(2),idx(sz-1)) := H(idx(2),idx(sz-1)) + acc*powfac*x(idx(sz));
    powfac := Double_Double_Numbers.Create(xps(idx(2))*xps(idx(sz)));
    H(idx(2),idx(sz)) := H(idx(2),idx(sz)) + acc*powfac*x(idx(sz-1));
   -- the row with index sz-2 has a general formula
    acc := offdiagfac*fwd(sz-4);
    powfac := Double_Double_Numbers.Create(xps(idx(sz-2))*xps(idx(sz-1)));
    H(idx(sz-2),idx(sz-1)) := H(idx(sz-2),idx(sz-1)) + acc*powfac*x(idx(sz));
    powfac := Double_Double_Numbers.Create(xps(idx(sz-2))*xps(idx(sz)));
    ...
    for rw in 3..sz-3 loop  -- row rw starts with fwd(rw-2)
      ...
      powfac := Double_Double_Numbers.Create(xps(idx(rw))*xps(idx(rw+1)));
      H(idx(rw),idx(rw+1)) := H(idx(rw),idx(rw+1)) + acc*powfac*bck(sz-rw-2);
      for k in rw+2..sz-2 loop
        acc := acc*x(idx(k-1));
        powfac := Double_Double_Numbers.Create(xps(idx(rw))*xps(idx(k)));
        H(idx(rw),idx(k)) := H(idx(rw),idx(k)) + acc*powfac*bck(sz-k-1);
      end loop;
      acc := acc*x(idx(sz-2));
      powfac := Double_Double_Numbers.Create(xps(idx(rw))*xps(idx(sz-1)));
      H(idx(rw),idx(sz-1)) := H(idx(rw),idx(sz-1)) + acc*powfac*x(idx(sz));
      powfac := Double_Double_Numbers.Create(xps(idx(rw))*xps(idx(sz)));
