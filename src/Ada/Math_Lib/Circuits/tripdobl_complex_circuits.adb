...
with TripDobl_Hessian_Updaters;

package body TripDobl_Complex_Circuits is
...
-- CONSTRUCTORS :
  ...
  function Allocate ( nbr,dim : integer32 ) return Circuit is
    ...
    forward : constant TripDobl_Complex_Vectors.Vector(1..dim-1)
            := (1..dim-1 => zero);
    backward : constant TripDobl_Complex_Vectors.Vector(1..dim-2)
             := (1..dim-2 => zero);
    cross : constant TripDobl_Complex_Vectors.Vector(1..dim-2)
          := (1..dim-2 => zero);

  begin
    res.dim := dim;
    res.pdg := -1;
    res.fwd := new TripDobl_Complex_Vectors.Vector'(forward);
    res.bck := new TripDobl_Complex_Vectors.Vector'(backward);
    res.crs := new TripDobl_Complex_Vectors.Vector'(cross);
    return res;
  end Allocate;
  ...
  begin
    ...
    res.yd := new TripDobl_Complex_Vectors.Vector'(0..dim => zero);
    return res;
  end Create;
  ...
  function Allocate ( neq,dim : integer32 )
    ...
    res : TripDobl_Complex_VecMats.VecMat(1..neq);
  ...
  begin
    for k in 1..neq loop
      ...
      begin
        for i in 1..dim loop
          for j in 1..dim loop
            mat(i,j) := TripDobl_Complex_Numbers.Create(integer(0));
          end loop;
        end loop;
        res(k) := new TripDobl_Complex_Matrices.Matrix'(mat);
      end;
    end loop;
    return res;
  end Allocate;
...
-- SINGULAR VALUE DECOMPOSITIONS :
  ...
  procedure Singular_Values
              ( s : in out System;
                ...
                vh : in TripDobl_Complex_VecMats.VecMat;
                U : out TripDobl_Complex_Matrices.Matrix;
                V : out TripDobl_Complex_Matrices.Matrix;
                e : out TripDobl_Complex_Vectors.Vector;
                svls : in TripDobl_Complex_VecVecs.VecVec ) is
  ...
  procedure Singular_Values
              ( c : in Circuit;
                x,yd : in TripDobl_Complex_Vectors.Link_to_Vector;
                pwt : in TripDobl_Complex_VecVecs.VecVec;
                A : out TripDobl_Complex_Matrices.Matrix;
                U : out TripDobl_Complex_Matrices.Matrix;
                V : out TripDobl_Complex_Matrices.Matrix;
                e : out TripDobl_Complex_Vectors.Vector;
                s : out TripDobl_Complex_Vectors.Vector ) is

    info : integer32;

  begin
    if c.pdg <= 1 then
      s := (s'range => TripDobl_Complex_Numbers.Create(integer32(0)));
    else
      Speel(c,x,yd,pwt,A);
      TripDobl_Complex_Singular_Values.SVD(A,c.dim,c.dim,s,e,U,V,0,info);
    end if;
  end Singular_Values;
...
-- ALGORITHMIC DIFFERENTIATION AND EVALUATION OF CIRCUITS :
  ...
  procedure EvalDiff
              ( s : in out System;
                x : in TripDobl_Complex_Vectors.Link_to_Vector ) is
  begin
    Power_Table(s.mxe,x,s.pwt);
  ...
  procedure EvalDiff2
              ( s : in out System;
                x : in TripDobl_Complex_Vectors.Link_to_Vector;
                vh : in TripDobl_Complex_VecMats.VecMat ) is
  begin
    Power_Table(s.mxe,x,s.pwt);
    EvalDiff2(s.crc,x,s.yd,s.pwt,s.fx,s.jm,vh);
  end EvalDiff2;
  ...
  procedure EvalDiff
              ( c : in Circuits;
                x,yd : in TripDobl_Complex_Vectors.Link_to_Vector;
                pwt : in TripDobl_Complex_VecVecs.VecVec;
                fx : out TripDobl_Complex_Vectors.Vector;
                jm : out TripDobl_Complex_Matrices.Matrix ) is
  begin
    for i in c'range loop
      ...
      for j in jm'range(2) loop
        jm(i,j) := yd(j);
        yd(j) := TripDobl_Complex_Numbers.Create(integer(0));
      end loop;
    end loop;
  end EvalDiff;
  ...
  procedure EvalDiff2
              ( c : in Circuits;
                ...
                pwt : in TripDobl_Complex_VecVecs.VecVec;
                fx : out TripDobl_Complex_Vectors.Vector;
                jm : out TripDobl_Complex_Matrices.Matrix;
                vh : in TripDobl_Complex_VecMats.VecMat ) is

    mat : TripDobl_Complex_Matrices.Link_to_Matrix;
...
-- ALGORITMIC DIFFERENTIATION AND EVALUATION OF CIRCUIT :
  ...
  procedure Indexed_Speel
              ( c : in Circuit;
                x,yd : in TripDobl_Complex_Vectors.Link_to_Vector;
                h : out TripDobl_Complex_Matrices.Matrix ) is
  begin
  ...
  procedure Indexed_Speel
              ( c : in Circuit;
                x,yd : in TripDobl_Complex_Vectors.Link_to_Vector ) is
  begin
  ...
  procedure Speel ( c : in Circuit;
                    x,yd : in TripDobl_Complex_Vectors.Link_to_Vector;
                    pwt : in TripDobl_Complex_VecVecs.VecVec ) is
  begin
  ...
  procedure Indexed_Speel
              ( idx : in Standard_Integer_VecVecs.VecVec;
                cff : in TripDobl_Complex_Vectors.Vector;
                cst : in TripDobl_Complex_Numbers.Complex_Number;
                x,yd : in TripDobl_Complex_Vectors.Link_to_Vector;
                fwd : in TripDobl_Complex_Vectors.Link_to_Vector;
                bck : in TripDobl_Complex_Vectors.Link_to_Vector;
                crs : in TripDobl_Complex_Vectors.Link_to_Vector ) is

    idk : Standard_Integer_Vectors.Link_to_Vector;
    idx1,idx2 : integer32;
    kcff : TripDobl_Complex_Numbers.Complex_Number;

    use TripDobl_Complex_Numbers,Standard_Integer_Vectors;
  ...
  procedure Indexed_Speel
              ( idx : in Standard_Integer_Vectors.Vector;
                cff : in TripDobl_Complex_Numbers.Complex_Number;
                x,yd : in TripDobl_Complex_Vectors.Link_to_Vector;
                fwd : in TripDobl_Complex_Vectors.Link_to_Vector;
                bck : in TripDobl_Complex_Vectors.Link_to_Vector;
                crs : in TripDobl_Complex_Vectors.Link_to_Vector;
                h : in out TripDobl_Complex_Matrices.Matrix ) is
  ...
  procedure Speel ( xps,idx,fac : in Standard_Integer_VecVecs.VecVec;
    ...
    idx1,idx2 : integer32;
    kcff,acc,wrk : TripDobl_Complex_Numbers.Complex_Number;
    factor : triple_double;
        ...
        if idk'last < idk'first then -- deal with stray 0 exponent
          ...
          if fck = null then
            ...
            if idk'last = 1 then
              ...
              factor := Triple_Double_Numbers.Create(xpk(idx1));
              ...
              if idk'last = 2 then
                idx2 := idk(2);
                wrk := acc*x(idx1);
                factor := Triple_Double_Numbers.Create(xpk(idx2));
  ...
  begin
    ...
    if idx'last = 1 then
      ...
      yd(0) := yd(0) + acc*x(idx1);
      factor := Triple_Double_Numbers.Create(xps(idx1));
      yd(idx1) := yd(idx1) + factor*acc;
    else
      Forward_Backward_Cross(idx,x,fwd,bck,crs);
      Multiply_Factor(xps,fac,x,cff,pwt,acc);
      ...
      if idx'last = 2 then
        idx2 := idx(2);
        wrk := acc*x(idx1);
        factor := Triple_Double_Numbers.Create(xps(idx2));
        wrk := factor*wrk; yd(idx2) := yd(idx2) + wrk;
        wrk := acc*x(idx2);
        factor := Triple_Double_Numbers.Create(xps(idx1));
        wrk := factor*wrk; yd(idx1) := yd(idx1) + wrk;
      ...
      begin
        ...
        if fck = null then
          ...
          if idk'last = 1 then -- one variable special case
            TripDobl_Hessian_Updaters.Speel1(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 2 then -- two variable special case
            TripDobl_Hessian_Updaters.Speel2(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 3 then -- three variable special case
            TripDobl_Hessian_Updaters.Speel3(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 4 then -- four variable special case
            TripDobl_Hessian_Updaters.Speel4(h,kcff,xpk,idk,fck.all,x.all,pwt);
          else
            TripDobl_Hessian_Updaters.SpeelN
...
-- AUXILIARY PROCEDURES :
  ...
  procedure Forward ( x : in TripDobl_Complex_Vectors.Link_to_Vector;
                      f : in TripDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  procedure Forward_Backward
              ( x,f,b : in TripDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  procedure Forward_Backward_Cross
              ( x,f,b,c : in TripDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  function Allocate
    ...
    res : TripDobl_Complex_VecVecs.VecVec(mxe'range);
    zero : constant TripDobl_Complex_Numbers.Complex_Number
         := TripDobl_Complex_Numbers.Create(integer(0));
  ...
  begin
    for k in mxe'range loop
      if mxe(k) > 1 then
        res(k) := new TripDobl_Complex_Vectors.Vector'(1..mxe(k)-1 => zero);
      end if;
    end loop;
    return res;
  end Allocate;
  ...
  procedure Multiply_Factor
              ( xps,fac : in Standard_Integer_Vectors.Link_to_Vector;
                ...
                cff : in TripDobl_Complex_Numbers.Complex_Number;
                pwt : in TripDobl_Complex_VecVecs.VecVec;
                res : out TripDobl_Complex_Numbers.Complex_Number ) is

    pwx : TripDobl_Complex_Vectors.Link_to_Vector;
...
-- DESTRUCTORS :
  ...
  begin
    c.dim := -1;
    c.pdg := -1;
    TripDobl_Complex_Vectors.Clear(c.fwd);
    TripDobl_Complex_Vectors.Clear(c.bck);
    TripDobl_Complex_Vectors.Clear(c.crs);
  end Clear;
  ...
  begin
    Clear(s.crc);
    TripDobl_Complex_Vectors.Clear(s.yd);
    TripDobl_Complex_VecVecs.Clear(s.pwt);
  end Clear;
