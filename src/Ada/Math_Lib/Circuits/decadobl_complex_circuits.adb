...
with DecaDobl_Hessian_Updaters;

package body DecaDobl_Complex_Circuits is
...
-- CONSTRUCTORS :
  ...
  function Allocate ( nbr,dim : integer32 ) return Circuit is
    ...
    forward : constant DecaDobl_Complex_Vectors.Vector(1..dim-1)
            := (1..dim-1 => zero);
    backward : constant DecaDobl_Complex_Vectors.Vector(1..dim-2)
             := (1..dim-2 => zero);
    cross : constant DecaDobl_Complex_Vectors.Vector(1..dim-2)
          := (1..dim-2 => zero);

  begin
    res.dim := dim;
    res.pdg := -1;
    res.fwd := new DecaDobl_Complex_Vectors.Vector'(forward);
    res.bck := new DecaDobl_Complex_Vectors.Vector'(backward);
    res.crs := new DecaDobl_Complex_Vectors.Vector'(cross);
    return res;
  end Allocate;
  ...
  begin
    ...
    res.yd := new DecaDobl_Complex_Vectors.Vector'(0..dim => zero);
    return res;
  end Create;
  ...
  function Allocate ( neq,dim : integer32 )
    ...
    res : DecaDobl_Complex_VecMats.VecMat(1..neq);
  ...
  begin
    for k in 1..neq loop
      ...
      begin
        for i in 1..dim loop
          for j in 1..dim loop
            mat(i,j) := DecaDobl_Complex_Numbers.Create(integer(0));
          end loop;
        end loop;
        res(k) := new DecaDobl_Complex_Matrices.Matrix'(mat);
      end;
    end loop;
    return res;
  end Allocate;
...
-- SINGULAR VALUE DECOMPOSITIONS :
  ...
  procedure Singular_Values
              ( s : in out System;
                ...
                vh : in DecaDobl_Complex_VecMats.VecMat;
                U : out DecaDobl_Complex_Matrices.Matrix;
                V : out DecaDobl_Complex_Matrices.Matrix;
                e : out DecaDobl_Complex_Vectors.Vector;
                svls : in DecaDobl_Complex_VecVecs.VecVec ) is
  ...
  procedure Singular_Values
              ( c : in Circuit;
                x,yd : in DecaDobl_Complex_Vectors.Link_to_Vector;
                pwt : in DecaDobl_Complex_VecVecs.VecVec;
                A : out DecaDobl_Complex_Matrices.Matrix;
                U : out DecaDobl_Complex_Matrices.Matrix;
                V : out DecaDobl_Complex_Matrices.Matrix;
                e : out DecaDobl_Complex_Vectors.Vector;
                s : out DecaDobl_Complex_Vectors.Vector ) is

    info : integer32;

  begin
    if c.pdg <= 1 then
      s := (s'range => DecaDobl_Complex_Numbers.Create(integer32(0)));
    else
      Speel(c,x,yd,pwt,A);
      DecaDobl_Complex_Singular_Values.SVD(A,c.dim,c.dim,s,e,U,V,0,info);
    end if;
  end Singular_Values;
...
-- ALGORITHMIC DIFFERENTIATION AND EVALUATION OF CIRCUITS :
  ...
  procedure EvalDiff
              ( s : in out System;
                x : in DecaDobl_Complex_Vectors.Link_to_Vector ) is
  begin
    Power_Table(s.mxe,x,s.pwt);
  ...
  procedure EvalDiff2
              ( s : in out System;
                x : in DecaDobl_Complex_Vectors.Link_to_Vector;
                vh : in DecaDobl_Complex_VecMats.VecMat ) is
  begin
    Power_Table(s.mxe,x,s.pwt);
    EvalDiff2(s.crc,x,s.yd,s.pwt,s.fx,s.jm,vh);
  end EvalDiff2;
  ...
  procedure EvalDiff
              ( c : in Circuits;
                x,yd : in DecaDobl_Complex_Vectors.Link_to_Vector;
                pwt : in DecaDobl_Complex_VecVecs.VecVec;
                fx : out DecaDobl_Complex_Vectors.Vector;
                jm : out DecaDobl_Complex_Matrices.Matrix ) is
  begin
    for i in c'range loop
      ...
      for j in jm'range(2) loop
        jm(i,j) := yd(j);
        yd(j) := DecaDobl_Complex_Numbers.Create(integer(0));
      end loop;
    end loop;
  end EvalDiff;
  ...
  procedure EvalDiff2
              ( c : in Circuits;
                ...
                pwt : in DecaDobl_Complex_VecVecs.VecVec;
                fx : out DecaDobl_Complex_Vectors.Vector;
                jm : out DecaDobl_Complex_Matrices.Matrix;
                vh : in DecaDobl_Complex_VecMats.VecMat ) is

    mat : DecaDobl_Complex_Matrices.Link_to_Matrix;
...
-- ALGORITMIC DIFFERENTIATION AND EVALUATION OF CIRCUIT :
  ...
  procedure Indexed_Speel
              ( c : in Circuit;
                x,yd : in DecaDobl_Complex_Vectors.Link_to_Vector;
                h : out DecaDobl_Complex_Matrices.Matrix ) is
  begin
  ...
  procedure Indexed_Speel
              ( c : in Circuit;
                x,yd : in DecaDobl_Complex_Vectors.Link_to_Vector ) is
  begin
  ...
  procedure Indexed_Speel
              ( idx : in Standard_Integer_VecVecs.VecVec;
                cff : in DecaDobl_Complex_Vectors.Vector;
                cst : in DecaDobl_Complex_Numbers.Complex_Number;
                x,yd : in DecaDobl_Complex_Vectors.Link_to_Vector;
                fwd : in DecaDobl_Complex_Vectors.Link_to_Vector;
                bck : in DecaDobl_Complex_Vectors.Link_to_Vector;
                crs : in DecaDobl_Complex_Vectors.Link_to_Vector ) is

    idk : Standard_Integer_Vectors.Link_to_Vector;
    idx1,idx2 : integer32;
    kcff : DecaDobl_Complex_Numbers.Complex_Number;

    use DecaDobl_Complex_Numbers,Standard_Integer_Vectors;
  ...
  procedure Indexed_Speel
              ( idx : in Standard_Integer_Vectors.Vector;
                cff : in DecaDobl_Complex_Numbers.Complex_Number;
                x,yd : in DecaDobl_Complex_Vectors.Link_to_Vector;
                fwd : in DecaDobl_Complex_Vectors.Link_to_Vector;
                bck : in DecaDobl_Complex_Vectors.Link_to_Vector;
                crs : in DecaDobl_Complex_Vectors.Link_to_Vector;
                h : in out DecaDobl_Complex_Matrices.Matrix ) is
  ...
  procedure Speel ( xps,idx,fac : in Standard_Integer_VecVecs.VecVec;
    ...
    idx1,idx2 : integer32;
    kcff,acc,wrk : DecaDobl_Complex_Numbers.Complex_Number;
    factor : deca_double;
        ...
        if idk'last < idk'first then -- deal with stray 0 exponent
          ...
          if fck = null then
            ...
            if idk'last = 1 then
              ...
              factor := Deca_Double_Numbers.Create(xpk(idx1));
              ...
              if idk'last = 2 then
                idx2 := idk(2);
                wrk := acc*x(idx1);
                factor := Deca_Double_Numbers.Create(xpk(idx2));
  ...
  begin
    ...
    if idx'last = 1 then
      ...
      yd(0) := yd(0) + acc*x(idx1);
      factor := Deca_Double_Numbers.Create(xps(idx1));
      yd(idx1) := yd(idx1) + factor*acc;
    else
      Forward_Backward_Cross(idx,x,fwd,bck,crs);
      Multiply_Factor(xps,fac,x,cff,pwt,acc);
      ...
      if idx'last = 2 then
        idx2 := idx(2);
        wrk := acc*x(idx1);
        factor := Deca_Double_Numbers.Create(xps(idx2));
        wrk := factor*wrk; yd(idx2) := yd(idx2) + wrk;
        wrk := acc*x(idx2);
        factor := Deca_Double_Numbers.Create(xps(idx1));
        wrk := factor*wrk; yd(idx1) := yd(idx1) + wrk;
      ...
      begin
        ...
        if fck = null then
          ...
          if idk'last = 1 then -- one variable special case
            DecaDobl_Hessian_Updaters.Speel1(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 2 then -- two variable special case
            DecaDobl_Hessian_Updaters.Speel2(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 3 then -- three variable special case
            DecaDobl_Hessian_Updaters.Speel3(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 4 then -- four variable special case
            DecaDobl_Hessian_Updaters.Speel4(h,kcff,xpk,idk,fck.all,x.all,pwt);
          else
            DecaDobl_Hessian_Updaters.SpeelN
...
-- AUXILIARY PROCEDURES :
  ...
  procedure Forward ( x : in DecaDobl_Complex_Vectors.Link_to_Vector;
                      f : in DecaDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  procedure Forward_Backward
              ( x,f,b : in DecaDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  procedure Forward_Backward_Cross
              ( x,f,b,c : in DecaDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  function Allocate
    ...
    res : DecaDobl_Complex_VecVecs.VecVec(mxe'range);
    zero : constant DecaDobl_Complex_Numbers.Complex_Number
         := DecaDobl_Complex_Numbers.Create(integer(0));
  ...
  begin
    for k in mxe'range loop
      if mxe(k) > 1 then
        res(k) := new DecaDobl_Complex_Vectors.Vector'(1..mxe(k)-1 => zero);
      end if;
    end loop;
    return res;
  end Allocate;
  ...
  procedure Multiply_Factor
              ( xps,fac : in Standard_Integer_Vectors.Link_to_Vector;
                ...
                cff : in DecaDobl_Complex_Numbers.Complex_Number;
                pwt : in DecaDobl_Complex_VecVecs.VecVec;
                res : out DecaDobl_Complex_Numbers.Complex_Number ) is

    pwx : DecaDobl_Complex_Vectors.Link_to_Vector;
...
-- DESTRUCTORS :
  ...
  begin
    c.dim := -1;
    c.pdg := -1;
    DecaDobl_Complex_Vectors.Clear(c.fwd);
    DecaDobl_Complex_Vectors.Clear(c.bck);
    DecaDobl_Complex_Vectors.Clear(c.crs);
  end Clear;
  ...
  begin
    Clear(s.crc);
    DecaDobl_Complex_Vectors.Clear(s.yd);
    DecaDobl_Complex_VecVecs.Clear(s.pwt);
  end Clear;
