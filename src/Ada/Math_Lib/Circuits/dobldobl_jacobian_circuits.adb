...
with DoblDobl_Jacobian_Evaluations;      use DoblDobl_Jacobian_Evaluations;

package body DoblDobl_Jacobian_Circuits is
...
-- DATA STRUCTURE :
  ...
  type Circuit_Rep ( m,nq,nv : integer32 ) is record
    ...
    c : DoblDobl_Complex_VecVecs.VecVec(1..nq);  -- coefficients
...
-- SELECTORS :
  ...
  function Coefficient ( c : Circuit; i,j : integer32 )
    ...
    res : Complex_Number;
    cff : constant DoblDobl_Complex_Vectors.Link_to_Vector
        := Coefficients(c,i);
...
end DoblDobl_Jacobian_Circuits;
