...
package body PentDobl_Circuit_Makers is
  ...
  function Random_Complex_Circuit
    ...
    res : PentDobl_Complex_Circuits.Circuit(nbr)
        := PentDobl_Complex_Circuits.Allocate(nbr,dim);

  begin
    for k in 1..nbr loop
      res.xps(k) := new Standard_Integer_Vectors.Vector'(Random_Indices(dim));
    end loop;
    res.cff := PentDobl_Random_Vectors.Random_Vector(1,nbr);
    res.cst := PentDobl_Random_Numbers.Random1;
  ...
  function Random_Complex_Circuit
    ...
    crc : constant PentDobl_Complex_Circuits.Circuit(nbr)
        := Random_Complex_Circuit(nbr,dim,pwr);
    res : constant PentDobl_Complex_Circuits.Link_to_Circuit
        := new PentDobl_Complex_Circuits.Circuit'(crc);
  ...
  function Random_Complex_Circuits
    ...
    res : PentDobl_Complex_Circuits.Circuits(1..neq);
  ...
  function Random_Complex_System
    ...
    crc : constant PentDobl_Complex_Circuits.Circuits(1..neq)
        := Random_Complex_Circuits(neq,nbr,dim,pwr);
    res : constant PentDobl_Complex_Circuits.System(neq,dim)
        := PentDobl_Complex_Circuits.Create(crc,dim);
  ...
  function Random_Complex_System
    ...
    sys : constant PentDobl_Complex_Circuits.System(neq,dim)
        := Random_Complex_System(neq,nbr,dim,pwr);
    res : constant PentDobl_Complex_Circuits.Link_to_System
        := new PentDobl_Complex_Circuits.System'(sys);
  ...
  function Gradient ( p : PentDobl_Complex_Polynomials.Poly;
    ...
    dp : PentDobl_Complex_Polynomials.Poly;

  begin
    for k in x'range loop
      dp := PentDobl_Complex_Polynomials.Diff(p,k);
      res(k) := PentDobl_Complex_Poly_Functions.Eval(dp,x);
      PentDobl_Complex_Polynomials.Clear(dp);
    end loop;
    return res;
  end Gradient;
  ...
  function Hessian ( p : PentDobl_Complex_Polynomials.Poly;
    ...
    res : PentDobl_Complex_Matrices.Matrix(x'range,x'range);

    use PentDobl_Complex_Polynomials;
    use PentDobl_Complex_Poly_Functions;
  ...
  function Constant_Coefficient
    ...
    res : constant PentDobl_Complex_Numbers.Complex_Number := Coeff(p,deg);
  ...
  begin
    if PentDobl_Complex_Numbers.REAL_PART(c) /= zero then
      return 1;
    elsif PentDobl_Complex_Numbers.IMAG_PART(c) /= zero then
      return 1;
    else
      return 0;
    end if;
  ...
  function Make_Complex_Circuit
    ...
    res : PentDobl_Complex_Circuits.Circuit(nbr-isz)
        := PentDobl_Complex_Circuits.Allocate(nbr-isz,dim);
    cnt : integer32 := 0;
  ...
  procedure Write_Matrix ( A : in PentDobl_Complex_Matrices.Matrix ) is
  begin
...
-- FROM CONVOLUTION CIRCUITS TO COMPLEX CIRCUITS :
  ...
  begin
    ...
    if c.cst = null
     then res.cst := PentDobl_Complex_Numbers.Create(integer(0));
  ...
  begin
    if c /= null then
      declare
        crc : constant PentDobl_Complex_Circuits.Circuit(c.nbr)
  ...
  function Make_Complex_System
    ...
    res : PentDobl_Complex_Circuits.System(s.neq,s.dim);
    crc : PentDobl_Complex_Circuits.Circuits(s.crc'range);

  begin
    for k in crc'range loop
      crc(k) := Make_Complex_Circuit(s.crc(k));
    end loop;
    res := PentDobl_Complex_Circuits.Create(crc,s.dim);
    return res;
  end Make_Complex_System;
  ...
  begin
    if s /= null then
      declare
        cfs : constant PentDobl_Complex_Circuits.System(s.neq,s.dim)
            := Make_Complex_System(s.all);
      begin
        res := new PentDobl_Complex_Circuits.System'(cfs);
      end;
    end if;
    return res;
  end Make_Complex_System;
