...
package QuadDobl_Complex_Circuits is
...
--   apply the reverse mode of algorithmic differentiation and evaluation.
  ...
  type Circuit ( nbr : integer32 ) is record
    ...
    cff : QuadDobl_Complex_Vectors.Vector(1..nbr); -- coefficients
    cst : QuadDobl_Complex_Numbers.Complex_Number; -- constant
    fwd : QuadDobl_Complex_Vectors.Link_to_Vector; -- forward products
    bck : QuadDobl_Complex_Vectors.Link_to_Vector; -- backward products
    crs : QuadDobl_Complex_Vectors.Link_to_Vector; -- cross products
  end record;
...
-- along with work space and the final outcomes.
  ...
  type System ( neq,dim : integer32 ) is record
    ...
    pwt : QuadDobl_Complex_VecVecs.VecVec(1..dim); -- power table
    yd : QuadDobl_Complex_Vectors.Link_to_Vector;  -- work space for a gradient
    fx : QuadDobl_Complex_Vectors.Vector(1..neq);  -- function value
    jm : QuadDobl_Complex_Matrices.Matrix(1..neq,1..dim); -- Jacobian matrix
  end record;
...
-- SINGULAR VALUE DECOMPOSITIONS :
  ...
  procedure Singular_Values
              ( s : in out System;
                ...
                svls : in QuadDobl_Complex_VecVecs.VecVec );
  procedure Singular_Values
              ( s : in Link_to_System;
                x : in QuadDobl_Complex_Vectors.Link_to_Vector;
                vh : in QuadDobl_Complex_VecMats.VecMat;
...
--   and optionally, the evaluated Hessian matrix.
  ...
  procedure Indexed_Speel
               ( c : in Circuit;
                 x,yd : in QuadDobl_Complex_Vectors.Link_to_Vector;
                 h : out QuadDobl_Complex_Matrices.Matrix );
  ...
  procedure Indexed_Speel
              ( idx : in Standard_Integer_Vectors.Vector;
                ...
                h : in out QuadDobl_Complex_Matrices.Matrix );
...
-- AUXILIARY PROCEDURES :
  ...
  procedure Forward ( x : in QuadDobl_Complex_Vectors.Link_to_Vector;
                      f : in QuadDobl_Complex_Vectors.Link_to_Vector );
  ...
  procedure Forward_Backward
              ( x,f,b : in QuadDobl_Complex_Vectors.Link_to_Vector );
  ...
  procedure Forward_Backward_Cross
              ( x,f,b,c : in QuadDobl_Complex_Vectors.Link_to_Vector );
  procedure Forward_Backward_Cross
              ( idx : in Standard_Integer_Vectors.Vector;
                x,f,b,c : in QuadDobl_Complex_Vectors.Link_to_Vector );
  ...
  procedure Multiply_Factor
              ( xps,fac : in Standard_Integer_Vectors.Link_to_Vector;
                ...
                cff : in QuadDobl_Complex_Numbers.Complex_Number;
                pwt : in QuadDobl_Complex_VecVecs.VecVec;
                res : out QuadDobl_Complex_Numbers.Complex_Number );
