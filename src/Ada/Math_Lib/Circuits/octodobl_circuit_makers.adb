...
package body OctoDobl_Circuit_Makers is
  ...
  function Random_Complex_Circuit
    ...
    res : OctoDobl_Complex_Circuits.Circuit(nbr)
        := OctoDobl_Complex_Circuits.Allocate(nbr,dim);

  begin
    for k in 1..nbr loop
      res.xps(k) := new Standard_Integer_Vectors.Vector'(Random_Indices(dim));
    end loop;
    res.cff := OctoDobl_Random_Vectors.Random_Vector(1,nbr);
    res.cst := OctoDobl_Random_Numbers.Random1;
  ...
  function Random_Complex_Circuit
    ...
    crc : constant OctoDobl_Complex_Circuits.Circuit(nbr)
        := Random_Complex_Circuit(nbr,dim,pwr);
    res : constant OctoDobl_Complex_Circuits.Link_to_Circuit
        := new OctoDobl_Complex_Circuits.Circuit'(crc);
  ...
  function Random_Complex_Circuits
    ...
    res : OctoDobl_Complex_Circuits.Circuits(1..neq);
  ...
  function Random_Complex_System
    ...
    crc : constant OctoDobl_Complex_Circuits.Circuits(1..neq)
        := Random_Complex_Circuits(neq,nbr,dim,pwr);
    res : constant OctoDobl_Complex_Circuits.System(neq,dim)
        := OctoDobl_Complex_Circuits.Create(crc,dim);
  ...
  function Random_Complex_System
    ...
    sys : constant OctoDobl_Complex_Circuits.System(neq,dim)
        := Random_Complex_System(neq,nbr,dim,pwr);
    res : constant OctoDobl_Complex_Circuits.Link_to_System
        := new OctoDobl_Complex_Circuits.System'(sys);
  ...
  function Make_Polynomial
             ( c : OctoDobl_Complex_Circuits.Circuit;
               index : boolean := false )
  ...
  function Gradient ( p : OctoDobl_Complex_Polynomials.Poly;
                      x : OctoDobl_Complex_Vectors.Vector )
                    return OctoDobl_Complex_Vectors.Vector is

    res : OctoDobl_Complex_Vectors.Vector(x'range);
    dp : OctoDobl_Complex_Polynomials.Poly;

  begin
    for k in x'range loop
      dp := OctoDobl_Complex_Polynomials.Diff(p,k);
      res(k) := OctoDobl_Complex_Poly_Functions.Eval(dp,x);
      OctoDobl_Complex_Polynomials.Clear(dp);
    end loop;
    return res;
  end Gradient;

  function Hessian ( p : OctoDobl_Complex_Polynomials.Poly;
                     x : OctoDobl_Complex_Vectors.Vector )
                   return OctoDobl_Complex_Matrices.Matrix is

    res : OctoDobl_Complex_Matrices.Matrix(x'range,x'range);

    use OctoDobl_Complex_Polynomials;
    use OctoDobl_Complex_Poly_Functions;
  ...
  function Constant_Coefficient
    ...
    res : constant OctoDobl_Complex_Numbers.Complex_Number := Coeff(p,deg);
  ...
  function Is_NonZero ( c : OctoDobl_Complex_Numbers.Complex_Number )
                   return integer32 is

    zero : constant octo_double := create(0.0);

  begin
    if OctoDobl_Complex_Numbers.REAL_PART(c) /= zero then
      return 1;
    elsif OctoDobl_Complex_Numbers.IMAG_PART(c) /= zero then
      return 1;
    else
      return 0;
    end if;
  ...
  function Make_Complex_Circuit
    ...
    res : OctoDobl_Complex_Circuits.Circuit(nbr-isz)
        := OctoDobl_Complex_Circuits.Allocate(nbr-isz,dim);
    cnt : integer32 := 0;
  ...
  procedure Write_Matrix ( A : in OctoDobl_Complex_Matrices.Matrix ) is
  begin
...
-- FROM CONVOLUTION CIRCUITS TO COMPLEX CIRCUITS :
  ...
  begin
    ...
    if c.cst = null
     then res.cst := OctoDobl_Complex_Numbers.Create(integer(0));
  ...
  function Make_Complex_Circuit
             ( c : OctoDobl_Speelpenning_Convolutions.Link_to_Circuit )
             return OctoDobl_Complex_Circuits.Link_to_Circuit is
  ...
  begin
    if c /= null then
      declare
        crc : constant OctoDobl_Complex_Circuits.Circuit(c.nbr)
  ...
  function Make_Complex_System
             ( s : OctoDobl_Speelpenning_Convolutions.System )
             return OctoDobl_Complex_Circuits.System is

    res : OctoDobl_Complex_Circuits.System(s.neq,s.dim);
    crc : OctoDobl_Complex_Circuits.Circuits(s.crc'range);

  begin
    for k in crc'range loop
      crc(k) := Make_Complex_Circuit(s.crc(k));
    end loop;
    res := OctoDobl_Complex_Circuits.Create(crc,s.dim);
    return res;
  end Make_Complex_System;

  function Make_Complex_System
             ( s : OctoDobl_Speelpenning_Convolutions.Link_to_System )
             return OctoDobl_Complex_Circuits.Link_to_System is
  ...
  begin
    if s /= null then
      declare
        cfs : constant OctoDobl_Complex_Circuits.System(s.neq,s.dim)
            := Make_Complex_System(s.all);
      begin
        res := new OctoDobl_Complex_Circuits.System'(cfs);
      end;
    end if;
    return res;
  end Make_Complex_System;

end OctoDobl_Circuit_Makers;
