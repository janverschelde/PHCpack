...
package PentDobl_Newton_Convolutions is
...
--   All computations are performed in penta double precision.
...
-- AUXILIARIES :
  ...
  procedure Minus ( v : in PentDobl_Complex_VecVecs.VecVec );
  ...
  function Max ( v : PentDobl_Complex_VecVecs.VecVec ) return penta_double;

  -- DESCRIPTION :
  --   Returns the largest absolute value over all values in v.

  function Max ( deg : integer32;
                 v : PentDobl_Complex_VecVecs.VecVec ) return penta_double;
  ...
  procedure MaxIdx ( v : in PentDobl_Complex_VecVecs.VecVec;
                     tol : in double_float;
                     maxval : out penta_double; idx : out integer32 );
