...
package body DoblDobl_Complex_Circuits is
...
-- CONSTRUCTORS :
  ...
  function Allocate ( nbr,dim : integer32 ) return Circuit is
    ...
    forward : constant DoblDobl_Complex_Vectors.Vector(1..dim-1)
            := (1..dim-1 => zero);
    backward : constant DoblDobl_Complex_Vectors.Vector(1..dim-2)
             := (1..dim-2 => zero);
    cross : constant DoblDobl_Complex_Vectors.Vector(1..dim-2)
          := (1..dim-2 => zero);

  begin
    res.dim := dim;
    res.pdg := -1;
    res.fwd := new DoblDobl_Complex_Vectors.Vector'(forward);
    res.bck := new DoblDobl_Complex_Vectors.Vector'(backward);
    res.crs := new DoblDobl_Complex_Vectors.Vector'(cross);
    return res;
  end Allocate;
  ...
  begin
    ...
    res.yd := new DoblDobl_Complex_Vectors.Vector'(0..dim => zero);
    return res;
  end Create;
  ...
  function Allocate ( neq,dim : integer32 )
    ...
    res : DoblDobl_Complex_VecMats.VecMat(1..neq);
...
-- SINGULAR VALUE DECOMPOSITIONS :
  ...
  procedure Singular_Values
              ( s : in out System;
                ...
                svls : in DoblDobl_Complex_VecVecs.VecVec ) is
  ...
  begin
    if c.pdg <= 1 then
      s := (s'range => DoblDobl_Complex_Numbers.Create(integer32(0)));
    else
      Speel(c,x,yd,pwt,A);
      DoblDobl_Complex_Singular_Values.SVD(A,c.dim,c.dim,s,e,U,V,0,info);
    end if;
  end Singular_Values;
...
-- ALGORITHMIC DIFFERENTIATION AND EVALUATION OF CIRCUITS :
  ...
  procedure EvalDiff2
              ( s : in out System;
                x : in DoblDobl_Complex_Vectors.Link_to_Vector;
                vh : in DoblDobl_Complex_VecMats.VecMat ) is
  begin
    Power_Table(s.mxe,x,s.pwt);
    EvalDiff2(s.crc,x,s.yd,s.pwt,s.fx,s.jm,vh);
  end EvalDiff2;
  ...
  procedure EvalDiff
              ( c : in Circuits;
                ...
                jm : out DoblDobl_Complex_Matrices.Matrix ) is
  begin
    for i in c'range loop
      ...
      for j in jm'range(2) loop
        jm(i,j) := yd(j);
        yd(j) := DoblDobl_Complex_Numbers.Create(integer(0));
      end loop;
    end loop;
  end EvalDiff;
  ...
  procedure EvalDiff2
    ...
    mat : DoblDobl_Complex_Matrices.Link_to_Matrix;
...
-- ALGORITMIC DIFFERENTIATION AND EVALUATION OF CIRCUIT :
  ...
  procedure Indexed_Speel
              ( c : in Circuit;
                x,yd : in DoblDobl_Complex_Vectors.Link_to_Vector;
                h : out DoblDobl_Complex_Matrices.Matrix ) is
  begin
  ...
  procedure Indexed_Speel
              ( c : in Circuit;
                x,yd : in DoblDobl_Complex_Vectors.Link_to_Vector ) is
  begin
  ...
  procedure Speel ( c : in Circuit;
                    x,yd : in DoblDobl_Complex_Vectors.Link_to_Vector;
                    pwt : in DoblDobl_Complex_VecVecs.VecVec ) is
  begin
  ...
  procedure Indexed_Speel
              ( idx : in Standard_Integer_VecVecs.VecVec;
                ...
                crs : in DoblDobl_Complex_Vectors.Link_to_Vector ) is

    idk : Standard_Integer_Vectors.Link_to_Vector;
    idx1,idx2 : integer32;
    kcff : DoblDobl_Complex_Numbers.Complex_Number;

    use DoblDobl_Complex_Numbers,Standard_Integer_Vectors;
  ...
  procedure Indexed_Speel
              ( idx : in Standard_Integer_Vectors.Vector;
                ...
                h : in out DoblDobl_Complex_Matrices.Matrix ) is
  ...
  procedure Speel ( xps,idx,fac : in Standard_Integer_VecVecs.VecVec;
    ...
    idx1,idx2 : integer32;
    kcff,acc,wrk : DoblDobl_Complex_Numbers.Complex_Number;
    factor : double_double;
        ...
        if idk'last < idk'first then -- deal with stray 0 exponent
          ...
          if fck = null then
            ...
            if idk'last = 1 then
              ...
              factor := Double_Double_Numbers.Create(xpk(idx1));
              ...
              if idk'last = 2 then
                idx2 := idk(2);
                wrk := acc*x(idx1);
                factor := Double_Double_Numbers.Create(xpk(idx2));
  ...
  begin
    ...
    if idx'last = 1 then
      ...
      yd(0) := yd(0) + acc*x(idx1);
      factor := Double_Double_Numbers.Create(xps(idx1));
      yd(idx1) := yd(idx1) + factor*acc;
    else
      Forward_Backward_Cross(idx,x,fwd,bck,crs);
      Multiply_Factor(xps,fac,x,cff,pwt,acc);
      ...
      if idx'last = 2 then
        idx2 := idx(2);
        wrk := acc*x(idx1);
        factor := Double_Double_Numbers.Create(xps(idx2));
        wrk := factor*wrk; yd(idx2) := yd(idx2) + wrk;
        wrk := acc*x(idx2);
        factor := Double_Double_Numbers.Create(xps(idx1));
        wrk := factor*wrk; yd(idx1) := yd(idx1) + wrk;
      ...
      begin
        ...
        if fck = null then
          ...
          if idk'last = 1 then -- one variable special case
            DoblDobl_Hessian_Updaters.Speel1(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 2 then -- two variable special case
            DoblDobl_Hessian_Updaters.Speel2(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 3 then -- three variable special case
            DoblDobl_Hessian_Updaters.Speel3(h,kcff,xpk,idk,fck.all,x.all,pwt);
          elsif idk'last = 4 then -- four variable special case
            DoblDobl_Hessian_Updaters.Speel4(h,kcff,xpk,idk,fck.all,x.all,pwt);
          else
            DoblDobl_Hessian_Updaters.SpeelN
...
-- AUXILIARY PROCEDURES :
  ...
  procedure Forward ( x : in DoblDobl_Complex_Vectors.Link_to_Vector;
                      f : in DoblDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  procedure Forward_Backward
              ( x,f,b : in DoblDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  procedure Forward_Backward_Cross
              ( x,f,b,c : in DoblDobl_Complex_Vectors.Link_to_Vector ) is
  ...
  function Allocate
    ...
    res : DoblDobl_Complex_VecVecs.VecVec(mxe'range);
    zero : constant DoblDobl_Complex_Numbers.Complex_Number
         := DoblDobl_Complex_Numbers.Create(integer(0));
  ...
  begin
    for k in mxe'range loop
      if mxe(k) > 1 then
        res(k) := new DoblDobl_Complex_Vectors.Vector'(1..mxe(k)-1 => zero);
      end if;
    end loop;
    return res;
  end Allocate;
  ...
  procedure Multiply_Factor
              ( xps,fac : in Standard_Integer_Vectors.Link_to_Vector;
                ...
                cff : in DoblDobl_Complex_Numbers.Complex_Number;
                pwt : in DoblDobl_Complex_VecVecs.VecVec;
                res : out DoblDobl_Complex_Numbers.Complex_Number ) is

    pwx : DoblDobl_Complex_Vectors.Link_to_Vector;
...
-- DESTRUCTORS :
  ...
  begin
    c.dim := -1;
    c.pdg := -1;
    DoblDobl_Complex_Vectors.Clear(c.fwd);
    DoblDobl_Complex_Vectors.Clear(c.bck);
    DoblDobl_Complex_Vectors.Clear(c.crs);
  end Clear;
  ...
  begin
    Clear(s.crc);
    DoblDobl_Complex_Vectors.Clear(s.yd);
    DoblDobl_Complex_VecVecs.Clear(s.pwt);
  end Clear;
