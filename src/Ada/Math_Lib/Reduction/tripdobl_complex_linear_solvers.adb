...
package body TripDobl_Complex_Linear_Solvers is

-- AUXLILIARIES :

  function cabs ( c : Complex_Number ) return triple_double is
  begin
...
-- TARGET ROUTINES :
  ...
  procedure Scale ( a : in out TripDobl_Complex_Matrices.Matrix;
                    b : in out TripDobl_Complex_Vectors.Vector ) is

    fac : Complex_Number;

    function Maximum ( a : in TripDobl_Complex_Matrices.Matrix;
                       i : in integer32 ) return Complex_Number is

      res : integer32 := a'first(2);
      max : triple_double := cabs(a(i,res));
      tmp : triple_double;
    ...
    procedure Divide ( a : in out TripDobl_Complex_Matrices.Matrix;
                       b : in out TripDobl_Complex_Vectors.Vector;
  ...
  function Norm1 ( a : TripDobl_Complex_Matrices.Matrix )
  ...
  function Norm1 ( a : TripDobl_Complex_VecVecs.VecVec )
                 return triple_double is

    res : triple_double := Create(0.0);
    sum : triple_double;
    aj : TripDobl_Complex_Vectors.Link_to_Vector;
...
-- TARGET ROUTINES :
  ...
  procedure lufac ( a : in out TripDobl_Complex_Matrices.Matrix;
                    n : in integer32;
    ...
    smax : triple_double;
    temp : Complex_Number;
    dd_min_one : constant triple_double := create(-1.0);
  ...
  procedure lufac ( a : in out TripDobl_Complex_VecVecs.VecVec;
                    n : in integer32;
                    ipvt : out Standard_Integer_Vectors.Vector;
                    info : out integer32 ) is
    ...
    smax : triple_double;
    temp : Complex_Number;
    ak,aj : TripDobl_Complex_Vectors.Link_to_Vector;
    dd_min_one : constant triple_double := create(-1.0);
    cp_min_one : constant Complex_Number := create(dd_min_one);
    ...
    if nm1 >= 1 then
      for k in 1..nm1 loop
        ...
        if is_zero(smax) then       -- this column is already triangularized
          ...
          temp := cp_min_one/ak(k);                   -- compute multipliers
          for i in kp1..n loop
            ak(i) := temp*ak(i);
          end loop;
          for j in kp1..n loop       -- row elimination with column indexing
  ...
  procedure estco ( a : in TripDobl_Complex_Matrices.Matrix;
                    n : in integer32;
                    ipvt : in Standard_Integer_Vectors.Vector;
                    anorm : in triple_double; rcond : out triple_double ) is

    z : TripDobl_Complex_Vectors.Vector(1..n);
    kb,kp1,l : integer32;
    s,sm,sum,ynorm : triple_double;
  ...
  procedure estco ( a : in TripDobl_Complex_VecVecs.VecVec;
                    n : in integer32;
                    ipvt : in Standard_Integer_Vectors.Vector;
                    anorm : in triple_double; rcond : out triple_double ) is
  ...
  procedure lufco ( a : in out TripDobl_Complex_Matrices.Matrix;
                    n : in integer32;
                    ipvt : out Standard_Integer_Vectors.Vector;
                    rcond : out triple_double ) is

    anorm : constant triple_double := Norm1(a);
    info : integer32;
  ...
  procedure lufco ( a : in out TripDobl_Complex_VecVecs.VecVec;
                    n : in integer32;
                    ipvt : out Standard_Integer_Vectors.Vector;
                    rcond : out triple_double ) is
  ...
  procedure lusolve ( a : in TripDobl_Complex_Matrices.Matrix;
                      n : in integer32;
                      ipvt : in Standard_Integer_Vectors.Vector;
                      b : in out TripDobl_Complex_Vectors.Vector ) is
  ...
  procedure lusolve ( a : in TripDobl_Complex_VecVecs.VecVec;
                      n : in integer32;
                      ipvt : in Standard_Integer_Vectors.Vector;
                      b : in out TripDobl_Complex_Vectors.Vector ) is

    ell,nm1,kb : integer32;
    ak : TripDobl_Complex_Vectors.Link_to_Vector;
    temp : Complex_Number;
  ...
  procedure Triangulate ( a : in out TripDobl_Complex_Matrices.Matrix;
                          tol : in double_float;
                          n,m : in integer32 ) is

    max,cbs : triple_double;
    temp : Complex_Number;
  ...
  procedure Diagonalize ( a : in out TripDobl_Complex_Matrices.Matrix;
                          n,m : in integer32 ) is
  ...
  begin
    ...
    while (k <= n) and (kcolumn <= m) loop
      ...
      if is_zero(max) then
        ...
        a(k,kcolumn) := TripDobl_Complex_Numbers.Create(integer(1));
        for i in 1..(k-1) loop
          for j in (kcolumn+1)..m loop
            a(i,j) := a(i,j) - a(i,kcolumn) * a(k,j);
          end loop;
...
-- TO TEST THE LU FACTORIZATION :
  ...
  function Permute ( P : Standard_Natural_Matrices.Matrix;
    ...
    fP : TripDobl_Complex_Matrices.Matrix(P'range(1),P'range(2));
  ...
  function Lower_Diagonal ( A : TripDobl_Complex_Matrices.Matrix )
                          return TripDobl_Complex_Matrices.Matrix is
  ...
  function Upper_Diagonal ( A : TripDobl_Complex_Matrices.Matrix )
                          return TripDobl_Complex_Matrices.Matrix is
  ...
  procedure Permute_Lower
              ( L : in out TripDobl_Complex_Matrices.Matrix;
...
end TripDobl_Complex_Linear_Solvers;
