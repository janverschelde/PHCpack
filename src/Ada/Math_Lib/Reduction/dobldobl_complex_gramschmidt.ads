...
package DoblDobl_Complex_GramSchmidt is

-- DESCRIPTION :
--   Offers the QR decomposition with the modified version of
--   the Gram-Schmidt orthonormalization method with complex
...
--   The routines do no pivoting.
  ...
  procedure Select_Pivot_Column
               ( k,m : in integer32;
                 ...
                 ind : out integer32; max : out double_double );
  ...
  --
  -- ON RETURN :
  --   maxerr    measures the deviation from orthonormality,
  ...
  -- ON RETURN :
  --   maxerr    measure the deviation of the decomposition,
  --             maxerr > tol implies fail, otherwise, if not fail,
  ...
  procedure Orthogonal_Projection
              ( n,m : in integer32;
                ...
                qb : out DoblDobl_Complex_Vectors.Vector );
