...
package body TripDobl_Complex_QR_Least_Squares is
  ...
  function cdabs ( a : Complex_Number ) return triple_double is
  ...
  --   corresponds to the 'cdabs' fortran function.
    ...
    res : constant triple_double
        := SQRT(sqr(REAL_PART(a)) + sqr(IMAG_PART(a)));
  ...
  procedure zswap ( a : in out TripDobl_Complex_Matrices.Matrix;
  ...
  procedure zcopy ( n,start : in integer32;
                    x : in TripDobl_Complex_Vectors.Vector;
                    y : out TripDobl_Complex_Vectors.Vector ) is
  ...
  function znrm2 ( a : TripDobl_Complex_Matrices.Matrix;
  ...
  function zdot ( a : TripDobl_Complex_Matrices.Matrix;
                  row,k1,k2 : integer32 ) 
  ...
  function zdotc ( row : integer32;
                   x : TripDobl_Complex_Matrices.Matrix;
                   y : TripDobl_Complex_Vectors.Vector )
                 return Complex_Number is
  ...
  procedure zaxpy ( a : in out TripDobl_Complex_Matrices.Matrix; 
  ...
  procedure zscal ( a : in out TripDobl_Complex_Matrices.Matrix;
  ...
  procedure zaxpy ( n,row,col : in integer32; f : in Complex_Number;
                    x : in TripDobl_Complex_Matrices.Matrix;
                    y : in out TripDobl_Complex_Vectors.Vector ) is
  ...
  procedure QRD ( x : in out TripDobl_Complex_Matrices.Matrix;
                  qraux : in out TripDobl_Complex_Vectors.Vector;
        ...
        if AbsVal(nrmxl) /= zero then                       -- for column ell
          ...
          for j in lp1..p loop                --  columns, updating the norms
            ...
            if (j >= pl) and (j <= pu) and (AbsVal(qraux(j)) /= zero) then
              tt := 1.0 - (cdabs(x(ell,j))/sqr(REAL_PART(qraux(j))));
              tt := dmax1(tt,zero);
              t := Create(tt);
              tt := 1.0 + 0.05*tt*(REAL_PART(qraux(j))/sqr(REAL_PART(work(j))));
              if tt /= one then
  ...
  procedure Basis ( qr : in out TripDobl_Complex_Matrices.Matrix;
                    x : in TripDobl_Complex_Matrices.Matrix ) is

    sum : Complex_Number;
    wrk : TripDobl_Complex_Vectors.Vector(qr'range(1));
  ...
  procedure QRLS ( x : in out TripDobl_Complex_Matrices.Matrix;
                   n,k : in integer32;
                   qraux,y : in TripDobl_Complex_Vectors.Vector;
                   qy,qty,b,rsd,xb : out TripDobl_Complex_Vectors.Vector;
...
end TripDobl_Complex_QR_Least_Squares;
