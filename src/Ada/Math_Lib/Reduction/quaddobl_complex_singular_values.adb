...
package body QuadDobl_Complex_Singular_Values is
...
-- AUXILIARY BLAS ROUTINES :
  ...
  function dmax1 ( x1,x2 : quad_double ) return quad_double is
  ...
  function dmax1 ( x1,x2,x3 : quad_double ) return quad_double is
  ...
  function dmax1 ( x1,x2,x3,x4 : quad_double ) return quad_double is
  ...
  function dmax1 ( x1,x2,x3,x4,x5 : quad_double ) return quad_double is
  ...
  function cabs1 ( z : Complex_Number ) return quad_double is
  ...
  function cdabs ( z : Complex_Number ) return quad_double is
  ...
  --   For efficiency, this should have been done anyway ...
    ...
    rpz : constant quad_double := REAL_PART(z);
    ipz : constant quad_double := IMAG_PART(z);
...
--                   raise;
  ...
  --   and continueing n steps with increment incx.
    ...
    norm,scale,ssq,temp : quad_double;
...
--    when others => put_line("exception caught by zdotc"); raise;
  ...
  procedure drotg ( da,db,c,s : in out quad_double ) is
...
-- TARGET PROCEDURES :
  ...
  procedure SVD ( x : in out Matrix; n,p : in integer32;
    ...
    t,r : Complex_Number;
    b,c,cs,el,emm1,f,g,scale,shift,sl,sm,sn : quad_double;
    smm1,t1,test,ztest : quad_double;
    wantu,wantv : boolean;
    zero : constant quad_double := create(0.0);
    one : constant quad_double := create(1.0);
    minusone : constant quad_double := create(-1.0);
...
--         raise;
  ...
  function Inverse_Condition_Number
             ( s : Vector ) return quad_double is

    smax : constant quad_double := AbsVal(s(s'first));
    one : constant quad_double := create(1.0);
    smin,val : quad_double;
  ...
  function Inverse_Condition_Number
             ( s : Vector; tol : double_float ) return quad_double is
...
end QuadDobl_Complex_Singular_Values;
