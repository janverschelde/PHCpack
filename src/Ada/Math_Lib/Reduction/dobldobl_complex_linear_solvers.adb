...
package body DoblDobl_Complex_Linear_Solvers is
...
-- TARGET ROUTINES :
  ...
  function Norm1 ( a : DoblDobl_Complex_VecVecs.VecVec )
    ...
    res : double_double := Create(0.0);
    sum : double_double;
    aj : DoblDobl_Complex_Vectors.Link_to_Vector;
...
-- TARGET ROUTINES :
  ...
  procedure lufac ( a : in out DoblDobl_Complex_VecVecs.VecVec;
    ...
    smax : double_double;
    temp : Complex_Number;
    ak,aj : DoblDobl_Complex_Vectors.Link_to_Vector;
    dd_min_one : constant double_double := create(-1.0);
    cp_min_one : constant Complex_Number := create(dd_min_one);
  ...
  begin
    ...
    while (k <= n) and (kcolumn <= m) loop
      ...
      if is_zero(max) then
        ...
        a(k,kcolumn) := DoblDobl_Complex_Numbers.Create(integer(1));
        for i in 1..(k-1) loop
          for j in (kcolumn+1)..m loop
            a(i,j) := a(i,j) - a(i,kcolumn) * a(k,j);
          end loop;
