...
--   Test on matrices of truncated dense power series.
  ...
  begin
    for i in v'range loop
      ...
      Standard_Complex_Series_io.put(v(i));
    end loop;
  end Write;
  ...
  procedure Write ( A : in Standard_Complex_Series_Matrices.Matrix ) is
  ...
  begin
    for i in A'range(1) loop
      for j in A'range(2) loop
        ...
        Standard_Complex_Series_io.put(A(i,j));
      end loop;
    end loop;
  end Write;
  ...
  -- DESCRIPTION :
  --   Basic output of a standard complex matrix.
  ...
  --   The series are all of the given degree.
    ...
    use Standard_Series_Linear_Solvers;
  ...
  function Trunc ( A : Standard_Complex_Series_Matrices.Matrix )
  ...
  procedure Zero_QRD ( A : Standard_Complex_Series_Matrices.Matrix ) is
  ...
  procedure Solve_Normal_Equations
              ( A : in Standard_Complex_Series_Matrices.Matrix;
                x,b : in Standard_Complex_Series_Vectors.Vector ) is
  ...
  --   and the residual of the normal equations is computed as well.
    ...
    Tb : constant Standard_Complex_Series_Vectors.Vector(A'range(2)) := T*b;
    n : constant integer32 := A'last(2);
    ipvt : Standard_Integer_Vectors.Vector(1..n);
    info : integer32;
    sol,rsd : Standard_Complex_Series_Vectors.Vector(1..n);

  begin
    Standard_Complex_Series_Matrices.Copy(TA,wrk);
    LUfac(wrk,n,ipvt,info);
    put("The pivots : "); put(ipvt); new_line;
    if info /= 0 then
      put("info : "); put(info,1); new_line;
    else
      Standard_Complex_Series_Vectors.Copy(Tb,sol);
  ...
  procedure Test_Normality
              ( qr : in Standard_Complex_Series_Matrices.Matrix ) is
  ...
  --   in standard double precision.
    ...
    use Standard_Complex_Series_io;
    use Standard_Complex_Series_Vectors;
  ...
  --   all other vectors to test whether they are orthogonal.
    ...
    u,v : Standard_Complex_Series_Vectors.Vector(qr'range(1));
    deg : constant integer32 := qr(qr'first(1),qr'first(2)).deg;
    ipr : Standard_Complex_Series.Series(deg);
  ...
  procedure Test_Basis
              ( wrk,A : in Standard_Complex_Series_Matrices.Matrix ) is
  ...
  begin
    Standard_Complex_Series_Matrices.Copy(wrk,qr);
    new_line;
    put_line("Computing the orthogonal part of the QR decomposition ...");
    Standard_Series_Least_Squares.Basis(qr,A);
    Test_Normality(qr);   
    new_line;
  ...
  --   and the residual is computed.
    ...
    use Standard_Complex_Series_Vectors;
    use Standard_Complex_Series_Matrices;
    use Standard_Series_Least_Squares;

    deg : constant integer32 := A(A'first(1),A'first(2)).deg;
    wrk : Standard_Complex_Series_Matrices.Matrix(A'range(1),A'range(2));
    ipvt : Standard_Integer_Vectors.Vector(A'range(2)) := (A'range(2) => 0);
    qraux,sol : Standard_Complex_Series_Vectors.Vector(A'range(2));
    info : integer32;
    n : constant integer32 := A'last(1);
    m : constant integer32 := A'last(2);
    rsd,dum,dum2,dum3 : Standard_Complex_Series_Vectors.Vector(1..n);
    err : Standard_Complex_Series_Vectors.Vector(1..m);
    ans : character;
    nrm_rsd,nrm_err : double_float;

  begin
    Standard_Complex_Series_Matrices.Copy(A,wrk);
    for i in qraux'range loop
      qraux(i) := new Series'(Standard_Complex_Series.Create(0,deg));
      sol(i) := new Series'(Standard_Complex_Series.Create(0,deg));
    end loop;
  ...
  --   Computations are done in standard double precision.
    ...
    A : constant Standard_Complex_Series_Matrices.Matrix(1..n,1..m)
      := Standard_Random_Series_Matrices.Random_Series_Matrix(1,n,1,m,degree);
    x : constant Standard_Complex_Series_Vectors.Vector(1..m)
      := Standard_Random_Series_Vectors.Random_Series_Vector(1,m,degree);
  ...
  procedure Test_Solving ( precision : in character ) is
  ...
  begin
    ...
    if ans = 'y' then
      ...
      case precision is
        when '0' => Standard_Random_Linear_Solve(dim,deg);
        when '1' => DoblDobl_Random_Linear_Solve(dim,deg);
        when '2' => QuadDobl_Random_Linear_Solve(dim,deg);
        when others => null;
      end case;
    else
      ...
      case precision is
        when '0' => Standard_Random_Least_Squares(nrows,ncols,deg);
        when '1' => DoblDobl_Random_Least_Squares(nrows,ncols,deg);
        when '2' => QuadDobl_Random_Least_Squares(nrows,ncols,deg);
        when others => null;
      end case;
    end if;
  end Test_Solving;
  ...
  procedure Test_Conversion ( precision : in character ) is
  ...
  begin
    ...
    new_line;
    case ans is
      when '0' => Test_Solving(prc);
      when '1' => Test_Conversion(prc);
      when others => null;
    end case;
  end Main;
