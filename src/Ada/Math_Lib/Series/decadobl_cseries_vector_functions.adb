...
package body DecaDobl_CSeries_Vector_Functions is
  ...
  begin
    for k in v'range loop
      res(k) := DecaDobl_Complex_Series_Functions.Eval(v(k),t);
    end loop;
    return res;
  end Eval;
  ...
  begin
    for k in v'range loop
      res(k) := DecaDobl_Complex_Series_Functions.Eval(v(k),t,w(k),w(w'last));
    end loop;
    return res;
  end Eval;
  ...
  function Shift ( v : DecaDobl_Complex_Series_Vectors.Vector;
    ...
    res : DecaDobl_Complex_Series_Vectors.Vector(v'range);

  begin
    for i in v'range loop
      res(i) := DecaDobl_Complex_Series_Functions.Shift(v(i),c);
    end loop;
    return res;
  end Shift;
  ...
  function Shift ( v : DecaDobl_Complex_Series_VecVecs.VecVec;
    ...
    res : DecaDobl_Complex_Series_VecVecs.VecVec(v'range);
  ...
  begin
    for i in v'range loop
      DecaDobl_Complex_Series_Functions.Shift(v(i),c);
    end loop;
  end Shift;
  ...
  begin
    for i in v'range loop
      if v(i) /= null then
        declare
          cp : constant DecaDobl_Complex_Series_Vectors.Vector(v(i)'range)
             := Make_Deep_Copy(v(i).all);
        begin
          res(i) := new DecaDobl_Complex_Series_Vectors.Vector'(cp);
        end;
      end if;
    end loop;
    return res;
  ...
  procedure Deep_Clear
              ( v : in out DecaDobl_Complex_Series_Vectors.Vector ) is
  ...
  begin
    for i in v'range loop
      if v(i) /= null
       then DecaDobl_Complex_Series.Clear(v(i));
      end if;
    end loop;
  end Deep_Clear;

  procedure Deep_Clear
              ( v : in out DecaDobl_Complex_Series_VecVecs.VecVec ) is
  ...
  begin
    ...
    DecaDobl_Complex_Series_VecVecs.Clear(v);
  end Deep_Clear;
