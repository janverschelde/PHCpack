...
package body HexaDobl_CSeries_Vector_Norms is
  ...
  begin
    for i in v'range loop
      res(i) := HexaDobl_Complex_Series_Norms.Conjugate(v(i));
    end loop;
    return res;
  end Conjugate;
  ...
  function Norm ( v : Vector ) return Series is
    ...
    res : constant Series := HexaDobl_Complex_Algebraic_Series.sqrt(sn,0);
  ...
  function Max_Norm ( v : Vector ) return hexa_double is
    ...
    res : hexa_double
        := HexaDobl_Complex_Series_Norms.Max_Norm(v(v'first).all);
    nrm : hexa_double;

  begin
    for i in v'first+1..v'last loop
      nrm := HexaDobl_Complex_Series_Norms.Max_Norm(v(i).all);
      if nrm > res
       then res := nrm;
      end if;
    end loop;
