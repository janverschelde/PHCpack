...
package body OctoDobl_Complex_Series_Functions is

  function Eval ( s : Series; t : octo_double ) return Complex_Number is
  ...
  function Eval ( s : Link_to_Series;
                  t : octo_double ) return Complex_Number is
  ...
  function Eval ( s : Series; t : octo_double;
                  a,b : integer32 ) return Complex_Number is

    dd_a : octo_double := create(a);
    dd_b : constant octo_double := create(b);
    pow : octo_double := dd_a/dd_b;
    pwt : octo_double := t**pow;
  ...
  function Eval ( s : Link_to_Series; t : octo_double;
                  a,b : integer32 ) return Complex_Number is
...
-- ORDER and FILTER :
  ...
  begin
    for k in 0..s.deg loop
      if OctoDobl_Complex_Numbers.AbsVal(s.cff(k)) > tol
       then return k;
      end if;
    end loop;
    return s.deg+1;
...
-- SHIFT OPERATORS :
  ...
  function Shift ( s : Series; c : octo_double ) return Series is
  ...
  begin
    for i in 0..s.deg loop
      ...
      for j in 0..i loop
       -- bcf := octo_double_numbers.create(sgn*binomial(i,j));
        bcf := binomial(i,j);
        bcf := octo_double_numbers.create(sgn)*bcf;
  ...
  function Shift ( s : Link_to_Series;
                   c : octo_double ) return Link_to_Series is
  begin
    if s = null
     then return null;
  ...
  procedure Shift ( s : in out Series; c : in octo_double ) is
  ...
  procedure Shift ( s : in Link_to_Series; c : in octo_double ) is
  begin
    if s /= null then
      declare
...
end OctoDobl_Complex_Series_Functions;
