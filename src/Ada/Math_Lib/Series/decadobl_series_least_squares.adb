...
with DecaDobl_Complex_Series_Norms;       use DecaDobl_Complex_Series_Norms;

package body DecaDobl_Series_Least_Squares is
...
-- AUXILIARIES :
  ...
  function dmax1 ( a,b : deca_double ) return deca_double is
  ...
  procedure zswap ( a : in out DecaDobl_Complex_Series_Matrices.Matrix;
  ...
  procedure zcopy ( n,start : in integer32;
                    x : in DecaDobl_Complex_Series_Vectors.Vector;
                    y : out DecaDobl_Complex_Series_Vectors.Vector ) is
  ...
  function znrm2 ( a : DecaDobl_Complex_Series_Matrices.Matrix;
  ...
  begin
    ...
    return DecaDobl_Complex_Algebraic_Series.sqrt(sum,0);
  end znrm2;

  function zdot ( a : DecaDobl_Complex_Series_Matrices.Matrix;
  ...
  function zdotc ( row : integer32;
                   x : DecaDobl_Complex_Series_Matrices.Matrix;
                   y : DecaDobl_Complex_Series_Vectors.Vector )
                 return Series is
  ...
  procedure zaxpy ( a : in out DecaDobl_Complex_Series_Matrices.Matrix; 
  ...
  procedure zscal ( a : in out DecaDobl_Complex_Series_Matrices.Matrix;
  ...
  procedure zaxpy ( n,row,col : in integer32; f : in Series;
                    x : in DecaDobl_Complex_Series_Matrices.Matrix;
                    y : in out DecaDobl_Complex_Series_Vectors.Vector ) is
  ...
  function Create ( x : deca_double; deg : integer32 ) return Series is
...
-- TARGET PROCEDURES :
  ...
  procedure QRD ( x : in out DecaDobl_Complex_Series_Matrices.Matrix;
                  qraux : in out DecaDobl_Complex_Series_Vectors.Vector;
                  jpvt : in out Standard_Integer_Vectors.Vector;
                  piv : in boolean ) is

    zero : constant deca_double := create(0.0);
    dd_one : constant deca_double := create(1.0);
    absq : deca_double;
    n : constant integer32 := x'length(1);  -- number of rows
    p : constant integer32 := x'length(2);  -- number of columns
    deg : constant integer32 := x(x'first(1),x'first(2)).deg;
    work : DecaDobl_Complex_Series_Vectors.Vector(x'range(2));
    jj,jp,lp1,lup,maxj,pl,pu : integer32;
    maxnrm,tt,quot : deca_double;
    nrmxl,t : Series(deg);
    fac : Complex_Number;
    negj,swapj : boolean;
  ...
  begin
    for i in work'range loop
      work(i) := new Series'(DecaDobl_Complex_Series.Create(0,deg));
    end loop;
    pl := 1; pu := 0;
    if piv then
    ...
    DecaDobl_Complex_Series_Vectors.Clear(work);
  end QRD;

  procedure Basis ( qr : in out DecaDobl_Complex_Series_Matrices.Matrix;
                    x : in DecaDobl_Complex_Series_Matrices.Matrix ) is

    deg : constant integer32 := qr(qr'first(1),qr'first(2)).deg;
    sum,acc : Series(deg);
    wrk : DecaDobl_Complex_Series_Vectors.Vector(qr'range(1));

  begin
    for i in wrk'range loop
      wrk(i) := new Series'(DecaDobl_Complex_Series.Create(0,deg));
    end loop;
    ...
    DecaDobl_Complex_Series_Vectors.Clear(wrk);
  end Basis;

  procedure QRLS ( x : in out DecaDobl_Complex_Series_Matrices.Matrix;
                   n,k : in integer32;
                   qraux,y : in DecaDobl_Complex_Series_Vectors.Vector;
                   qy,qty,b,rsd,xb : out DecaDobl_Complex_Series_Vectors.Vector;
                   job : in integer32; info : out integer32 ) is

    zero : constant deca_double := create(1.0);
    absq : deca_double;
    deg : constant integer32 := x(x'first(1),x'first(2)).deg;
...
end DecaDobl_Series_Least_Squares;
