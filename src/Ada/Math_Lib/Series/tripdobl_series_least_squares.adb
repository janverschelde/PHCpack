...
with TripDobl_Complex_Series_Norms;       use TripDobl_Complex_Series_Norms;

package body TripDobl_Series_Least_Squares is
...
-- AUXILIARIES :
  ...
  function dmax1 ( a,b : triple_double ) return triple_double is
  ...
  procedure zswap ( a : in out TripDobl_Complex_Series_Matrices.Matrix;
  ...
  procedure zcopy ( n,start : in integer32;
                    x : in TripDobl_Complex_Series_Vectors.Vector;
                    y : out TripDobl_Complex_Series_Vectors.Vector ) is
  ...
  function znrm2 ( a : TripDobl_Complex_Series_Matrices.Matrix;
  ...
  begin
    ...
    return TripDobl_Complex_Algebraic_Series.sqrt(sum,0);
  end znrm2;

  function zdot ( a : TripDobl_Complex_Series_Matrices.Matrix;
  ...
  function zdotc ( row : integer32;
                   x : TripDobl_Complex_Series_Matrices.Matrix;
                   y : TripDobl_Complex_Series_Vectors.Vector )
                 return Series is
  ...
  procedure zaxpy ( a : in out TripDobl_Complex_Series_Matrices.Matrix; 
  ...
  procedure zscal ( a : in out TripDobl_Complex_Series_Matrices.Matrix;
  ...
  procedure zaxpy ( n,row,col : in integer32; f : in Series;
                    x : in TripDobl_Complex_Series_Matrices.Matrix;
                    y : in out TripDobl_Complex_Series_Vectors.Vector ) is
  ...
  function Create ( x : triple_double; deg : integer32 ) return Series is
...
-- TARGET PROCEDURES :
  ...
  procedure QRD ( x : in out TripDobl_Complex_Series_Matrices.Matrix;
                  qraux : in out TripDobl_Complex_Series_Vectors.Vector;
                  jpvt : in out Standard_Integer_Vectors.Vector;
                  piv : in boolean ) is

    zero : constant triple_double := create(0.0);
    dd_one : constant triple_double := create(1.0);
    absq : triple_double;
    n : constant integer32 := x'length(1);  -- number of rows
    p : constant integer32 := x'length(2);  -- number of columns
    deg : constant integer32 := x(x'first(1),x'first(2)).deg;
    work : TripDobl_Complex_Series_Vectors.Vector(x'range(2));
    jj,jp,lp1,lup,maxj,pl,pu : integer32;
    maxnrm,tt,quot : triple_double;
    nrmxl,t : Series(deg);
    fac : Complex_Number;
    negj,swapj : boolean;
  ...
  begin
    for i in work'range loop
      work(i) := new Series'(TripDobl_Complex_Series.Create(0,deg));
    end loop;
    pl := 1; pu := 0;
    if piv then
    ...
    TripDobl_Complex_Series_Vectors.Clear(work);
  end QRD;

  procedure Basis ( qr : in out TripDobl_Complex_Series_Matrices.Matrix;
                    x : in TripDobl_Complex_Series_Matrices.Matrix ) is

    deg : constant integer32 := qr(qr'first(1),qr'first(2)).deg;
    sum,acc : Series(deg);
    wrk : TripDobl_Complex_Series_Vectors.Vector(qr'range(1));

  begin
    for i in wrk'range loop
      wrk(i) := new Series'(TripDobl_Complex_Series.Create(0,deg));
    end loop;
    ...
    TripDobl_Complex_Series_Vectors.Clear(wrk);
  end Basis;

  procedure QRLS ( x : in out TripDobl_Complex_Series_Matrices.Matrix;
                   n,k : in integer32;
                   qraux,y : in TripDobl_Complex_Series_Vectors.Vector;
                   qy,qty,b,rsd,xb : out TripDobl_Complex_Series_Vectors.Vector;
                   job : in integer32; info : out integer32 ) is

    zero : constant triple_double := create(1.0);
    absq : triple_double;
    deg : constant integer32 := x(x'first(1),x'first(2)).deg;
...
end TripDobl_Series_Least_Squares;
