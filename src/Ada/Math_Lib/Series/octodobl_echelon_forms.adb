...
package body OctoDobl_Echelon_Forms is
    ...
    for i in A'range(1) loop
      for j in A'range(2) loop
        if Is_Integer(A(i,j)) then
          put(" "); put(integer32(hihihi_part(REAL_PART(A(i,j)))),2);
        else
          put("  *");
        end if;
      end loop;
  ...
  procedure Swap_Elements
              ( v : in out OctoDobl_Complex_Vectors.Vector;
                i,j : in integer32 ) is
  ...
  function Max_on_Row
    ...
    res : integer32 := j;
    maxval : octo_double := AbsVal(A(i,j));
    val : octo_double;
  ...
  function Determinant
             ( L : OctoDobl_Complex_Matrices.Matrix ) return Complex_Number is
  ...
  procedure Solve_with_Echelon_Form
              ( L : in OctoDobl_Complex_Matrices.Matrix;
                ...
                x : out OctoDobl_Complex_Vectors.Vector ) is
  ...
  procedure Solve
    ...
    L : OctoDobl_Complex_Matrices.Matrix(A'range(1),A'range(2)) := A;
    U : OctoDobl_Complex_Matrices.Matrix(A'range(1),A'range(2));
    row_ipvt : Standard_Integer_Vectors.Vector(A'range(1));
    col_ipvt,pivots : Standard_Integer_Vectors.Vector(A'range(2));
    pb : OctoDobl_Complex_Vectors.Vector(b'range);
