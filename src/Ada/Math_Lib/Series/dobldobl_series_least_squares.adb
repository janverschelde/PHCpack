...
-- AUXILIARIES :
  ...
  procedure zswap ( a : in out DoblDobl_Complex_Series_Matrices.Matrix;
  ...
  procedure zcopy ( n,start : in integer32;
                    x : in DoblDobl_Complex_Series_Vectors.Vector;
                    y : out DoblDobl_Complex_Series_Vectors.Vector ) is
  ...
  function znrm2 ( a : DoblDobl_Complex_Series_Matrices.Matrix;
  ...
  function zdotc ( row : integer32;
                   x : DoblDobl_Complex_Series_Matrices.Matrix;
                   y : DoblDobl_Complex_Series_Vectors.Vector )
                 return Series is
  ...
  procedure zaxpy ( a : in out DoblDobl_Complex_Series_Matrices.Matrix; 
  ...
  procedure zscal ( a : in out DoblDobl_Complex_Series_Matrices.Matrix;
  ...
  procedure zaxpy ( n,row,col : in integer32; f : in Series;
                    x : in DoblDobl_Complex_Series_Matrices.Matrix;
                    y : in out DoblDobl_Complex_Series_Vectors.Vector ) is
...
-- TARGET PROCEDURES :
  ...
  procedure QRD ( x : in out DoblDobl_Complex_Series_Matrices.Matrix;
                  qraux : in out DoblDobl_Complex_Series_Vectors.Vector;
    ...
    deg : constant integer32 := x(x'first(1),x'first(2)).deg;
    work : DoblDobl_Complex_Series_Vectors.Vector(x'range(2));
  ...
  begin
    for i in work'range loop
      work(i) := new Series'(DoblDobl_Complex_Series.Create(0,deg));
    end loop;
    pl := 1; pu := 0;
    if piv then
    ...
    DoblDobl_Complex_Series_Vectors.Clear(work);
  end QRD;

  procedure Basis ( qr : in out DoblDobl_Complex_Series_Matrices.Matrix;
                    x : in DoblDobl_Complex_Series_Matrices.Matrix ) is

    deg : constant integer32 := qr(qr'first(1),qr'first(2)).deg;
    sum,acc : Series(deg);
    wrk : DoblDobl_Complex_Series_Vectors.Vector(qr'range(1));

  begin
    for i in wrk'range loop
      wrk(i) := new Series'(DoblDobl_Complex_Series.Create(0,deg));
    end loop;
    ...
    DoblDobl_Complex_Series_Vectors.Clear(wrk);
  end Basis;

  procedure QRLS ( x : in out DoblDobl_Complex_Series_Matrices.Matrix;
                   n,k : in integer32;
                   qraux,y : in DoblDobl_Complex_Series_Vectors.Vector;
                   qy,qty,b,rsd,xb : out DoblDobl_Complex_Series_Vectors.Vector;
...
end DoblDobl_Series_Least_Squares;
