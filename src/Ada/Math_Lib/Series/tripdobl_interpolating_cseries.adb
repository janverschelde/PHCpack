...
with TripDobl_Complex_QR_Least_Squares;
with TripDobl_Complex_Singular_Values;

package body TripDobl_Interpolating_CSeries is

  function Eval ( v : TripDobl_Complex_Vector_Series.Vector;
                  t : Complex_Number )
                return TripDobl_Complex_Vectors.Vector is

    vec : constant TripDobl_Complex_Vectors.Link_to_Vector := v.cff(0);
    res : TripDobl_Complex_Vectors.Vector(vec'range) := vec.all;
  ...
  function Eval ( m : TripDobl_Complex_Matrix_Series.Matrix;
                  t : Complex_Number )
                return TripDobl_Complex_Matrices.Matrix is

    mat : TripDobl_Complex_Matrices.Link_to_Matrix := m.cff(0);
    res : TripDobl_Complex_Matrices.Matrix(mat'range(1),mat'range(2))
        := mat.all;
    one : constant triple_double := create(1.0);
    pwt : Complex_Number := Create(one);
  ...
  function Rank ( A : TripDobl_Complex_Matrices.Matrix;
                  tol : double_float ) return integer32 is

    n : constant integer32 := A'last(1);
    p : constant integer32 := A'last(2);
    mm : constant integer32 := TripDobl_Complex_Singular_Values.Min0(n+1,p);
    s : TripDobl_Complex_Vectors.Vector(1..mm);
    e : TripDobl_Complex_Vectors.Vector(1..p);
    u : TripDobl_Complex_Matrices.Matrix(1..n,1..n);
    v : TripDobl_Complex_Matrices.Matrix(1..p,1..p);
    job : constant integer32 := 11;
    info : integer32;
    x : TripDobl_Complex_Matrices.Matrix(A'range(1),A'range(2)) := A;

  begin
    TripDobl_Complex_Singular_Values.SVD(x,n,p,s,e,u,v,job,info);
    return TripDobl_Complex_Singular_Values.Rank(s,tol);
  end Rank;

  function Full_Rank
             ( m : TripDobl_Complex_Matrix_Series.Matrix;
               d : integer32; verbose : boolean := true ) return boolean is

    t : constant Complex_Number := TripDobl_Random_Numbers.Random1;
    mat : TripDobl_Complex_Matrices.Link_to_Matrix := m.cff(0);
    val : TripDobl_Complex_Matrices.Matrix(mat'range(1),mat'range(2))
        := mat.all;
    one : constant triple_double := create(1.0);
    pwt : Complex_Number := Create(one);
  ...
  begin
    ...
    TripDobl_Complex_Singular_Values.SVD(val,n,p,s,e,u,v,job,info);
    rnk := TripDobl_Complex_Singular_Values.Rank(s,tol);
    if verbose then
  ...
  function Sample ( v : TripDobl_Complex_Vector_Series.Vector;
                    t : TripDobl_Complex_Vectors.Vector )
                  return TripDobl_Complex_VecVecs.VecVec is

    res : TripDobl_Complex_VecVecs.VecVec(t'range);
    lv : constant TripDobl_Complex_Vectors.Link_to_Vector := v.cff(0);

  begin
    for i in t'range loop
      declare
        e : constant TripDobl_Complex_Vectors.Vector(lv'range)
          := Eval(v,t(i));
      begin
        res(i) := new TripDobl_Complex_Vectors.Vector'(e);
      end;
    end loop;
    return res;
  end Sample;

  function Sample ( m : TripDobl_Complex_Matrix_Series.Matrix;
                    t : TripDobl_Complex_Vectors.Vector )
                  return TripDobl_Complex_VecMats.VecMat is

    res : TripDobl_Complex_VecMats.VecMat(t'range);
    lm : constant TripDobl_Complex_Matrices.Link_to_Matrix := m.cff(0);

  begin
    for i in t'range loop
      declare
        e : constant TripDobl_Complex_Matrices.Matrix(lm'range(1),lm'range(2))
          := Eval(m,t(i));
      begin
        res(i) := new TripDobl_Complex_Matrices.Matrix'(e);
      end;
    end loop;
    return res;
  end Sample;
  ...
  function Solve_Linear_Systems
             ( m : TripDobl_Complex_VecMats.VecMat;
               v : TripDobl_Complex_VecVecs.VecVec )
             return TripDobl_Complex_VecVecs.VecVec is

    res : TripDobl_Complex_VecVecs.VecVec(m'range);
    dim : constant integer32 := v(0)'last;
    info : integer32;
    ipvt : Standard_Integer_Vectors.Vector(1..dim);
    sol : TripDobl_Complex_Vectors.Vector(1..dim);
    wrk : TripDobl_Complex_Matrices.Matrix(1..dim,1..dim);

  begin
    for i in v'range loop
      sol := v(i).all;
      wrk := m(i).all;
      TripDobl_Complex_Linear_Solvers.lufac(wrk,dim,ipvt,info);
      TripDobl_Complex_Linear_Solvers.lusolve(wrk,dim,ipvt,sol);
      res(i) := new TripDobl_Complex_Vectors.Vector'(sol);
    end loop;
    return res;
  ...
  function Residuals
             ( m : TripDobl_Complex_VecMats.VecMat;
               v,x : TripDobl_Complex_VecVecs.VecVec )
             return Triple_Double_Vectors.Vector is

    res : Triple_Double_Vectors.Vector(v'range);
    dim : constant integer32 := v(0)'last;
    lm : TripDobl_Complex_Matrices.Link_to_Matrix;
    lv,lx : TripDobl_Complex_Vectors.Link_to_Vector;
    wrk : TripDobl_Complex_Vectors.Vector(1..dim);
  ...
  begin
    for i in res'range loop
      ...
      res(i) := TripDobl_Complex_Vector_Norms.Max_Norm(wrk);
    end loop;
    return res;
  end Residuals;

  function Transpose ( x : TripDobl_Complex_VecVecs.VecVec )
                     return TripDobl_Complex_VecVecs.VecVec is
  ...
  begin
    for i in res'range loop
      res(i) := new TripDobl_Complex_Vectors.Vector(1..deg+1);
    end loop;
  ...
  function Vandermonde_Matrix
    ...
    one : constant triple_double := create(1.0);
    res : TripDobl_Complex_Matrices.Matrix(1..dim,1..dim);
    idx : integer32 := 0;
  ...
  function Solve_Interpolation_Systems
             ( v : TripDobl_Complex_Matrices.Matrix;
               f : TripDobl_Complex_VecVecs.VecVec )
             return TripDobl_Complex_VecVecs.VecVec is

    res : TripDobl_Complex_VecVecs.VecVec(f'range);
    dim : constant integer32 := v'last(1);
    wrk : TripDobl_Complex_Matrices.Matrix(v'range(1),v'range(2)) := v;
    sol : TripDobl_Complex_Vectors.Vector(1..dim);
    ipvt : Standard_Integer_Vectors.Vector(1..dim);
    info : integer32;
  ...
  function Construct ( x : TripDobl_Complex_VecVecs.VecVec )
                     return TripDobl_Complex_Vector_Series.Vector is

    dim : constant integer32 := x'last;
    lx0 : constant TripDobl_Complex_Vectors.Link_to_Vector := x(x'first); 
  ...
  function Interpolate
             ( mat : TripDobl_Complex_Matrix_Series.Matrix;
               rhs : TripDobl_Complex_Vector_Series.Vector;
               verbose : boolean := true )
             return TripDobl_Complex_Vector_Series.Vector is

    res : TripDobl_Complex_Vector_Series.Vector(mat.deg);
    dim : constant integer32 := rhs.cff(0)'last;
    t : constant TripDobl_Complex_Vectors.Vector(0..mat.deg)
      := TripDobl_Random_Vectors.Random_Vector(0,mat.deg);
    m : constant TripDobl_Complex_VecMats.VecMat(t'range) := Sample(mat,t);
    v : constant TripDobl_Complex_VecVecs.VecVec(t'range) := Sample(rhs,t);
    x : TripDobl_Complex_VecVecs.VecVec(t'range);
    r : Triple_Double_Vectors.Vector(t'range);
    xt : TripDobl_Complex_VecVecs.VecVec(1..dim);
    vdm : TripDobl_Complex_Matrices.Matrix(1..mat.deg+1,1..mat.deg+1);
    cff : TripDobl_Complex_VecVecs.VecVec(1..dim);
  ...
  function Diff ( m : TripDobl_Complex_VecMats.VecMat;
                  t : Complex_Number; pow,ord : integer32 )
                return TripDobl_Complex_Matrices.Matrix is

    one : constant triple_double := create(1.0);
    lm0 : constant TripDobl_Complex_Matrices.Link_to_Matrix := m(0);
  ...
  function Hermite_Matrix
    ...
    lmt : constant TripDobl_Complex_Matrices.Link_to_Matrix := m(0);
    adim : constant integer32 := lmt'last(1);
    rdim : constant integer32 := adim*(m'last+1);
    res : TripDobl_Complex_Matrices.Matrix(1..rdim,1..rdim);
    wrk : TripDobl_Complex_Matrices.Matrix(1..adim,1..adim);
  ...
  function Hermite_Vector
    ...
    lv0 : constant TripDobl_Complex_Vectors.Link_to_Vector := v(0);
    adim : constant integer32 := lv0'last; 
    rdim : constant integer32 := adim*(v'last+1);
    res : TripDobl_Complex_Vectors.Vector(1..rdim);
    wrk : TripDobl_Complex_Vectors.Vector(lv0'range);
    one : constant triple_double := create(1.0);
    pwt : Complex_Number := Create(one);
    fac : Complex_Number;
  ...
  function Hermite_Interpolate
    ...
    A : TripDobl_Complex_Matrices.Matrix(1..bigdim,1..bigdim)
      := Hermite_Matrix(mat.cff(0..deg),t);
    b : TripDobl_Complex_Vectors.Vector(1..bigdim)
  ...
  begin
    ...
    TripDobl_Complex_Linear_Solvers.lufac(A,bigdim,ipvt,info);
    TripDobl_Complex_Linear_Solvers.lusolve(A,bigdim,ipvt,b);
    if verbose then
    ...
    for k in 0..deg loop
      ...
      res.cff(k) := new TripDobl_Complex_Vectors.Vector'(wrk);
    end loop;
    return res;
  ...
  function Hermite_Laurent_Matrix
    ...
    lmt : TripDobl_Complex_Matrices.Link_to_Matrix := m(0);
    ...
    res : TripDobl_Complex_Matrices.Matrix(1..nrows,1..ncols);
  ...
  function Hermite_Laurent_Vector
    ...
    lv : TripDobl_Complex_Vectors.Link_to_Vector := v(0);
    nvr : constant integer32 := lv'last;
    dim : constant integer32 := nvr*(2*v'last+1);
    res : TripDobl_Complex_Vectors.Vector(1..dim);
  ...
  procedure Write_Integer_Matrix
              ( A : in TripDobl_Complex_Matrices.Matrix ) is
  ...
  function Hermite_Laurent_Interpolate
    ...
    zero : constant triple_double := create(0.0);
    res : TripDobl_Complex_Vector_Series.Vector(deg);
    A : TripDobl_Complex_Matrices.Matrix(1..nrows,1..ncols)
      := Hermite_Laurent_Matrix(mat.cff(0..deg));
    b : constant TripDobl_Complex_Vectors.Vector(1..nrows)
      := Hermite_Laurent_Vector(rhs.cff(0..deg));
    qraux : TripDobl_Complex_Vectors.Vector(1..ncols)
          := (1..ncols => Create(zero));
    jpvt : Standard_Integer_Vectors.Vector(1..ncols) := (1..ncols => 0);
    sol : TripDobl_Complex_Vectors.Vector(1..ncols);
    rsd,dum,dum2,dum3 : TripDobl_Complex_Vectors.Vector(1..nrows);
    info,idx : integer32;
    wrk : TripDobl_Complex_Vectors.Vector(1..nbc);

    use TripDobl_Complex_QR_Least_Squares;
...
end TripDobl_Interpolating_CSeries;
