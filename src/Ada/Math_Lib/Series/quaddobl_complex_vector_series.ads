...
--   is represented as a vector of vectors, of a certain degree.
  ...
  type Vector ( deg : integer32 ) is record
    ...
    cff : QuadDobl_Complex_VecVecs.VecVec(0..deg);
  end record;
...
-- DESTRUCTORS :
  ...
  procedure Clear ( v : in out QuadDobl_Complex_Vector_Series.Vector );
  procedure Clear ( v : in out QuadDobl_Complex_Vector_Series.Link_to_Vector );
