...
package body DoblDobl_Complex_Series_Functions is
  ...
  function Eval ( s : Link_to_Series;
                  t : double_double ) return Complex_Number is
...
-- SHIFT OPERATORS :
  ...
  function Shift ( s : Link_to_Series;
                   c : double_double ) return Link_to_Series is
  begin
    if s = null
     then return null;
  ...
  procedure Shift ( s : in out Link_to_Series; c : in double_double ) is
  begin
    if s /= null then
      declare
