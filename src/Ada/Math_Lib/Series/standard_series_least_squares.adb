...
-- AUXILIARIES :
  ...
  procedure zswap ( a : in out Standard_Complex_Series_Matrices.Matrix;
  ...
  procedure zcopy ( n,start : in integer32;
                    x : in Standard_Complex_Series_Vectors.Vector;
                    y : out Standard_Complex_Series_Vectors.Vector ) is
  ...
  function znrm2 ( a : Standard_Complex_Series_Matrices.Matrix;
  ...
  function zdotc ( row : integer32;
                   x : Standard_Complex_Series_Matrices.Matrix;
                   y : Standard_Complex_Series_Vectors.Vector )
                 return Series is
  ...
  procedure zaxpy ( a : in out Standard_Complex_Series_Matrices.Matrix; 
  ...
  procedure zscal ( a : in out Standard_Complex_Series_Matrices.Matrix;
  ...
  procedure zaxpy ( n,row,col : in integer32; f : in Series;
                    x : in Standard_Complex_Series_Matrices.Matrix;
                    y : in out Standard_Complex_Series_Vectors.Vector ) is
...
-- TARGET PROCEDURES :
  ...
  procedure QRD ( x : in out Standard_Complex_Series_Matrices.Matrix;
                  qraux : in out Standard_Complex_Series_Vectors.Vector;
    ...
    deg : constant integer32 := x(x'first(1),x'first(2)).deg;
    work : Standard_Complex_Series_Vectors.Vector(x'range(2));
  ...
  begin
    for i in work'range loop
      work(i) := new Series'(Standard_Complex_Series.Create(0,deg));
    end loop;
    pl := 1; pu := 0;
    if piv then
    ...
    Standard_Complex_Series_Vectors.Clear(work);
  end QRD;

  procedure Basis ( qr : in out Standard_Complex_Series_Matrices.Matrix;
                    x : in Standard_Complex_Series_Matrices.Matrix ) is

    deg : constant integer32 := qr(qr'first(1),qr'first(2)).deg;
    sum,acc : Series(deg);
    wrk : Standard_Complex_Series_Vectors.Vector(qr'range(1));

  begin
    for i in wrk'range loop
      wrk(i) := new Series'(Standard_Complex_Series.Create(0,deg));
    end loop;
    ...
    Standard_Complex_Series_Vectors.Clear(wrk);
  end Basis;

  procedure QRLS ( x : in out Standard_Complex_Series_Matrices.Matrix;
                   n,k : in integer32;
                   qraux,y : in Standard_Complex_Series_Vectors.Vector;
                   qy,qty,b,rsd,xb : out Standard_Complex_Series_Vectors.Vector;
...
end Standard_Series_Least_Squares;
