...
package body DecaDobl_Complex_Algebraic_Series is
  ...
  function sqrt ( c : Series; i : natural32;
    ...
    wrk,dx : Series(c.deg);
    one : constant deca_double := create(integer32(1));
    two : constant deca_double := create(integer32(2));
    half : constant deca_double := one/two;
  ...
  begin
    if AbsVal(cpc.cff(0)) < tol then
      ...
      res := Create(0,c.deg);
      res.cff(0) := DecaDobl_Complex_Numbers_Polar.Root(cpc.cff(0),2,i);
  ...
  function Root ( c : Series; n,i : natural32;
    ...
    ddn : constant deca_double := create(n);
    denominator : constant deca_double := one/ddn;
    fac : constant Complex_Number := Create(denominator);

  begin
    res.cff(0) := DecaDobl_Complex_Numbers_Polar.Root(cpc.cff(0),n,i);
    for i in 0..c.deg loop
...
end DecaDobl_Complex_Algebraic_Series;
