...
package DecaDobl_Complex_Matrix_Series is
...
--   All coefficients are in deca double precision.

  type Matrix ( deg : integer32 ) is record
    cff : DecaDobl_Complex_VecMats.VecMat(0..deg);
  end record;

-- CONSTRUCTORS :

  function Create ( A : DecaDobl_Complex_Series_Matrices.Matrix )
                  return DecaDobl_Complex_Matrix_Series.Matrix;
  ...
  function Create ( A : DecaDobl_Complex_Matrix_Series.Matrix )
                  return DecaDobl_Complex_Series_Matrices.Matrix;
