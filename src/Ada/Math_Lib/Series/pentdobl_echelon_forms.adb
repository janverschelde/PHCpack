...
package body PentDobl_Echelon_Forms is
    ...
    for i in A'range(1) loop
      for j in A'range(2) loop
        if Is_Integer(A(i,j)) then
          put(" "); put(integer32(thumb_part(REAL_PART(A(i,j)))),2);
        else
          put("  *");
        end if;
      end loop;
  ...
  procedure Swap_Rows
              ( A : in out PentDobl_Complex_Matrices.Matrix;
                i,j : in integer32 ) is
  ...
  function Max_on_Row
    ...
    res : integer32 := j;
    maxval : penta_double := AbsVal(A(i,j));
    val : penta_double;
  ...
  function Determinant
             ( L : PentDobl_Complex_Matrices.Matrix ) return Complex_Number is
  ...
  procedure Solve_with_Echelon_Form
              ( L : in PentDobl_Complex_Matrices.Matrix;
                b : in PentDobl_Complex_Vectors.Vector;
                x : out PentDobl_Complex_Vectors.Vector ) is
  ...
  procedure Multiply_and_Permute
              ( x : in out PentDobl_Complex_Vectors.Vector;
                U : in PentDobl_Complex_Matrices.Matrix;
  ...
  function Permute ( v : PentDobl_Complex_Vectors.Vector; 
  ...
  procedure Solve
              ( dim : in integer32;
                ...
                b : in PentDobl_Complex_Vectors.Vector;
                x : out PentDobl_Complex_Vectors.Vector;
                verbose : in boolean := true ) is

    L : PentDobl_Complex_Matrices.Matrix(A'range(1),A'range(2)) := A;
    U : PentDobl_Complex_Matrices.Matrix(A'range(1),A'range(2));
    row_ipvt : Standard_Integer_Vectors.Vector(A'range(1));
    col_ipvt,pivots : Standard_Integer_Vectors.Vector(A'range(2));
    pb : PentDobl_Complex_Vectors.Vector(b'range);
