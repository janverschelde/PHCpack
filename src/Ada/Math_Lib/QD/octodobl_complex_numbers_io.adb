...
package body OctoDobl_Complex_Numbers_io is
  ...
  procedure put ( s : out string; c : in Complex_Number ) is
    ...
    x : constant octo_double := REAL_PART(c);
    y : constant octo_double := IMAG_PART(c);

  begin
    s := (s'range => ' ');
    to_string(x,128,0,false,false,true,' ',re,re_ends);
    to_string(y,128,0,false,false,true,' ',im,im_ends);
    declare
      t : constant string
