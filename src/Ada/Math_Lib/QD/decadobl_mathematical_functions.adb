...
-- AUXILIARY ROUTINES for SIN and COS :
  ...
  procedure sincos_taylor
              ( x : in deca_double; sin_x,cos_x : out deca_double ) is
  ...
  --   a Taylor series approximation respectivley for sin(x) and cos(x).
    ...
    thresh : constant double_float := 0.5*da_eps*f;
    p,s,t,y : deca_double; 
    i : integer;
  ...
  function sin_taylor ( x : deca_double ) return deca_double is
  ...
  function cos_taylor ( x : deca_double ) return deca_double is
  ...
  procedure Reduce_Modulo_2pi
              ( x : in deca_double; t : out deca_double;
  ...
  begin
   ...
   -- approximately reduce modulo pi/2 and then modulo pi/1024
    q := double_float'floor(thumb_right(r)/thumb_right(pi2) + 0.5);
    t := r - pi2*q;
    j := integer(q);
    if j < -2 or j > 2 then
      ...
      q := double_float'floor(thumb_right(t)/thumb_right(pi1024) + 0.5);
      t := t - pi1024*q;
      k := integer(q);
      if k < 0
       then abs_k := -k;
...
-- TARGET TRIGONOMETRIC FUNCTIONS :
  ...
  function SIN ( x : deca_double ) return deca_double is
  ...
  function COS ( x : deca_double ) return deca_double is
  ...
  procedure SINCOS ( x : in deca_double; sin_x,cos_x : out deca_double ) is
  begin
    if is_zero(x) then
      sin_x := Create(0.0);
      cos_x := Create(1.0);
  ...
  function TAN ( x : deca_double ) return deca_double is
  ...
  function ARCTAN ( x : deca_double ) return deca_double is
  ...
  function ARCTAN ( y,x : deca_double ) return deca_double is
  ...
  begin
    if is_zero(x) then
      ...
      declare
        r : constant deca_double := SQRT(sqr(x) + sqr(y));
        xx : constant deca_double := x/r;
        yy : constant deca_double := y/r;
        f : constant double_float
          := Standard_Mathematical_Functions.Angle(to_double(y),to_double(x));
        z : deca_double := create(f);
        sin_z,cos_z : deca_double;
      begin
        sincos(z,sin_z,cos_z);
        if abs(thumb_right(xx)) > abs(thumb_right(yy)) then
  ...
  function ARCSIN ( x : deca_double ) return deca_double is
  begin
    if x < -1.0 or x > 1.0 then
      ...
      declare
        one : constant deca_double := create(1.0);
        s1x : constant deca_double := SQRT(one - sqr(x));
      begin
        return ARCTAN(x,s1x);
      end;
    end if;
  ...
  function ARCCOS ( x : deca_double ) return deca_double is
  begin
    if x < -1.0 or x > 1.0 then
  ...
  function Angle ( x,y : deca_double ) return deca_double is

    first : constant deca_double := x;
    second : constant deca_double := y;
