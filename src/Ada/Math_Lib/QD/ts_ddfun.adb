...
procedure ts_ddfun is

  procedure Test_SINCOS is

  -- DESCRIPTION :
  --   Verifies if the square of sine and cosine on a random double double
  --   or a user given number sum up to one.

    x,s,c,y,t,a,inv_s,inv_c,err : double_double;
  ...
  begin
    ...
    loop
      ...
      Ask_Yes_or_No(ans);
      if ans = 'y'
       then x := DoblDobl_Random_Numbers.Random;
  ...
  -- DESCRIPTION :
  --   Compares the square of the square root of a random or given number.

    x,y,z,err : double_double;
    ans : character;
  ...
  begin
    ...
    loop
      ...
      if ans /= 'y' then
        ...
        x := DoblDobl_Random_Numbers.Random;
        if is_negative(x)
         then Min(x);
        end if;
      end if;
                    put("        x : "); put(x); new_line;
      y := sqrt(x); put("  sqrt(x) : "); put(y); new_line;
      z := sqr(y);  put("sqrt(x)^2 : "); put(z); new_line;
      err := x - z;
  ...
  begin
    new_line;
    put_line("MENU for testing double double mathematical functions :");
    put_line("  1. test square root of double doubles");
    put_line("  2. test cos, sin, tan and their inverses for double doubles");
