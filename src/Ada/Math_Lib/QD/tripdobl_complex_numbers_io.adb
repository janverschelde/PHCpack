...
package body TripDobl_Complex_Numbers_io is
  ...
  procedure put ( s : out string; c : in Complex_Number ) is
    ...
    x : constant triple_double := REAL_PART(c);
    y : constant triple_double := IMAG_PART(c);

  begin
    s := (s'range => ' ');
    to_string(x,50,0,false,false,true,' ',re,re_ends);
    to_string(y,50,0,false,false,true,' ',im,im_ends);
    declare
      t : constant string
