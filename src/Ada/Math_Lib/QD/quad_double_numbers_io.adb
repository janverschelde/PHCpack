...
package body Quad_Double_Numbers_io is

  procedure read ( s : in string; ps : in out integer;
                   x : out quad_double; fail : out boolean ) is
  ...
  begin
    s(s'first) := ' '; expn := 0;
    if hihi_part(x) = 0.0 then          -- x equals zero
  ...
  procedure write ( x : in quad_double; precision : in natural32 ) is
  ...
  procedure get ( x : in out quad_double ) is
  ...
  procedure get ( file : in file_type; x : in out quad_double ) is
  ...
  procedure get ( x,y : in out quad_double ) is
  ...
  procedure get ( file : in file_type; x,y : in out quad_double ) is
  ...
  procedure put ( x : in quad_double ) is
  begin
    put(standard_output,x,64);
  end put;

  procedure put ( file : in file_type; x : in quad_double ) is
  begin
    put(file,x,64);
  end put;

  procedure put ( x : in quad_double; precision : in natural32 ) is
  begin
