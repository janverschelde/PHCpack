...
package body OctoDobl_Complex_Poly_Strings is
...
-- NOTE : The implementation is a wrapper to Multprec_Complex_Poly_Strings.
  ...
  function Multprec_Terms_to_OctoDobl_Complex
             ...
             return OctoDobl_Complex_Term_Lists.Term_List is

    res,res_last : OctoDobl_Complex_Term_Lists.Term_List;
  ...
  begin
    while not Multprec_Complex_Term_Lists.Is_Null(tmp) loop
      ...
      declare
        ddt : OctoDobl_Complex_Polynomials.Term;
      begin
        ddt.cf := Multprec_to_OctoDobl_Complex(mpt.cf);
        ddt.dg := OctoDobl_Complex_Polynomials.Degrees(mpt.dg);
        -- the Append makes a copy anyway
        OctoDobl_Complex_Term_Lists.Append(res,res_last,ddt);
      end;
      tmp := Multprec_Complex_Term_Lists.Tail_Of(tmp);
    end loop;
    return res;
  end Multprec_Terms_to_OctoDobl_Complex;
  ...
  begin
    ...
    p := Multprec_Polynomial_to_OctoDobl_Complex(q);
  ...
  procedure Parse ( s : in string; k : in out integer;
    ...
    first,second : OctoDobl_Complex_Term_Lists.Term_List;

  begin
    Multprec_Complex_Poly_Strings.Parse(s,k,n,size,q,q_last);
    p := Multprec_Terms_to_OctoDobl_Complex(q);
    Multprec_Complex_Term_Lists.Clear(q);
    if OctoDobl_Complex_Term_Lists.Is_Null(p) then
      p_last := p;
    else
      first := p;
  ...
  function Write ( p : Poly ) return string is
    ...
    q : Multprec_Complex_Polynomials.Poly
      := OctoDobl_Complex_to_Multprec_Polynomial(p);
...
end OctoDobl_Complex_Poly_Strings;
