...
package body DoblDobl_Complex_Poly_Randomizers is
  ...
  function Complex_Randomize ( p : Poly ) return Poly is
    ...
    begin
      rt.cf := DoblDobl_Random_Numbers.Random;
  ...
  function Complex_Randomize1 ( p : Poly ) return Poly is
    ...
    begin
      rt.cf := DoblDobl_Random_Numbers.Random1;
      rt.dg := new Standard_Natural_Vectors.Vector'(t.dg.all);
      Add(res,rt);
      Clear(rt);
      cont := true;
...
end DoblDobl_Complex_Poly_Randomizers;
