...
package body HexaDobl_Complex_Poly_Strings is
...
-- NOTE : The implementation is a wrapper to Multprec_Complex_Poly_Strings.
  ...
  function Multprec_Terms_to_HexaDobl_Complex
             ...
             return HexaDobl_Complex_Term_Lists.Term_List is

    res,res_last : HexaDobl_Complex_Term_Lists.Term_List;
  ...
  begin
    while not Multprec_Complex_Term_Lists.Is_Null(tmp) loop
      ...
      declare
        ddt : HexaDobl_Complex_Polynomials.Term;
      begin
        ddt.cf := Multprec_to_HexaDobl_Complex(mpt.cf);
        ddt.dg := HexaDobl_Complex_Polynomials.Degrees(mpt.dg);
        -- the Append makes a copy anyway
        HexaDobl_Complex_Term_Lists.Append(res,res_last,ddt);
      end;
      tmp := Multprec_Complex_Term_Lists.Tail_Of(tmp);
    end loop;
    return res;
  end Multprec_Terms_to_HexaDobl_Complex;
  ...
  begin
    ...
    p := Multprec_Polynomial_to_HexaDobl_Complex(q);
  ...
  procedure Parse ( s : in string; k : in out integer;
    ...
    first,second : HexaDobl_Complex_Term_Lists.Term_List;

  begin
    Multprec_Complex_Poly_Strings.Parse(s,k,n,size,q,q_last);
    p := Multprec_Terms_to_HexaDobl_Complex(q);
    Multprec_Complex_Term_Lists.Clear(q);
    if HexaDobl_Complex_Term_Lists.Is_Null(p) then
      p_last := p;
    else
      first := p;
  ...
  function Write ( p : Poly ) return string is
    ...
    q : Multprec_Complex_Polynomials.Poly
      := HexaDobl_Complex_to_Multprec_Polynomial(p);
...
end HexaDobl_Complex_Poly_Strings;
