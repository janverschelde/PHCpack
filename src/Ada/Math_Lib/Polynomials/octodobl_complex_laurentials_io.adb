...
package body OctoDobl_Complex_Laurentials_io is
...
-- THE INPUT OPERATIONS :
  ...
  begin
    Multprec_Complex_Laurentials_io.Set_Working_Precision(20);
    Multprec_Complex_Laurentials_io.get(file,mp);
    p := Multprec_Laurential_to_OctoDobl_Complex(mp);
...
-- AUXILIARIES FOR OUTPUT ROUTINES :
  ...
  -- DESCRIPTION :
  --   Writes the octo double complex number to file
  --   in the format ( real_part(c) + imag_part(c)*I ).

    re : constant octo_double := REAL_PART(c);
    im : constant octo_double := IMAG_PART(c);
...
end OctoDobl_Complex_Laurentials_io;
