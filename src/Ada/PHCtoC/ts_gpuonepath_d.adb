...
procedure ts_gpuonepath_d is

-- DESCRIPTION :
--   Procedure to test the development of the acceleration of the tracking
--   of one solution path, in standard double precision.
--   The Ada procedure remains in control.
--   Data is passed to the C++ code via the systems and solutions containers.

  procedure Standard_GPU_Track ( execmode,verbose : integer32 ) is

  -- DESCRIPTION :
  --   Calls the accelerated path tracker standard double precision.
  ...
  --   If verbose > 0, then additional output is written to screen.
    ...
    pragma import(C, track, "gpuonepath_d");

  begin
    return_of_call := track(execmode,verbose);
  end Standard_GPU_Track;
  ...
  begin
    ...
    Standard_GPU_Track(execmode,verbose);
