...
double test_real ( int dim, int nvr, int pwr, int deg )
{
   ...
   if(nvr > 2)
   {
      ...
      for(int i=0; i<=deg; i++)
      {
         cout << outputhihihi_d[dim][i] << "  " << outputlohihi_d[dim][i]
              << endl;
         cout << outputhilohi_d[dim][i] << "  " << outputlolohi_d[dim][i]
              << endl;
         cout << outputhihilo_d[dim][i] << "  " << outputlohilo_d[dim][i]
              << endl;
         cout << outputhilolo_d[dim][i] << "  " << outputlololo_d[dim][i]
              << endl;
         errsum = errsum 
...
double test_complex ( int dim, int nvr, int pwr, int deg )
{
   ...
   for(int i=0; i<dim; i++)
   {
      ...
      for(int j=0; j<=deg; j++)
      {
         ...
         cout << inputrehilohi[i][j] << "  " << inputrelolohi[i][j] << endl;
         cout << inputrehihilo[i][j] << "  " << inputrelohilo[i][j] << endl;
         cout << inputrehilolo[i][j] << "  " << inputrelololo[i][j] << endl;
         cout << inputimhihihi[i][j] << "  " << inputimlohihi[i][j] << endl;
         cout << inputimhilohi[i][j] << "  " << inputimlolohi[i][j] << endl;
         cout << inputimhihilo[i][j] << "  " << inputimlohilo[i][j] << endl;
         cout << inputimhilolo[i][j] << "  " << inputimlololo[i][j] << endl;
      }
   }
   double errsum = 0.0;
   double errtot = 0.0;
