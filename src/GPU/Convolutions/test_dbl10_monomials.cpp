...
double test_complex ( int dim, int nvr, int pwr, int deg )
{
   ...
   for(int i=0; i<=deg; i++)
   {
      cout << outputrertb_h[dim][i] << "  " << outputrerix_h[dim][i]
                                    << "  " << outputrermi_h[dim][i] << endl
           << outputrerrg_h[dim][i] << "  " << outputrerpk_h[dim][i] << endl;
      cout << outputreltb_h[dim][i] << "  " << outputrelix_h[dim][i]
                                    << "  " << outputrelmi_h[dim][i] << endl
           << outputrelrg_h[dim][i] << "  " << outputrelpk_h[dim][i] << endl;
      cout << outputimrtb_h[dim][i] << "  " << outputimrix_h[dim][i]
                                    << "  " << outputimrmi_h[dim][i] << endl
           << outputimrrg_h[dim][i] << "  " << outputimrpk_h[dim][i] << endl;
      cout << outputimltb_h[dim][i] << "  " << outputimlix_h[dim][i]
                                    << "  " << outputimlmi_h[dim][i] << endl
           << outputimlrg_h[dim][i] << "  " << outputimlpk_h[dim][i] << endl;
   }
   if(nvr == dim) // the product of all input series equals one
   {
      for(int i=0; i<=deg; i++)
