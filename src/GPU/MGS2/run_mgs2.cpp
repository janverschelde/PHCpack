...
int main ( int argc, char *argv[] )
{
   ...
   if(mode < 9)
   {
      ...
      if(mode > 5) // extra right hand side column
      {
         for(int j=0; j<dim; j++)
         {
            double temp = random_double()*2*M_PI;
   ...
   if(mode < 3)
      run_orthonormalization(v,v_h,BS,dim,freq,mode);
   else if(mode < 6)
      run_QR_decomposition(v,v_h,BS,dim,freq,mode);
   else if(mode < 9)
      run_QR_least_squares(v,v_h,BS,dim,freq,mode);
   else
      run_newton_on_chandra(v,v_h,BS,dim,freq,mode);

   if(mode == 2 || mode == 5 || mode == 8)
      cout << "seed for random numbers : " << timevalue << endl;
 ...
 ( complexH<T1>** v, complex<T>* v_h, int BS, int dim, int freq, int mode )
{
   ...
   for(int i=0; i<freq; i++)
   {
      ...
      if(mode == 9 || mode == 11)
      {
         ...
         GPU_mgs2qrls(v_h,R_h,sol_h,dimR,dim,dim+1,1,BS);
         for(int k=0; k<dim; k++) comp1_gqd2qd(&sol_h[k],&sol[k]);
      }
      if(mode == 10 || mode == 11) CPU_mgs2qrls(v,R,sol,dim,dim+1);
