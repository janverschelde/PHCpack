...
T1 mgs_test_any(Workspace& workspace_cpu, CPUInstHom& cpu_inst_hom, int device_option, int n_path) {
	...
	if(gpu_test == 1){
		...
		if(n_path == 1){
			...
			GPU_MGS(cpu_inst_hom, sol_gpu[0], matrix_gpu_q[0], matrix_gpu_r[0], V_data);
		}
		else{
			GPU_MGS_Mult(cpu_inst_hom, sol_gpu, matrix_gpu_q, matrix_gpu_r, V_data, n_path);
		}
	}
...
T1 mgs_test_large(Workspace& workspace_cpu, CPUInstHom& cpu_inst_hom) {
	...
	// GPU MGS
	GPU_MGS(cpu_inst_hom, sol_gpu, matrix_gpu_q, matrix_gpu_r, workspace_cpu.matrix);
	// CPU MGS
