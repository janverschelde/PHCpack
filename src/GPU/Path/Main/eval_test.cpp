...
T1 eval_compare(const CPUInstHom& cpu_inst_hom, \
		CT* gpu_workspace, CT* gpu_matrix, const CT* cpu_workspace, const CT* cpu_matrix)
{
...
T1 eval_test_classic(Workspace& workspace_cpu, CPUInstHom& cpu_inst_hom, CT* sol0, CT t, PolySys& Classic_Sys, int n_path) {
	...
	if(classic_check){
		...
		for(int sol_idx=0; sol_idx<n_path; sol_idx++){
			err_check_class_workspace(deri_val[sol_idx], f_val[sol_idx], workspace_cpu_all[sol_idx].matrix, n_eq, dim);
		}
	...
	CT** gpu_matrix_all;
	gettimeofday(&start, NULL);
	GPU_Eval(cpu_inst_hom, sol, t_mult, gpu_workspace_all, gpu_matrix_all, n_path, x_t_idx, n_predictor);
	gettimeofday(&end, NULL);
	seconds  = end.tv_sec  - start.tv_sec;
	useconds = end.tv_usec - start.tv_usec;
	...
	for(int sol_idx=0; sol_idx<n_path; sol_idx++){
		...
		T1 err_tmp = eval_compare(cpu_inst_hom, gpu_workspace_all[sol_idx], gpu_matrix_all[sol_idx], \
				workspace_cpu_all[sol_idx].all, workspace_cpu_all[sol_idx].matrix);
		if(err_tmp > err){
			err = err_tmp;
		}
