// The file workspace.tpp contains the definitions for the prototypes 
// listed in workspace_host.h.
   ...
   int n_eq, int dim, int n_predictor, int* max_deg_base, int verbose )
{
   ...
   V[0] = matrix;
   for(int i=0; i<dim; i++) V[i+1] = V[i] + n_eq;
...
{
   std::cout << "------- Initialize x and t value for Testing only ---------"
             << std::endl;
