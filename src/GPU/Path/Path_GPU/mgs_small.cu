...
void mgs_small_idx(GT* V, GT* R, GT* sol, int rows, int cols, size_t small_mgs_size, int n_matrix, int n_matrix_R, int n_path, int* path_idx) {
	//int rows = dim;
	...
	mgs_small_kernel_idx<<<n_path,32,small_mgs_size>>>(V,R,sol,rows,rowsLog2-1,half_size_init,cols,n_matrix, n_matrix_R, path_idx);
}
...
void mgs_small1_idx(GT* V, GT* R, GT* sol, int rows, int cols, int n_matrix, int n_matrix_R, int n_path, int* path_idx) {
	...
	for(int piv=0; piv<cols-1; piv++) {
		mgs_small_normalize_kernel1_idx<16><<<n_path,BS>>>
		(V,R,dimR,rows,half_size_init,cols,piv,n_matrix,n_matrix_R,path_idx);
		mgs_small_reduce_kernel16_idx<16><<<n_path,32>>>
		(V,R,dimR,rows,half_size_init,cols,piv,n_matrix,n_matrix_R,path_idx);
	}
...
extern __shared__ GT shared_array[];
template <unsigned int n_th>
__global__ void mgs_small_dynamic_kernel_template(GT* V, GT* R, GT* sol, int rows, int rowsLog2, int half_size_init, int cols, int n_matrix, int n_matrix_R){
	// 10
	GT* V_shared = shared_array;// contains pivot column
	GT* R_shared = V_shared + n_matrix;// contains pivot column
	GT* tmp_col= R_shared + n_matrix_R;// temporary column for pivot column * reduction column
...
template <unsigned int n_th>
__global__ void mgs_small_normalize_kernel1_idx(GT* v, GT* R, \
int dimR, int rows, int half_size, int cols, int pivot, int n_matrix, int n_matrix_R, int* path_idx) {
	//int b = blockIdx.x;
	int t_idx = threadIdx.x;
	//int block = b+pivot;    // column for reduction w.r.t. pivot
	//int i = block*rows + t_idx; // idx
