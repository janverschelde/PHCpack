...
void eval_mon_tree(GPUWorkspace& workspace, const GPUInst& inst){
	...
	for(int i=0; i<last_level; i++){
		if(i==0){
			...
			eval_mon_tree_kernel<256><<<inst.mon_level_grid[9], inst.mon_level_BS>>>(workspace.mon, workspace.x, workspace_coef_tmp,\
					pos_start_tmp, inst.mon_pos, inst.n_mon_level[i]);
		}
	...
	if(inst.level > max_level+1){
		...
		if(max_level == 6){
			...
			dim3 mon_level_rest_grid = get_grid(n_mon_tmp, inst.mon_level_BS, 1, 256);
			eval_mon_tree_n_kernel<256><<<mon_level_rest_grid, inst.mon_level_BS>>>(workspace.mon, workspace.x, workspace_coef_tmp,\
					pos_start_tmp, inst.mon_pos, n_mon_tmp);
		}
	}
}
