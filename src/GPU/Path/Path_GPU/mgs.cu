...
int GPU_MGS_Mult(const CPUInstHom& hom, CT**& sol_gpu, CT**& matrix_gpu_q,  CT**& matrix_gpu_r, CT* V, int n_path) {
	...
	if(hom.dim <= BS_QR){
		...
		//mgs_small(workspace.V, workspace.R, workspace.sol, hom.n_eq, hom.dim+1, workspace.n_matrix, workspace.n_path);
