__global__ void path_mult_x_init_kernel(GT* x_mult_horizontal, GT* x_array_mult, int workspace_size, int dim, int n_predictor){
	int idx = threadIdx.x;
	//int idx = bidx + tidx;
...
__global__ void path_mult_x_finish_kernel(GT* x_mult_horizontal, GT* x_array_mult, int* x_t_idx_mult, int workspace_size, int dim, int n_predictor){
	int idx = threadIdx.x;
	//int idx = bidx + tidx;
	int path_idx = blockIdx.x;
...
__global__ void path_mult_init_kernel(GT* t_mult, GT* t_last_mult, GT* delta_t_mult, GT* t_array_mult,  GT* one_minor_t_mult, GT* alpha, \
		int* path_success, int* newton_success, int* end_range, int* n_success, double* max_f_val_last, \
		int* n_point_mult, int* x_t_idx_mult, int workspace_size, int* path_idx_mult, int n_path, GT* matrix_mult, int n_sum_zero, int* sum_zeros, int n_predictor){
