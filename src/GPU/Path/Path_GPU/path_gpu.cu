...
bool GPU_Path(CPUInstHom& hom, Parameter path_parameter, CT* cpu_sol0, CT cpu_t, CT*& x_gpu, int n_path, int inverse) {
	cuda_set();

	std::cout << "n_path = " << n_path << std::endl;

	GPUInst inst(hom, n_path);
	//std::cout << "mon_pos_block_size = " << hom.CPU_inst_hom_block.mon_pos_block_size \
	...
	bool success = path_single(workspace, inst, path_parameter, cpu_t, n_path, inverse);
