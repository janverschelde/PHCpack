...
bool path_single
 ( GPUWorkspace& workspace, GPUInst& inst, Parameter path_parameter,
   CT cpu_t, int n_path, int inverse = 0 )
{
   bool debug = false;
   // debug = true;
   int n_point = 1;
   ...
   *tmp_t_last = cpu_t;
   ...
   while(tmp_t_last->real < T1(1))
   {
      ...
      predict_newton_kernel
         <<<inst.predict_grid, inst.predict_BS>>>
      ...
      bool newton_success = newton_single(workspace,inst,
         path_parameter,end_range);
   ...
   CT*& x_gpu, int n_path, int inverse )
{
   ...
   bool success = path_single(workspace, inst,path_parameter,cpu_t,
                              n_path,inverse);
