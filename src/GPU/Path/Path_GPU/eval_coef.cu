...
__global__ void eval_coef_kernel(GT* workspace_coef, const GT* coef_orig, int n_coef, \
...
//		workspace_coef[idx] = (*t)*coef_orig[2*idx] + (*one_minor_t)*coef_orig[2*idx+1];
		workspace_coef[idx] = (*t)*coef_orig[idx] + (*one_minor_t)*coef_orig[idx+n_coef];
	}
}
...
__global__ void eval_coef_kernel(GT* workspace_coef, const GT* coef_orig, int n_coef, \
		GT* t, GT* one_minor_t, int workspace_size, int* x_t_idx, int* path_idx_mult) {
	//__shared__ GT div_diff_sh[shmemsize];
    int bidx = (gridDim.x*blockIdx.y+blockIdx.x)*blockDim.x;
	int tidx = threadIdx.x;
	int idx = bidx + tidx;
...
//template <unsigned int n_th>
__global__ void eval_coef_copy_kernel(GT* workspace_coef, const GT* coef_orig, int n_coef) {
	//__shared__ GT div_diff_sh[shmemsize];
    int bidx = (gridDim.x*blockIdx.y+blockIdx.x)*blockDim.x;
	int tidx = threadIdx.x;
	int idx = bidx + tidx;
