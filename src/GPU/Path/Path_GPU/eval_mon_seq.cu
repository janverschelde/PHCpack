...
__global__ void eval_mon_seq_with_base_kernel(GT* workspace_mon, GT* x, GT* workspace_coef,
int* mon_pos_start, unsigned short* mon_pos, unsigned short* mon_exp, int n_mon) {
	int idx = (gridDim.x*blockIdx.y+blockIdx.x)*blockDim.x + threadIdx.x;
...
void eval_mon_seq_with_base(GPUWorkspace& workspace, const GPUInst& inst){
	eval_mon_single_with_base_kernel<<<inst.mon_level_grid[0], inst.mon_level0_BS>>>(
			workspace.mon, workspace.x, workspace.coef, inst.mon_pos_start,
