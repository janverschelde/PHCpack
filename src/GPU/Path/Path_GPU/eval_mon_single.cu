...
__global__ void eval_mon_single_with_base_kernel(GT* workspace_mon, GT* x, GT*workspace_coef, \
                           int* mon_pos_start, unsigned short* mon_pos, unsigned short* mon_exp, \
                           int n_mon) {
	int idx = (gridDim.x*blockIdx.y+blockIdx.x)*blockDim.x + threadIdx.x;
...
__global__ void eval_mon_single_kernel(GT* workspace_mon, GT* x, GT*workspace_coef, \
                           int* mon_pos_start, unsigned short* mon_pos, \
                           int n_mon, int workspace_size, int* x_t_idx, int dim) {
	int idx = (gridDim.x*blockIdx.y+blockIdx.x)*blockDim.x + threadIdx.x;
...
__global__ void eval_mon_single_kernel(GT* workspace_mon, GT* x, GT*workspace_coef, \
                           int* mon_pos_start, unsigned short* mon_pos, \
                           int n_mon, int workspace_size, int* x_t_idx, int dim, int* path_idx_mult) {
	int idx = (gridDim.x*blockIdx.y+blockIdx.x)*blockDim.x + threadIdx.x;
