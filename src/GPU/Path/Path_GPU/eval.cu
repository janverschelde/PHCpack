...
//	std::cout << "inst.dim = " << inst.dim << std::endl;
	...
	if(inst.PED_hom == true){
		...
		eval_coef_copy_kernel<<<inst.coef_grid, inst.coef_BS>>>(workspace.coef,\
				inst.coef, inst.n_coef);
	}

	if(inst.max_deg_base!= NULL){
		eval_base_table_kernel<<<1, inst.dim>>>(inst.dim, inst.max_deg_base,\
				inst.base_table_start, workspace.x, workspace.deg_table);

		std::cout << "n_constant = " << inst.n_constant \
				  << " n_mon_base_start = " << inst.n_mon_base_start << std::endl;

		eval_base_kernel<<<1, inst.n_mon_base>>>(inst.n_mon_base, inst.mon_pos,\
				inst.mon_exp, inst.mon_pos_start + inst.n_mon_base_start, \
...
int GPU_Eval(const CPUInstHom& hom, CT* cpu_sol0, CT* cpu_t, \
	...
	GPUWorkspace workspace(inst.mon_pos_size, inst.n_coef, inst.n_constant, inst.n_eq,\
