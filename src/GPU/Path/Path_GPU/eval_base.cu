__global__ void eval_base_table_kernel(int dim, int* max_deg_base,\
		int* base_table_start, GT* x, GT* base_table) {
...
__global__ void eval_base_kernel(int n_mon_base, unsigned short* pos, unsigned short* exp,\
	...
	if(idx < n_mon_base) {
		...
		for(int var_idx=base_start; var_idx<n_var; var_idx++){
			tmp_coef *= base_table[base_table_start[mon_pos[var_idx]]+mon_exp[var_idx]-2];
		}
		coef[idx] = tmp_coef;
	}
}
